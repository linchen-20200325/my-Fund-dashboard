...
| `notes` | str | 選填 | `第一年` | 備註 |
| `policy_tier` | str | 選填 (P3) | `core` / `satellite` | 該基金在保單內定位；~~空白 fallback 為既有基金名啟發~~ → **空白就是「尚未決定」，系統不猜、也不寫回**（見下方「⚠️ 2026 政策變更」）。大小寫不分，非法值視為空。|

**(policy_id, fund_url)** 是這張表的主鍵 — 即「同一張保單下不會有兩筆同檔基金的重複列」，但「同檔基金可以在不同保單裡各出現一次」。

**向後相容**：舊 8 欄 Sheet 不需立即加 `policy_tier`；儀表板會自動降級為空字串。
~~並使用既有基金名啟發判定核心/衛星~~（**已撤銷，見下方政策變更**）。新建 Sheet 建議直接寫 9 欄完整 schema。

---

### ⚠️ 2026 政策變更：空白就是空白，系統不猜

> **有意識的政策變更，不是漏刪。** 決策者：**客戶**。
> 舊表述（上方兩處刪除線）依本 repo 慣例**保留不刪**，兩邊理由並陳如下。

**現行行為**

| `policy_tier` 欄 | 系統怎麼看 | 存檔時寫回什麼 |
|---|---|---|
| `core` | 核心 | `core` |
| `satellite` | 衛星 | `satellite` |
| **空白 / 非法值** | **尚未決定**（不是「衛星」） | **空白** —— 原樣不動 |

**舊規則當初為什麼合理**：這張表是選填欄，早期多數使用者不會填。
有一個 fallback，畫面上的核心/衛星比例才不會整組空掉 ——
「先給一個大致的分類，總比什麼都不顯示好」在當時是成立的權衡。

**被權衡掉的是什麼**：那個 fallback 不只用於**顯示**，它還會被**寫回這張 Sheet**。

1. 每次「載入基金資料」都用基金名稱關鍵字猜一個級別，**覆寫**從本表讀回的值；
2. 下一次「全部寫入」再把猜測寫回本表。

⇒ 使用者留白的欄位、以及**親手填過的值**，都會被一次往返永久改寫，
而且每存一次再蓋一次 —— 這是**不可逆的資料汙染**，代價遠大於「比例不要空掉」。

猜測本身也結構上必然錯：關鍵字表把「配息」列為**核心**特徵且**優先檢查**，
而台灣基金名稱依法幾乎都帶「(基金之配息來源可能為本金)」
⇒ 只要是配息型，**不管它實際上是科技、生技還是半導體基金，一律被判成核心**。

**你要做什麼**：想指定級別就填 `core` / `satellite`；**不想指定就留白** ——
留白會被原樣保留，不會再被系統改掉。

⚠️ **顯示端尚未同步**：畫面上的核心/衛星**比例**目前仍把「尚未決定」計入衛星，
說明文字也還寫著「其餘以基金名稱關鍵字推定」。
**那是下一批要處理的，本批只負責先讓你的 Sheet 不再被寫壞。**
