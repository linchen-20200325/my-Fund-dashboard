...
>
> **本輪實測（指令與輸出見 §7.7）**：
> - `git grep -n "realized_pnl" -- '*.py'` → **0 命中**（全 repo 唯二命中是本文件自己那兩行）。
...
|---|---|---|
| ~~`r"~~.+?~~"` ＋ `re.S`~~ | **431 / 1898** | **40** |
| **`r"~~[^\n]+?~~"`（不跨行、不加 `re.S`）** | **30 / 1898** | **61** |

⇒ **那個 bug 讓本文件 401 行、21 個候選對這條指令完全隱形** ——
而其中就包含本節自己剛寫下的那幾句。
⚠️ **它長得像通過**：它回了 40 個候選、看起來很忙，**沒有任何跡象顯示它瞎了兩成的文件**。
...
**修正後本輪實跑（兩次，含負對照）**：
- **正常情形**：於 repo 根目錄跑上方（已修正的）指令 → **回 61 個候選**
  （量測日 2026-09-14，量測對象為本檔定稿），**exit 0**。
  ⚠️ **61 是候選數，不是全稱句數** —— 其中含偽陽性（如「保單**唯一**識別碼」），
