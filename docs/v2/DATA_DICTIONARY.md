...
| 版本 | 被跳過的行 | 回出的候選 |
|---|---|---|
| ⛔ **第一版**：`r"~~.+?~~"` ＋ `re.S`（跨行） | **431 / 1903** | **41** |
| **`r"~~[^\n]+?~~"`（不跨行、不加 `re.S`）** | **30 / 1903** | **62** |

⇒ **那個 bug 讓本文件 401 行、21 個候選對這條指令完全隱形** ——
而其中就包含本節自己剛寫下的那幾句。
...
3. ⭐ **§7.4.1 那條偵測指令自己有 bug，回了 41 個候選卻瞎了本檔兩成以上的行**（`431 / 1903`）—— 詳見該節。
