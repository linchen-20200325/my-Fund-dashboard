...
⚠️ **本文件確實含有「只有 N 處」「唯一」「全部都是 X」這類可被一條指令推翻的全稱句** ——
**本節的規則不是「不寫」，而是「寫了就要附上它的 scope 與可複跑指令」**（`CLAUDE.md §-1.5.1c 判定 2`
的方法教訓）。**完整說明與偵測指令見 §7.4。**

> ⚠️ **2026-09-14 第四輪就地更正（有意識的更正，不是漏刪；由獨立稽核抓出）**：
> 本段原寫 ~~「本文件**刻意不寫**『只有 N 處』『全部都是 X』這類能被一條 grep 推翻的全稱句」~~
> —— **這句自我描述本身為假**（§7.4 同句一併更正）。
> ⛔ **它的危害不在於「說錯一句話」，而在於它會關掉讀者的檢查**：
> 一個讀到「本文件沒有這種句子」的人，**就不會再去檢查這種句子**。
> **一句假的自我描述，比它所描述的那些句子更危險。**
...
| T3 | **MoneyDJ**（主站 + `tcbbankfund` + `chubb` 等子網域） | NAV 歷史、績效 wb01、風險 wb07、配息 wb05、持股 | `repositories/fund/nav_metrics.py`（`fetch_nav`／`fetch_performance_wb01`／`fetch_risk_metrics`／`fetch_div`／`fetch_holdings`） | NAV T+1 ～ T+3；**wb01 績效與 wb07 風險表為月更**；⚠️ **配息（wb05／`funddividend`）為「月／季更」，不是月更**（**2026-09-14 第四輪更正**，見下註）；持股月更 |
...
> ⚠️ **2026-09-14 第四輪就地更正（有意識的更正，不是漏刪）——「更新頻率」欄把三個不同的頻率壓成一個**：
> 本列原寫 ~~「wb01/wb05/wb07 為**月更**」~~。**實測程式碼各自的自陳（AST 把每個 `@_daily_cache`
> 註解對回它所裝飾的函式，非行窗猜測）**：
>
> | 函式 | 程式碼自陳 |
> |---|---|
> | `fetch_performance_wb01` | 「MoneyDJ 績效頁**月更**」 |
> | `fetch_risk_metrics`（wb07） | 「wb07 風險表**月更**」 |
> | **`fetch_div`（wb05／`funddividend`）** | 「配息為歷史型資料（**月／季更**）」 |
>
> ⇒ **wb05 是「月／季更」，被原列一併寫成「月更」。** 本列原也**漏列 `fetch_div`**（用途欄寫了配息 wb05，
> 實作欄卻沒有對應函式），一併補上。
> ⚠️ **承重方向要講清楚：這個錯會讓新鮮度預算偏緊，不是偏鬆** ——
> 把季更資料當月更看，會在**第 2、3 個月把正常的「尚未更新」誤判成「資料過期／抓取失敗」**。
> 對照 `CLAUDE.md §2.4`：過期判定要吃**該來源真實的發布節奏**，不是全部套同一個數。
...
- ⚠️ **既有分頁的欄數不是一個定值，至少有三種形狀** —— `_get_worksheet` 有「只補缺的那幾格」的
  header 修補邏輯（`_NAV_HEADERS[len(_hdr):]`，一般化寫法，不是固定補一格）：

  | 既有 header 形狀 | 欄 | 補的範圍 | 來歷 |
  |---|---|---|---|
  | **3 欄最小 schema** | `code`\|`date`\|`nav` | 補 `D1:G1` **四格** | **user 2026-08-19 明文要求支援**（使用者手動只維護這三欄） |
  | **6 欄舊 schema** | 上表前 6 欄 | 補 `G1` **一格** | `currency` 為後加 |
  | **7 欄現行** | `_NAV_HEADERS` 全欄 | 不補 | — |

  ⇒ **v2 ETL 必須容忍上列三種歷史形狀**（見 §2.5），**且不得以欄數推斷欄義** ——
  v1 本身**不讀表頭文字**（`load_points`／`append_points` 皆 `get_all_values()[1:]` 跳過第 1 列、
  再以位置取值），表頭是給人看的，使用者可自行改名。

> ⚠️ **2026-09-14 第四輪就地更正（有意識的更正，不是漏刪；由獨立稽核抓出）**：
> 本段原寫 ~~「**程式碼自陳**既有分頁可能只有 **6 欄**（`currency` 為後加）…⇒ v2 ETL 必須容忍
> **6 欄與 7 欄兩種**歷史形狀」~~ —— **兩件事都錯，而且是同一個病的兩面**：
> **(1) 出處是假的。** 那句「既有分頁是 6 欄建的」在 head 上**是一段被劃掉的舊句**
> —— `_get_worksheet` 的 docstring **已於 2026-09-01 就地更正**，更正後的現行表述是
> 「既有分頁**有幾欄不一定**（6 欄舊 schema／3 欄最小 schema 都真實存在）」。
> **原稿引用的是那個已被上游撤下的版本，卻掛上「程式碼自陳」的標籤。**
> **(2) 漏掉的那一種是使用者真的在用的那一種。** 3 欄最小 schema 是 **user 2026-08-19 明文要求**，
> 有專屬守衛 `tests/test_nav_history_gs_min_schema_v19489.py`（5 條，鎖 3 欄讀取／跨日期格式去重／
> append 自動補 metadata）。
> ⛔ **承重**：§2.5 的同一句是「v2 ETL 怎麼寫」那一欄 ——
> **照原稿實作的 ETL，會在使用者真實存在的 3 欄表上取錯欄位**（它會假設 `r[3]` 是 `fund_name`，
> 而 3 欄表根本沒有 `r[3]`）。**這不是措辭問題，是會產生錯資料的規格錯誤。**
> **本輪實測（AST 取 docstring 全文，非行窗；`git grep` 查守衛檔）**，指令見 §7.7。
> ⚠️ **這與 §4.3 的那個 Blocker 是同一種病**：**引用一個已被上游撤下的版本，然後掛上實測標籤。**
...
⛔ **這不是一份可用來統計缺漏率的語料**：10 個點橫跨 **14.4 年**（2011-11-18 ～ 2026-04-23，
**5270 天**；量測日 2026-09-14），是**稀疏降級快取**，不是日頻 NAV 序列。
**拿它算出來的任何「缺漏率」都會是一個沒有意義的數字。**

> ⚠️ **2026-09-14 第四輪就地更正（有意識的更正，不是漏刪；由獨立稽核抓出）**：
> 本句原寫 ~~「10 個點橫跨 **15 年**」~~ —— **實測 14.43 年**。
> **成因可辨識**：`2026 − 2011 = 15` 是**曆年年份相減**，不是**跨度**；真正的跨度是
> `(2026-04-23) − (2011-11-18) = 5270 天 = 14.43 年`。
> **結論完全不受影響**（10 個點無論橫跨 14 年還是 15 年都不是可統計的語料），
> **改的是那個可被一行算術推翻的數字** —— 本文件 §7.4 要求所有量測值可複跑，年份相減不是量測。
...
> ⭐ **2026-09-14 第四輪就地補（由獨立稽核抓出）——上表少了一句，而少的那句正好是 G-1 的關鍵**：
> 上表 `sharpe`／`std_1y` 是 **0/8 缺漏（8 檔全部有值）**，而同一份快照 `nav_points` 只有 30 筆、
> `MIN_OBS_SHARPE_SORTINO` 要 250。**原文沒有解釋這兩件事怎麼同時成立**，
> 讀者很容易推出「30 筆也能自算 Sharpe」—— **那與 G-1 的承重論證正好相反。**
>
> **分界線在「這個數字是自算的還是官方的」**：
>
> | 指標 | 缺漏 | 來源 | 為什麼是這個結果 |
> |---|---|---|---|
> | `ret_3y*`／`ret_5y*` | **8/8 缺** | 自算 | 30 筆不足 3～5 年 |
> | `max_drawdown` | **8/8 缺** | **永遠自算**（`services/fund_service.py` 就地註解：「max_dd **永遠自算**」） | 同上 |
> | `sharpe`／`std_1y` | **0/8 缺** | **MoneyDJ wb07 官方風險表** | 官方現成數字，**不吃 250 筆閘門** |
>
> ⇒ **「100% 缺漏」的三列恰好是自算的，「0% 缺漏」的兩列恰好是官方取回的。**
> 這不只不矛盾，它**正是 G-1 的證據**：**v1 在長序列不足時，是靠官方值頂住的，不是靠自算。**
> ⇒ **v2 若要「自算」風險指標（客戶指定 Sharpe 的算式，§3.1.1），就不能再靠這條官方捷徑** ——
> 缺口因此比只看缺漏率更大。
>
> ⚠️ **這一格的來源判定是「由程式碼路徑反推」，不是直接量到的，據實標明**：
> `snap.json` 的 `metrics` **只有 11 個 key，不含 `sharpe_source`**（實測 8/8 檔 key 集合完全相同），
> 也就是**快照本身沒有記錄來源**。本輪的推論依據是：(a) `services/fund_service.py` 就地註解
> 「夏普**優先用 wb07**（更精確）；自算值需 ≥ `MIN_OBS_SHARPE_SORTINO` 筆」；
> (b) 30 < 250 ⇒ 自算分支必然被閘門擋掉 ⇒ 有值就只能來自 wb07；
> (c) `std_source` 在同處被寫成 `"wb07" if (risk_override and risk_override.get("risk_table")) else "nav"`。
> ⛔ **本文件不宣稱已直接查證該 8 個數字的來源欄** —— runtime 會產生 `sharpe_source`，
> 但**這份快照沒有把它存下來**。**要直接查證，需要一次帶 provenance 的重新取數，本階段禁止打外部 API。**
> 📌 **這件事本身就是 v2 的一條要求**：**快照若不存 provenance，事後就只能靠反推**
> （`CLAUDE.md §2.2` 血緣追蹤）。
...
| `nav_history` | 獨立 Google Sheet | v2 價格表 | **ETL（自動）＋形狀偵測** | ⚠️ **需容忍 3 欄／6 欄／7 欄三種**歷史 header 形狀（§2.2.3，**2026-09-14 第四輪更正：原寫「6 欄／7 欄兩種」，漏掉 user 2026-08-19 明文要求支援的 3 欄最小 schema**）。⛔ **不得以位置硬取 `r[3]`..`r[6]`** —— 3 欄表沒有那些欄 |
...
| `realized_pnl_twd` | float | 已實現損益（TWD） | ⚠️ **v2 全新，v1 無此欄、無賣出路徑** —— §4.3 ＋ §5 **G-13** |
...
> **未改寫**。**這 11 條 v1 已完整實作**（本輪逐條實測：(1)(2)(3)(10) 是 `models/ledger.py::FundPosition`
> 的欄位，(4)(6)(8)(9)(11) 是同 class 的 `net_investment_twd`／`value_orig`／`value_twd`／`roi_price`／
> `roi_total_chubb`，(5)(7) 由呼叫端傳入）。v2 的工作是**把它從 service 層的記憶體物件變成持久化的表**。
>
> ⛔ **但上表不只有這 11 條，而多出來的那一條 v1 沒有** —— `realized_pnl_twd`
> **不在 CHUBB 11 欄之內**，`FundPosition` 也**沒有**這個欄位（實測該 dataclass **7 個欄位**：
> `fund_code`／`currency`／`units`／`cost_unit`／`fx_avg`／`cost_unit_with_div`／`dividends_received_twd`）。
> **⇒ 「v1 已完整實作」這句話的射程只到那 11 條，不涵蓋整張表。** 見 §4.3 與 §5 **G-13**。
...
> ⛔ **2026-09-14 第四輪就地更正（有意識的更正，不是漏刪；由獨立稽核抓出）—— 本節原本蓋掉了一個
> 客戶明文指定的規格缺口**：
> 本節原寫 ~~「✅ **v1 已完整實作此法**（**實測** `services/ledger_service.py`），v2 **沿用公式、不重新發明**」~~，
> 而其下的**賣出**區塊給出 `realized_pnl_twd += …` 的公式，§4.2 表又把 `realized_pnl_twd` 標成
> ~~「推導自 §4.3」~~。**整條鏈讀起來是「賣出與已實現損益 v1 已經有了，階段 2 只要把它持久化」。**
>
> **本輪實測（指令與輸出見 §7.7）**：
> - `git grep -n "realized_pnl" <rev> -- '*.py'` → **0 命中**（`exit 1`）。
>   **正對照**：同形狀指令查 `cost_unit_with_div` → **8 檔**命中 ⇒ 指令不是恆空。
>   ⚠️ **`-- '*.py'` 這個 scope 不可省** —— 不限副檔名時**唯一**命中的檔就是**本文件自己**
>   （**刻意不寫它有幾行**：本節每改一次那個數字就變一次，寫下去就是替下一輪製造一個過期數字；
>   同 §0.1 第 3 條與 §7.5.2 記載的自我污染家族）。
>   📌 **本輪就在這裡踩了一次**：本行初稿寫「全 repo **唯二**命中是本文件自己**那兩行**」，
>   而**寫完本節之後它就變成 11 行** —— **一句在寫下的當下為真、下一個段落就過期的自我計數。**
> - `Ledger` 的方法（AST 逐一列出，非行窗）＝ `subscribe`／`dividend_cash`／`dividend_reinvest`／
>   `to_dict`／`from_dict`，**沒有賣出**；`git grep -nE "^\s*def (sell|redeem)\b"` → **0 命中**。
> - `models/ledger.py::FundPosition` **7 個欄位**，**沒有已實現損益欄**。
> - **v1 自己的模組 docstring 也這樣說**：它列出「Phase 1：…subscribe / dividend_cash /
>   dividend_reinvest。Phase 2：Switch…＋ XIRR」——**賣出不在任何一個 Phase 裡**。
>
> ⛔ **為什麼這是封鎖級，不是措辭問題**：本節上一行引的客戶指定原文就是
> 「**已實現／未實現損益演算法全站統一**」，而 §5 的缺口清單 **G-1～G-12 沒有任何一項涵蓋它** ——
> **一個客戶明文指定的規格缺口，被一句「✅ 已完整實作」蓋掉了。**
> 下一份照抄本節的規格會把它排進「搬遷」而不是「新建」，工作量與風險都被低估。
> **本輪新增 §5 G-13 補上這個缺口。**
> ✅ **公式本身沒有被刪**（它是有效的 v2 設計），改的是它的**身分標籤**：
> ~~v1 已實作、照抄即可~~ → **v2 待實作的設計**。
> ⚠️ **這與 §2.2.3 的那個 Blocker 是同一種病**：**把「不存在的東西」寫成「已實測、已實作」。**

**v1 的實作涵蓋到哪裡（本輪逐一實測，分兩半讀）**：

| 交易動作 | v1 有沒有 | 實測依據 |
|---|---|---|
| **買進** | ✅ 有 | `Ledger.subscribe()` |
| **現金配息** | ✅ 有 | `Ledger.dividend_cash()` |
| **配息再投資** | ✅ 有 | `Ledger.dividend_reinvest()` |
| **轉換（同幣別／跨幣別）** | ✅ 有 | `Switch.switch_same_currency()`／`switch_cross_currency()`／`_do_switch()` |
| ⛔ **賣出／贖回** | ❌ **沒有** | `Ledger` **沒有任何賣出方法**；`FundPosition` **沒有已實現損益欄** |
| ⛔ **已實現損益** | ❌ **沒有** | 全 repo `realized_pnl` **0 命中** |

⇒ **買進／配息／轉換這四條，v2 沿用公式、不重新發明**（下方公式即 v1 逐字實作）；
⇒ ⛔ **賣出與已實現損益是 v2 必須新建的能力，不是沿用** —— 見 §5 **G-13**。
...
>
> ⛔ **上面這三行是本文件為 v2 提的設計，不是對 v1 的描述。⛔ 不得照抄為「v1 已有、直接搬」。**
> **v1 唯一會減少單位數的路徑是「轉換」**（`_do_switch`：A 端 `units` 減少、
> `cost_unit`／`fx_avg`／`cost_unit_with_div` 不變、TWD 歷史成本守恆搬到 B 端），
> **它不產生已實現損益** —— 成本基礎是被**移轉**走的，不是被**實現**掉的。
>
> ⚠️ **一個容易誤讀的旁證，先講清楚**：`sell` **確實**是 v1 的合法交易型別
> （`repositories/ledger_repository.py::KNOWN_ACTIONS` 五值之一），
> `ui/tab3_t7_ledger.py` 的 T7 轉換也**確實**會寫一列 `action="sell"` 進流水帳。
> **但那只是「記了一筆」，沒有任何成本引擎消費它**：
> 全 repo 唯一會把 `sell` 摺進部位的是一支**一次性遷移腳本**
> （`scripts/migrate_v149_schema.py::_fold_ledger_json`），
> 而它用的是 `Σ units×nav(buy) − Σ units×nav(sell)` —— **以成交淨值扣成本，不是以平均成本扣**，
> **既不是加權移動平均法，也不產生已實現損益**。
> ⇒ **「型別存在」≠「演算法存在」。** 這正是 §5 G-13 要記的那個落差。
...
>
> ⚠️ **一處精度更正（2026-09-14 第四輪，同一把尺順帶量到的）**：上式第一行寫 `new_units = …`，
> 讀起來像 v1 會**自己算出** `new_units`。**實測不是** —— v1 的
> `dividend_reinvest(new_units, nav_at_div, txn_date)` 把 `new_units` 當**呼叫端傳入的參數**，
> 只驗證 `> 0`，**不檢查它是否等於 `div_per_unit × units / nav_at_div`**。
> ⇒ **v2 若要保證該恆等式成立，必須自己算或自己驗，不能假設 v1 已經替你把關。**
> （對照：`dividend_cash` 的 `virtual_units` **是** v1 自己算的，兩者不同，不要混為一談。）
...
>
> ⛔ **2026-09-14 第四輪就地補（有意識的補充，不是漏刪）——本草稿描述的是一個「本站目前還做不到」的行為**：
> 下表「這筆已實現損益 (25−15) × 100 ＝ **1,000 元**」是在描述**本站賣出時會怎麼認列**，
> 而 §4.3 已實測：**v1 沒有賣出路徑、沒有已實現損益欄**（§5 **G-13**）。
> **⇒ 本草稿在 G-13 落地之前不得上線** —— 上線就會對使用者宣告一個不存在的能力，
> 那是 `CLAUDE.md §1`「Fail Loud, Never Fake」的鏡像違規（**假造能力宣告**）。
> ✅ **草稿內容本身不必重寫**：一旦 §4.3 的賣出設計實作完成，這段描述即為真。
> **改的是它的前置條件，不是它的文字。**
...
| **X-5** | **已實現損益的統一演算法** | 基金側採加權移動平均（客戶定死，§4.3）。⚠️ **2026-09-14 第四輪補：基金側目前也還沒有它** —— v1 無賣出路徑、無已實現損益欄（§5 **G-13**），所以這一列不是「基金側已定、只等股票側對齊」，而是**兩側都要新建** | 母法要求「已實現／未實現損益演算法**全站統一**」⇒ 股票／ETF 側**必須採同一法**。需確認股票側不是 FIFO。⚠️ **且協調時機提前了**：若基金側先自行實作再回頭對齊，等於製造一次無謂的返工 |
...
>
> ⚠️ **2026-09-14 第四輪新增 G-13，而它是怎麼被漏掉的，比它本身更值得記**：
> G-13 **不是本輪新發現的事實** —— 它一直都在，只是被 §4.3 一句
> ~~「✅ v1 已完整實作此法（實測 …）」~~ **蓋住了**。
> **一個寫著「已實作」的句子，會讓缺口清單的產出者不去把它當成缺口。**
> ⇒ **可操作規則**：**每一句「v1 已實作 X」都必須反過來問一次「那 X 的反面（X 做不到的情形）
> 有沒有進缺口清單」** —— 缺口清單的完整性，取決於它上游那些「已實作」宣稱的正確性。
> ⛔ **本表因此同樣不是窮舉**：本輪只把「v1 已實作」這一族宣稱逐句重驗了一遍（見 §7.7），
> **沒有**逐節反查每一條客戶指定項是否都有對應落點。**「只有 13 項」本文件不宣稱。**
...
| **G-13** | ⛔ **v1 無賣出路徑、無已實現損益** —— 而客戶明文指定「已實現／未實現損益演算法**全站統一**」 | `realized_pnl` 全 repo **0 命中**（正對照 `cost_unit_with_div` → 8 檔）；`Ledger` 方法集 **無賣出**（AST 列出）；`FundPosition` **7 欄無已實現損益欄**；v1 模組 docstring 的 Phase 1／2 清單**不含賣出**。⚠️ `sell` **是** `KNOWN_ACTIONS` 的合法值、T7 轉換也會寫該列，但**沒有成本引擎消費它**（唯一摺算它的是一次性遷移腳本 `scripts/migrate_v149_schema.py`，且用成交淨值扣、非平均成本扣） | **客戶指定項須全新建置**（⛔ **不是**沿用／遷移） |
...
| **基金 NAV 的歷史缺漏率** | 本地語料 `cache/nav/` **只有 1 檔、10 個點橫跨 14.4 年**（2011-11-18 ～ 2026-04-23，5270 天；**2026-09-14 第四輪更正，原寫「15 年」＝曆年相減而非跨度**），不是可統計的語料（§2.3.3） |
...
- ⚠️ **本文件含有全稱句，規則是「附 scope ＋ 附指令」，不是「不寫」** —— 見下方 §7.4.1。
- ⛔ 在獨立稽核完成前，本文件任一句**不得**被當成「已查證的事實」去支撐實作決策。

#### 7.4.1 ⭐ 本文件的全稱句：據實承認、附偵測指令（2026-09-14 第四輪新增）

> ⚠️ **2026-09-14 第四輪就地更正（有意識的更正，不是漏刪；由獨立稽核抓出）**：
> §1.3 與本節原本各有一句 ~~「本文件**刻意不寫**『只有 N 處』『全部都是 X』這類可被一條 grep
> 推翻的全稱句」~~ —— **兩句都是假的**，而且**假的方向最糟：它讓讀者以為不必檢查這類句子。**
> **舊表述的用意仍然成立**（那類句子確實脆弱、確實該少寫）；
> **被權衡掉的是它的事實面** —— 本文件**做不到**「不寫」，實際上也**沒有**做到。

**現行規則（三條，取代舊的「不寫」）**：

1. **可以寫全稱句，但必須就地附上 scope**（掃了哪些路徑、哪個 rev、排不排除本文件自身）。
   ⛔ **沒寫 scope 的全稱句視同未量** —— §3.1.3 就是這樣出事的（結論比它的 grep scope 大）。
2. **必須附可複跑指令**，且該指令要**配一顆正對照**（證明它不是恆回 0）
   **與一顆空檔防護**（證明讀到的不是空輸入）。理由見 §0.1 的三顆對照。
3. **凡是取決於「有沒有漏看」的全稱句，一律標為單組實測、未經第二組驗證**（`CLAUDE.md §-2` 規則 5／6）。

**本文件全稱句的偵測指令（repo 根目錄；⚠️ 它給的是候選，不是判定，命中須人工逐一判讀）**：

```
python3 - <<'EOF'
import re
s = open("docs/v2/DATA_DICTIONARY.md", encoding="utf-8").read()
assert len(s) > 30000, "空檔防護：讀到的不是本文件"          # ⭐ 缺這行，空檔會回 0 候選
skip = set()                    # 已被劃掉的句子不算數；⚠️ 必須「不跨行」，理由見下方
for m in re.finditer(r"~~[^\n]+?~~", s):        # ⛔ 不要寫 r"~~.+?~~" + re.S
    skip.update(range(s[:m.start()].count(chr(10))+1, s[:m.end()].count(chr(10))+2))
PAT = r"只有\s*[0-9一二三四五六七八九十兩]|僅有\s*[0-9]|全部(都)?是|唯一|全都|沒有任何一[處個檔條]|8/8"
for i, ln in enumerate(s.splitlines(), 1):
    if i not in skip and re.search(PAT, re.sub(r"[*~`]", "", ln)):
        print(i, ln.strip()[:110])
EOF
```

⚠️ **這條指令刻意寬**（會抓到「保單**唯一**識別碼」這種偽陽性），理由同 §-1.5.1c 判定 2 的教訓：
**為了乾淨而收窄字表，正是漏抓的成因。** **寧可多抓、人工判讀。**
⚠️ **`**` 與 `~~` 必須先剝掉** —— 本輪實測，不剝的話 `**只有 2 處**`、`**只有兩個成員**`
這兩句**結構上掃不到**（第三輪稽核給的清單就少了它們）。

⛔ **⭐ 本輪在寫下這條指令之後、據以落筆之前，實跑時抓到它自己有一個 bug —— 照實記，因為它正是本輪的主題**：

第一版把「跳過已劃掉的句子」寫成 `re.finditer(r"~~.+?~~", s, re.S)`。
**`re.S` 讓 `.` 吃換行，於是非貪婪配對會把兩個毫不相干的 `~~` 標記配成一對**，
中間整段全部進 `skip`。**實測（同一份定稿，只改這一個 regex）**：

| 版本 | 被跳過的行 | 回出的候選 |
|---|---|---|
| ⛔ **第一版**：`r"~~.+?~~"` ＋ `re.S`（跨行） | **438 / 1910** | **41** |
| **`r"~~[^\n]+?~~"`（不跨行、不加 `re.S`）** | **30 / 1910** | **62** |

⇒ **那個 bug 讓本文件 408 行（`438 − 30`）、21 個候選（`62 − 41`）對這條指令完全隱形** ——
而其中就包含本節自己剛寫下的那幾句。
⚠️ **它長得像通過**：它回了 41 個候選、看起來很忙，**沒有任何跡象顯示它瞎了兩成的文件**。
⚠️ **上表四個數字會隨本檔每一次改寫而變動**（本輪第一次量到的是 `279`／`41` vs `21`／`55`，
定稿時已變成上表的值）—— **它們是「同一份定稿、只改一個 regex」的對照，不是可引用的常數。**
**要用請現場重跑**（§7.4 第 2 條）。**不變的是方向：跨行版恆少看見一大片，且不會有任何徵兆。**
**⇒ 這正是本文件 §1.1／§0.1 反覆記載的那個形狀：工具沒在量它宣稱在量的東西。**
**⇒ 也正是本輪兩個 Blocker 的同一個病，只是這次犯在「用來抓這種病的工具」上。**
📌 **可操作規則**：**markdown 的刪除線不跨行，所以任何吃 `~~` 的正則都必須以 `[^\n]` 封住**；
`.` ＋ `re.S` 在**成對標記**上永遠有這個危險。

**修正後本輪實跑（兩次，含負對照）**：
- **正常情形**：於 repo 根目錄跑上方（已修正的）指令 → **回 62 個候選**
  （量測日 2026-09-14，量測對象為本檔定稿），**exit 0**。
  ⚠️ **62 是候選數，不是全稱句數** —— 其中含偽陽性（如「保單**唯一**識別碼」），
  也含**本指令自己那一行 `PAT`**（它列出了自己要找的字串，因此必然命中自己 ——
  與 §7.5.2、§0.1 記載的是**同一個自我污染家族**，此處**不設法消除，改為就地標明**，
  因為消除它需要再加一條排除規則，而那條規則本身又會被下一個人誤讀成「掃不到的地方」）。
- ⭐ **負對照（空檔防護真的會 fire）**：把本檔換成一個 0 位元組的同名檔再跑同一條指令 →
  **`AssertionError: 空檔防護`、exit 1**，**一個候選都不印**。
  **沒有這一條，空檔會安靜地回「0 個候選」，而 0 正好就是舊表述宣告的值**（同 §0.1 第 3 條對照）。

**本輪跑過的那些，逐句附指令（⛔ 分類敘述，非窮舉 —— 上面那條指令回的候選比本表多，
本表只列「可被一條 git 指令當場推翻」的那一類）**：

| 出處 | 句子 | 驗證指令（rev ＝ `9cbf0377`） | 本輪重跑 |
|---|---|---|---|
| §2.3.3／§7.3 | `cache/nav/` **只有 1 個檔** | `git ls-tree -r --name-only 9cbf0377 -- cache/nav/` | ✅ 1 行 |
| §2.4.2 | `fetch_url_with_retry` 的**唯一**命中是 `fund_fetcher.py` | `git grep -n "^def fetch_url_with_retry" 9cbf0377`（正對照：去掉 `_with_retry` 回 4 行） | ✅ 1 行 |
| §3.1.3 | `repositories/financial_repository.py` 是本 repo **唯一**抓個股財報的模組 | 該檔公開 API 以 AST 列出 ＝ `resolve_ticker`／`fetch_stock_three_ratios` | ✅ |
| §3.1.3 | 字面 `forward_pe` 在 `*.py` **只有 2 處** | `git grep -n "forward_pe" 9cbf0377 -- '*.py'` | ✅ 2 行 |
| §7.5.1 | `CONSTITUTION_FILES` **只有兩個成員** | 以 AST 讀 `tests/test_constitution_file_refs.py` 的該 `AnnAssign`（⚠️ **它是 `AnnAssign` 且元素是 `Path / "x"` 的 `BinOp`**，`literal_eval` 會丟例外；本輪第一次探測就因此**回空**，差點被讀成「查無此物」） | ✅ 2 個 |
| §4.3／§5 G-13 | 全 repo `realized_pnl` **0 命中** | `git grep -n "realized_pnl" 9cbf0377 -- '*.py'`（正對照：`cost_unit_with_div` → 8 檔） | ✅ 0 命中 |

⚠️ **本表是單組實測、未經第二組獨立複驗**（`CLAUDE.md §-2` 規則 6）。
⛔ **「本文件的全稱句只有這些」本文件不宣稱** —— 那正是一句取決於「有沒有漏看」的全稱句。
**要重新產生完整候選清單，跑上面那條指令，不要引用本表。**
...
> **總計 `81` 個 unique path-like token —— `67` 個直接存在，`14` 個不存在。**
...
> 故當時的 `77`／`63`／`14` 與 MISSING 十四項清單**逐項相同**；腳本的三顆正／負對照亦照舊 PASS。
>
> 📌 **2026-09-14 第四輪定稿後重跑：`77`／`63`／`14` → `81`／`67`／`14`（有意識的更新，不是漏刪）。**
> 第四輪**新增 4 個反引號路徑，4 個全部存在**，故 **`MISSING` 停在 14、十四項清單一字未動**
> （本輪定稿後以 `set` 逐項比對確認：**新增 4、移除 0、MISSING 集合完全相同**）。
> 新增的 4 個：`scripts/migrate_v149_schema.py`（§4.3 舉證 v1 唯一摺算 `sell` 的地方）、
> `tests/test_nav_history_gs_min_schema_v19489.py`（§2.2.3 的 3 欄 schema 守衛）、
> `ui/tab3_t7_ledger.py`（§4.3 寫 `action="sell"` 那一列的地方）、
> `ui/tab2_single_fund.py`（§7.7 對帳 chip 的 caller）。
> ⚠️ **一處刻意的寫法調整，順帶記下**：本輪原本在 §2.3.3／§7.7 寫了裸檔名 `` `fund_service.py` ``，
> 它會**在 MISSING 裡多出第 15 項**（裸檔名不在 `git ls-tree` 的輸出裡）。
> **已改寫為完整路徑 `services/fund_service.py`** —— **既不動十四項清單，也讓引用更精確。**
> 📌 **這一步就是 7.5.3 教訓 (3) 的實際執行**：**不是「改完再看數字對不對」，是「數字不對就回頭改文字」。**
...
| **「§0.1 的禁用語表是本文件唯一算錯的自我計數」** | ⛔ **本輪不作此宣稱。** 本輪只重量了**被點名的那幾個**，加上 §7.5.2 的 `77`／`63`／`14`；**沒有逐節掃過每一個自我計數** —— 那取決於有沒有漏看（⚠️ **第四輪註**：§7.5.2 的三個數字已因第四輪新增路徑而更新為 `81`／`67`／`14`，本列引的 `77`／`63`／`14` 是**第三輪當時**的值，**刻意不改** —— 它描述的是那一輪） |
...
> ⛔ **2026-09-14 第四輪就地補 —— 本節第一列的「未驗」聲明不是一把保護傘，射程要講清楚**：
> 本節第一列寫「§2～§6 的實質數字本輪未重跑、未複驗，因此**也不為它們背書**」。
> **第四輪實測後發現，那句話被兩個 Blocker 當成了擋箭牌，而它擋不住。**
>
> **關鍵區別（本輪就地釘死，往後一體適用）**：
>
> | 句型 | 例 | 未驗聲明蓋不蓋得住 |
> |---|---|---|
> | **標了未驗的待驗事項** | 「本推論未查證」「本組不宣稱」 | ✅ **蓋得住** —— 讀者已被告知要自己驗 |
> | ⛔ **肯定式的實測宣稱** | 「✅ v1 **已完整實作**此法（**實測** …）」（§4.3）／「**程式碼自陳**既有分頁只有 6 欄」（§2.2.3） | ❌ **蓋不住** |
>
> **理由**：一句掛著「實測」「程式碼自陳」標籤的肯定句，**它的全部作用就是告訴讀者「這個不用你再驗了」**。
> **一節在別處聲明「我沒重跑」，並不會把這句話變回待驗事項** ——
> 讀者不會、也不該為了讀一句「實測 X」而先回頭查交件自陳有沒有把 X 排除掉。
> ⇒ **凡是要掛「實測／已實作／程式碼自陳／0 命中」這類標籤的句子，就必須在那一輪真的跑過**；
> **跑不了就把標籤拿掉，改寫成不帶標籤的敘述**（`CLAUDE.md §-2.A` 第 1 款的同一條規矩）。
> ⛔ **這一條對本文件既有的每一句都回溯適用** ——
> **本輪只把「v1 已實作 X」與「程式碼自陳 X」兩族逐句重驗了**（清單見 §7.7），
> **其餘帶標籤的句子仍未逐句重驗**，那是第四輪的已知盲點，見 §7.7.1。

### 7.7 第四輪回修的射程與其未驗事項（2026-09-14）

**兩個 Blocker ＋ 五個 Advisory。⭐ 兩個 Blocker 是同一種病：把「不存在的東西」寫成「已實測、已實作」。**

| # | 改了什麼 | 實質結論有沒有動 |
|---|---|---|
| **B-1** | §4.3 ~~「✅ v1 已完整實作此法（實測）」~~ → 實測 v1 **無賣出路徑、無已實現損益**；賣出公式改標為 **v2 待實作的設計**；§4.2 的 `realized_pnl_twd` 改標 v2 全新；§4.6 文案草稿標明它描述的是**尚未存在的能力**；**§5 新增 G-13** | ⚠️ **有，而且是往「多一個缺口」的方向**：一個客戶明文指定的規格缺口原本被蓋住。**加權移動平均法本身、CHUBB 11 式、買進／配息／轉換公式一字未改** |
| **B-2** | §2.2.3／§2.5 ~~「程式碼自陳既有分頁只有 6 欄 ⇒ 容忍 6／7 兩種」~~ → **3 欄／6 欄／7 欄三種**；出處改標為該 docstring **2026-09-01 就地更正後**的現行表述 | ⚠️ **有**：漏掉的 3 欄最小 schema 是 **user 2026-08-19 明文要求**、有專屬守衛。照原稿寫的 ETL 會在真實的 3 欄表上**取錯欄位** |
| A-1 | §1.3／§7.4 ~~「本文件刻意不寫全稱句」~~ → 據實承認 ＋ **新增 §7.4.1**（規則三條 ＋ 偵測指令 ＋ 逐句指令表） | ⛔ 沒有。**被點名的那幾句本身一個字都沒改**（重跑後全部為真） |
| A-2 | 本 PR 描述落後 head 一顆 commit | ⛔ 沒有（PR body，非本檔） |
| A-3 | §2.1 ~~「wb01/wb05/wb07 為月更」~~ → **wb05 是「月／季更」**；並補上原本漏列的 `fetch_div` | ⛔ 沒有。⚠️ 但方向要記：此錯讓新鮮度預算**偏緊**，會把正常的季更誤判成過期 |
| A-4 | §2.3.3／§7.3 ~~「10 個點橫跨 15 年」~~ → **14.4 年**（5270 天）。成因：**曆年年份相減，不是跨度** | ⛔ 沒有 |
| A-5 | §2.3.3 補上 `sharpe`／`std_1y` **0/8 缺漏**與「30 筆算不出 Sharpe」如何並存 —— **自算的三列全缺、官方取回的兩列全有** | ⛔ 沒有，**反而補強了 G-1 的論證**（v1 是靠官方值頂住，不是靠自算） |

**⭐「同一把尺對全部同類重跑」的結果（本輪的硬要求，逐句列出，不只修被點名的那一句）**

**族 A：「v1 已實作 X」—— **在本輪動手前的定稿 `660128a` 上**全文 10 句，逐句重驗**

> ⚠️ **這裡的 scope 必須釘 rev，理由是本輪已經為同一個病栽了三次**：
> 同一條字表在 `660128a` 上回 **10**，在**本輪定稿**上回 **39** ——
> 因為本輪的更正註本身就在大量討論「v1 已實作什麼」。
> **「全文 10 句」若不釘 rev，它在被寫下的那一刻就已經不成立。**
> **複跑**：`git show 660128a:docs/v2/DATA_DICTIONARY.md | grep -cE "v1 \*\*?已|v1 已|已完整實作|已實作|已有實作|v1 逐字實作|v1 已有"` → **10**
> （⚠️ `git show <未 fetch 的 rev>` 失敗時管線會回 0，**先確認該 rev 在本地**）。

| 出處 | 宣稱 | 本輪判定 |
|---|---|---|
| §2.6.1 | 幣別正規化 v1 已有 SSOT | ✅ `services/currency.py` 的 `CCY_NORMALIZE` ＋ `normalize_ccy` 都在 |
| §2.6.3 | 跨幣別 B 端成本反推 v1 已實作 | ✅ `_do_switch` 逐行對上（`twd_cost_basis = n_redeem × cost_unit_a × fx_avg_a`，再除 `n_added × fx_avg_b`） |
| §3.1.1 | 還原權值已有實作 | ✅ `services/fund_service.py` 該段為**活文字**（非劃掉） |
| §3.1.4 | 對帳四態 chip v1 已有 | ✅ `services/fund_service.py` 就地註解寫「UI 的四態 chip」，`ui/tab2_single_fund.py` 真的讀 `div_yield_reconcile` |
| §3.2 | 來源標籤做成會印給使用者看的字串 | ✅ `services/fund_total_return.py` 逐字：官方值以「MoneyDJ 官方」開頭、自算值以「自算」開頭 |
| §3.3 | v1 已有 `⬜ 資料不足` 慣例 | ⚠️ **本輪未重量**（第三輪量過 113／83，本輪沿用未複驗） |
| §4.2 | CHUBB **11 條** v1 已完整實作 | ✅ **11 條逐條對上**；⛔ **但射程被本輪縮小** —— 原句緊貼的那張表另有 `realized_pnl_twd`，**不在 11 條內、v1 也沒有** |
| §4.3 | v1 已完整實作**此法** | ❌ **B-1，本輪推翻** |
| §4.4 | 除息 v1 已有實作（CHUBB 規範） | ✅ `dividend_cash`／`dividend_reinvest` 逐行對上 |
| §4.4 | 含息成本下調公式 v1 **逐字**實作 | ✅ 該 docstring 逐字相同（`virtual_units`／`new_cost_with_div`） |

⭐ **同一把尺順帶量到、不在稽核清單上的一項**：§4.4(b) 把 `new_units = …` 寫得像 v1 會自己算，
**實測 v1 是把它當呼叫端傳入的參數、只驗 `> 0`**（已就地補註）。
**⇒ 這一項就是「同一把尺」的產出** —— 只修被點名的那一句的話，它不會被發現。

**族 B：「程式碼自陳 X」—— 全文 20+ 句，本輪改用機械法掃「引用到已被上游撤下的版本」**

- **方法**：對本文件引用到的來源檔逐一抓出所有 `~~…~~`（**上游自己劃掉的舊句**），
  再檢查本文件引用的字串**是否落在其中**。**這比逐句重讀語意可靠，因為它針對的正是 B-2 的病灶。**
- **結果**：掃 11 個被引用的來源檔 → **27 個劃掉的舊句**；本文件的引用落在其中的**只有 B-2 那一處**
  （`services/nav_history_gs.py` 的「既有分頁是 6 欄建的」）。
  抽驗 `infra/cache.py`／`services/fund_invest_calc.py`／`services/fund_service.py` 的被引字串 → **全部是活文字**。
- ⛔ **這個方法的射程有限，據實寫明**：它只抓「**引用到劃掉的舊句**」這一種。
  **它抓不到**「原文沒有劃掉、但本文件轉述時說歪了」那一種 ——
  **後者本輪沒有逐句重讀**，見 §7.7.1。

**⛔ 本輪刻意沒有碰的（與前三輪相同的檔案邊界）**：
`.py` 一個字都沒動；`CLAUDE.md`／`EXCEPTIONS.md`／v2 母法／UI 規格與線框稿**一個 byte 都沒動**。
**本 PR 仍然只新增一個檔。**
**parquet 數字、缺漏率、FRED 逐格、程式常數、CHUBB 11 式那一層，本輪未重算，也未改寫。**

**本輪實跑過的主要指令（rev ＝ `9cbf0377`，可逐條複跑）**

```
git grep -n "realized_pnl" 9cbf0377 -- '*.py'                  → 0 命中（exit 1）
git grep -l "cost_unit_with_div" 9cbf0377 -- '*.py'            → 8 檔          ← 正對照
git grep -nE "^\s*def (sell|redeem)\b" 9cbf0377 -- '*.py'      → 0 命中（exit 1）
git ls-tree -r --name-only 9cbf0377 -- cache/nav/              → 1 行
git grep -n "forward_pe" 9cbf0377 -- '*.py'                    → 2 行
git grep -n "^def fetch_url_with_retry" 9cbf0377               → 1 行（fund_fetcher.py）
```

⚠️ **AST 而非行窗**：`Ledger`／`FundPosition` 的方法與欄位、`_get_worksheet` 的 docstring 全文、
`_NAV_HEADERS`、`CONSTITUTION_FILES`、以及把每個 `@_daily_cache` 註解對回它所裝飾的函式，
**一律以 `ast` 解出**，**沒有用固定行數的上下文窗**
（`CLAUDE.md §-2.A` 第 4 款：固定窗會把相隔數百行的兩段接在一起印）。

⛔ **本輪自己踩到、並在落筆前修掉的三個坑，照實記下**：
1. **空檔防護救了一次**：第一支編輯腳本寫 `assert len(s) > 100000`，**當場 fire** ——
   因為 `107779` 是**位元組**不是**字元**（CJK 一字 3 bytes，本檔**當時** 62469 字元）。
   **若把門檻寫成「大於 0」，這個混淆就會安靜地活下去。**
2. **一次「空輸出 ≠ 沒有」**：用 `ast` 找 `CONSTITUTION_FILES` 時**回空**，
   差一步就要寫成「查無此符號」。實際是它為 `AnnAssign`、元素是 `Path / "x"` 的 `BinOp`，
   `literal_eval` 丟例外。**先跑一條裸 `git grep` 當正對照，才分辨出「工具不會讀」與「東西不存在」。**
3. ⭐ **§7.4.1 那條偵測指令自己有 bug，回了 41 個候選卻瞎了本檔兩成以上的行**（`438 / 1910`）—— 詳見該節。
   **本輪的主題是「工具沒量到它宣稱在量的東西」，而本輪的工具自己就犯了一次。**

### 7.7.1 ⚠️ 第四輪沒有驗到什麼（照實寫，不寫「全都查過了」）

| 沒驗到 | 為什麼 |
|---|---|
| **§2～§6 的實質數字**（parquet 統計、缺漏率、FRED 逐格、程式常數） | **不在本輪射程**，本輪**未重跑、未複驗，也不為它們背書**。⚠️ **但請先讀 §7.6.1 的那條區別** —— 這句話**蓋不住**任何掛著「實測／已實作／程式碼自陳」標籤的肯定句 |
| **「程式碼自陳 X」的語意是否被轉述歪了** | 本輪只機械掃了「引用到上游劃掉的舊句」這一種（見上）。**原文沒劃掉、但轉述說歪了的那一種，本輪沒有逐句重讀** |
| **`snap.json` 那 8 個 `sharpe` 的實際來源欄** | 該快照**沒有存 `sharpe_source`**；本輪是**由程式碼路徑反推**（已就地標明）。直接查證需要一次帶 provenance 的重新取數，**本階段禁止打外部 API** |
| **「本文件的全稱句只有 §7.4.1 表列那些」** | ⛔ **本輪不作此宣稱。** §7.4.1 的指令回**候選**，判定靠人工；**候選比表列多**，本輪只逐句重跑了「可被一條 git 指令當場推翻」的那一類 |
| **「§5 的缺口只有 13 項」** | ⛔ **本輪不作此宣稱。** 本輪只反查了「v1 已實作」這一族宣稱的反面；**沒有**逐條反查每一個客戶指定項是否都有落點 |
| **`⬜ 資料不足` 的 113／83** | 第三輪量的，**本輪未重量** |
| **本輪新寫的每一句是否也犯了同樣的病** | ⚠️ **本輪是實作者自己檢查自己** —— 上面那三個坑是自查抓到的，但**「只有三個」本輪不宣稱**（`CLAUDE.md §-2` 規則 4：不可自己查自己） |
