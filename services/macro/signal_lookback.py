...
def evaluate_signal_at_event(
    ...
    2. 在峰前 max_lookback_days 區間找警戒轉折日 → lead_time_days

    v18.281 修正：mode 參數（鏡像 stock v18.160）
    ...
    # 2) 峰前 max_lookback_days 內找警戒轉折日（v18.281 edge mode 預設）
...
def lookback_all_signals(
    ...
    Args:
        ...
        max_lookback_days: 警戒搜尋上限
        mode: "edge"（v2 預設，找跨越警戒的轉折日）/ "state"（v1 legacy）
    ...
    for spec in specs:
        ...
        out[spec.key] = [
            evaluate_signal_at_event(ev, series, spec, lookback_days, max_lookback_days, mode)
            for ev in events
        ]
    return out
