...
背景
====
user 要在基金健診顯示每檔是「核心資產」還是「衛星資產」。原擬單用  3-3-3
...
2.  3-3-3 明確通過(穩健長線達標)→ 核心(source="3-3-3")
...
def classify_core_satellite(
    ...
    Args:
        ...
        passed_333:  3-3-3 結果(True 通過 / False 未通過 / None 資料不足)
    Returns:
        {
          "label":  "核心" / "衛星" / "待定",
          "emoji":  對應色點,
    ...
    # 2.  3-3-3 明確通過 → 核心(穩健長線達標)
    if passed_333 is True:
        _extra = f";類別「{cat[:14]}」" if cat else ""
        return _pack("核心", "3-3-3", f"通過  3-3-3(成立≥3年 + 3年年化>7%){_extra}")
    # 3. 廣泛分散型 → 核心
    if cat_role == "核心":
...
def summarize_core_satellite_allocation(items) -> dict:
    ...
    吃 `portfolio_core_pct`)。本函式分類依據完全不同(MoneyDJ 基金類別關鍵字 +
     3-3-3),回答的是「我手上這幾檔本質上偏穩健還是偏主題」。兩者並列時若都給
