...
def estimate_monthly_income(
    ...
    **不知道 ≠ 不一致** —— 反過來擋掉，等於把大多數基金的配息試算一起關掉。
    ...
    if _monthly_twd is not None:
        ...
        #    就是 `/100.0` 之後才傳，容差是照小數訂的。
        #    ⚠️ ~~傳百分點會讓容差整整鬆 100 倍。~~
        #    → **2026-09-08 更正（有意識的更正，不是漏刪）：機制講反了，本組自己重推。**
        #    `math.isclose` 的判定式是 `|a-b| <= max(rel_tol*max(|a|,|b|), abs_tol)`：
        #      · **`rel_tol` 是尺度無關的** —— 兩種用法的相對項**完全相同**（都是 0.05×max）；
        #      · 有尺度的只有 `abs_tol` —— 餵小數 ⇒ 地板 ＝ **0.1 個百分點**（0.001×100）；
        #        餵百分點 ⇒ 地板 ＝ **0.001 個百分點**，也就是**嚴 100 倍，不是鬆**。
        #      · 相對項勝出的門檻：餵小數 `max>2%`、餵百分點 `max>0.02%` ⇒
        #        **配息率 >2% 時兩種用法判定完全相同**；≤2% 時餵百分點**比較嚴**。
        #    **舊表述的結論仍然成立**（確實該傳小數，本行一個字都沒改）；
        #    **被權衡掉的是它的理由** —— 真正的理由是「`abs_tol=0.001` 這個常數
        #    是照『0.1 個百分點』訂的（見 `reconcile_dividend_yield` docstring），
        #    餵百分點會**悄悄改掉那個常數的語意**」，不是「容差變鬆」。
        #    ⚠️ 守衛因此必須用**低配息率**的案例，否則殺不掉這顆突變 ——
        #    見 `tests/test_wf03_research_invest_calc.py::`
