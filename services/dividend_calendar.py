...
用途:吃每檔基金的**配息歷史**,推估「**目標月**的**除息基準日** + 配息入帳日」,產生月曆結構供 L3 渲染。
(目標月由 caller 給:App 傳本月;LINE 月底推播傳下個月 —— 本層不預設「本月」。)
...
v19.532 對抗式稽核四修(每條都附實跑數字,見對應常數 / 函式註解):
  ...
  4. `_stale_state` / `predict_ex_for_month` / `build_month_calendar` 收 `ref_day`,cron(當時每月
     1 號;v19.537 起 28 號)不再被當成月中 15 號 → 誤算半個月陳舊度而誤殺月配基金。
...
**引擎推估邏輯一行沒動**(三口徑逐位元相同),改的全是「顯示什麼 / 顯示成什麼樣」:
  ...
  4. 裁示 4:`ALL_UNPRED_LINE_HEAD` 首行的「本月」→ **實際目標月**(cron 每月 28 號推下個月,
...
v19.536(總管 2026-08-26 裁示)—— **引擎推估邏輯一行沒動**:
  ...
  chip 標籤 / LINE caption 首行四處,**獨漏這個標題** —— 推播每月 28 號推下個月時,H1 寫
...
def _stale_state(last_ex: "_dt.date", ref_year: int, ref_month: int, step: int,
    ...
    `.github/workflows/dividend_calendar_notify.yml` 當時是 `cron: "0 0 1 * *"`(每月 1 號),
    `now.day` 恆為 1,「月中是無偏中點」只在執行日均勻分布時成立,執行日恆為某一天時
    ...
    **整檔基金從月曆上消失**。傳真實日期才是對的(把常數改成 1 只是偏到另一邊,App 路徑會受害)。
    ⚠️ **v19.537 起 cron 改 `23 0 28 * *`(每月 28 號)→ 偏誤方向翻面**:不傳 ref_day 會變成
    **少算 13 天**(該判過舊的沒判)。結論不變:知道今天幾號的 caller 一律傳真實日。
...
def predict_ex_for_month(schedule: dict, year: int, month: int,
    ...
    ⚠️ `ref_day`(**v19.532 bug 4**):ref 月的「幾號」。未給 → `_REF_DAY_OF_MONTH`(15)。
       production 的 cron 是每月 **28 號**執行(`dividend_calendar_notify.yml`;v19.537 前為 1 號),
       恆定用 15 號等於每次都把陳舊度算偏半個月 —— 1 號時**多算 14 天**(實測把「真實靜默
       113 天(< 122 天門檻)」算成 127 天,整檔基金無聲從月曆消失),28 號時反過來**少算 13 天**
       (該判過舊的沒判)。**知道今天幾號的 caller 請一律傳下來**(見 `_stale_state`)。
...
def build_month_calendar(funds: list, year: int, month: int,
    ...
    否則排程日與 15 號固定差半個月:cron 每月 **28 號**(v19.537)→ 少算 13 天陳舊度;
    v19.537 前的每月 1 號 → 多算 14 天,月配基金會在門檻附近整檔消失。
...
# 逐字文案,但 cron(每月 28 號)推的是**下個月** —— 標題寫「本月」、同一張圖的徽章 / 副標卻
...
# v19.534 裁示 4:首行原寫「本月」,但 cron(每月 28 號)推的是**下個月** —— 規格 §15.4 逐字
...
# cron 每月 28 號推的是**下個月**,「本月」在那個情境是錯的,而徽章上寫的又是真正的目標月。
...
def month_label(year, month) -> str:
    ...
    v19.534 追加 7:畫面上原本多處寫死「本月」,但推播每月 28 號推的是**下個月** ——
...
def build_summary_text(cal: dict) -> str:
    """月曆結構 → LINE 月底推播提醒文字(推下個月)。無事件 → 誠實說本月無推估除息基準日(§1)。
