...
def _holding_line(a: dict) -> str:
    """單檔一段精簡文字(名稱 + 建議 + 表現差原因 + 換入標的)。"""
    _name = a.get("name") or a.get("code") or "?"
    _bits = [f"• {_name}", f"  {a.get('action_zh', '')}".rstrip()]
...
def build_notification(result: dict, *, portfolio_name: "str | None" = None,
    ...
    _tail = (
        f"持倉 {_summary.get('n_holdings', len(_advices))} 檔｜"
    ...
    _lines.append("\n\n".join(_holding_line(a) for a in _shown))
