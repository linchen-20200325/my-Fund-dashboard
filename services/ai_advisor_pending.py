"""services/ai_advisor_pending.py — 事前 AI 建議:top-N 因子權重候選比對

v19.250 B(本次):pending review ceremony 退役後,本檔只剩**事前 AI 建議**
線(recommend_weights),供 ui/tab_crisis_backtest.py 的 AutoSearch top-5
winners 比對 + 多因子 plateau 候選比對使用(獨立功能,非 pending review 機制)。
explain_pending_weights / _format_weights_table / _math_summary / _build_prompt
4 個 explain 線符號連同 pending API 整批退役。

設計:
- 純函式 + 純文字 prompt,不依賴 streamlit context
- AI 失敗或無 key → 回退 fallback 規則摘要(取最高 plateau 並標稀疏度 / OOS 旗標)
"""
...
# v19.11:user 對「預警」的口徑偏好 — 注入 AI 線 prompt
