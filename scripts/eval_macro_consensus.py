...
⚠️ 已知限制
============
1. **2008 GFC / 2000 dot-com 視 parquet 涵蓋而定**：bootstrap 35 年 → 1991+ 全覆蓋；
   舊版 15 年只到 2011-06。跑 `scripts/update_macro_history.py --bootstrap --years 35`
   後 eval 自動覆蓋 4 大熊市。
2. **factor 數量取決於 parquet 內容**：bootstrap v19.29+ → 12 factor (含 DXY/PPI/COPPER)，
   舊 v19.27- → 9 factor。仍缺 PMI / BREADTH（FRED 無單一來源）。
...
def build_factor_panel(
    ...
    v19.29 從 9 → 12 factor：新增 DXY (DTWEXBGS) / PPI (PPIACO) / COPPER (PCOPPUSDM)
    （需 parquet 已 bootstrap 含這 3 series，否則該欄不出現 graceful fallback 回 9-factor）。
    """
    ...
    # v19.29 新增 3 factor — bootstrap 後可用，未 bootstrap 自動 skip
    if "DTWEXBGS" in fred_monthly:
        # DXY proxy 月變化 % (對齊 _s_dxy(chg_m))
        out["DXY"] = (fred_monthly["DTWEXBGS"] / fred_monthly["DTWEXBGS"].shift(1) - 1) * 100
    if "PPIACO" in fred_monthly:
        # PPI YoY % (對齊 _s_ppi(v))
        out["PPI"] = (fred_monthly["PPIACO"] / fred_monthly["PPIACO"].shift(12) - 1) * 100
    if "PCOPPUSDM" in fred_monthly:
        # Copper 月變化 % (對齊 _s_copper(chg))
        out["COPPER"] = (fred_monthly["PCOPPUSDM"] / fred_monthly["PCOPPUSDM"].shift(1) - 1) * 100
