...
def test_phase_accuracy_returns_dataframe():
    df, spx = msc.generate_synthetic_demo(n_months=60)
    score = msc.compute_historical_score(df)
    acc = msc.phase_accuracy(score, spx, horizon_months=6)
    assert isinstance(acc, pd.DataFrame)
    assert set(acc["phase"]) == {"Peak", "Expansion", "Recovery", "Recession"}
    assert "hit_rate_pct" in acc.columns


def test_overall_accuracy_in_range():
    df, spx = msc.generate_synthetic_demo(n_months=60)
    score = msc.compute_historical_score(df)
    acc = msc.overall_accuracy(score, spx, horizon_months=6)
...
def test_grid_search_returns_sorted():
    df, spx = msc.generate_synthetic_demo(n_months=60)
    score = msc.compute_historical_score(df)
    grid = msc.grid_search_phase_thresholds(
        score, spx, horizon_months=6,
        peak_grid=(7.0, 8.0), exp_grid=(4.0, 5.0), rec_grid=(2.0, 3.0),
    )
    if not grid.empty:
        # 第一列應是最高 overall_acc
        assert grid["overall_acc_pct"].iloc[0] == grid["overall_acc_pct"].max()


def test_grid_search_rejects_invalid_orderings():
    """門檻必須 peak > expansion > recovery；亂序不應入列。"""
    df, spx = msc.generate_synthetic_demo(n_months=60)
    score = msc.compute_historical_score(df)
    grid = msc.grid_search_phase_thresholds(
        score, spx, horizon_months=6,
        peak_grid=(5.0,), exp_grid=(7.0,), rec_grid=(8.0,),  # 全亂序
    )
    assert grid.empty
