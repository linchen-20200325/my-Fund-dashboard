...
| **EX-PASSTHRU-1**(v19.246 R15 重 framing:**原 doc 描述錯誤** — 3 fn 實際全是 L1 facade 不是 pass-through) | UI 直呼以下 L1 facade fetcher:<br>- `repositories.fund.tdcc_search_fund`(`ui/tab2_single_fund.py:147`)— 多 endpoint(TDCC 3-2 + 3-4)整合 + dedup + nav merge + keyword match<br>- `repositories.fund.get_latest_fx`(9 caller files / 18 call sites)— **4 源 fallback chain**(Yahoo → FRED DEX* → open.er-api → Frankfurter)+ positive-only `_FX_CACHE` TTL_300 + currency normalize + `=X` 補綴 + v19.201 P2-6 註冊 `_CACHE_REGISTRY`<br>- `repositories.news_repository.fetch_market_news`(`ui/tab1_macro.py:1188` / `ui/tab3_t7_ledger.py:2710`)— 11 RSS feeds 並聯 + keyword filtering + systemic risk classify + dedup + sort | L3 UI 直接 import L1 facade(multi-source orchestration + cache + 後處理 + 業務分類)而**非** L2 wrapper | **v19.246 R15 復查發現原 doc framing 錯誤**:這 3 fn **實際是 L1 facade**(EX-PASSTHRU-1「pass-through」描述不對),全帶多源 fallback chain + 結果後處理 + 自帶 cache。**理論上應升級**(規則嚴格),但實務評估:<br>(a) 3 fn 各別運作穩定 ≥1 年(v18.x 以來)<br>(b) 升級 blast radius 大:`get_latest_fx` 9 caller / `tdcc_search_fund` 1 caller / `fetch_market_news` 2 caller<br>(c) L2 facade 加一層後,業務邏輯仍要決定留 L1 還是搬 L2,真要動 = 完整重寫 fallback chain(類似 R8 EX-L1ORCH-1 升級規模)<br>**§-1 工作準則**:沒 user 明確指派「動 EX-PASSTHRU-1 升級」或實際 bug 觸發 → 維持例外,doc framing 修正承認 L1 facade 模式。**升級觸發條件(新)**:user 明確要求集中 cache(跨 fetcher 統一 TTL)、新增第 5 個 source、或結果後處理出 bug → 才開升級 PR。F-H6 v19.79 原決策 + v19.246 R15 doc 對齊。 |
