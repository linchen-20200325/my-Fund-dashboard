...
5. **例外僅限唯讀、不產生 diff 的動作**（純對話回答、讀單一檔案確認一個事實）—— **只要會寫入檔案就一律派工**，改文件、改本憲法同樣算；**「看起來很小的改動」不算例外**（理由見下）。**⚠️ 唯讀但跨多檔的探索 / 稽核不在例外內**，依規則 4 仍須多組獨立派工 —— 例外針對的是**動作規模**，不是「唯讀」本身。把工作**發回原 subagent 續做**仍屬派工，不算總管自己動手。
