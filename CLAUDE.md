...
| **EX-PASSTHRU-1**(v19.247 R16 部分升級:`get_latest_fx` UI 直 import L1 退役) | UI 直呼以下 L1 facade fetcher(剩 2 個):<br>- `repositories.fund.tdcc_search_fund`(`ui/tab2_single_fund.py:147`)— 多 endpoint(TDCC 3-2 + 3-4)整合 + dedup + nav merge + keyword match<br>- ~~`repositories.fund.get_latest_fx`~~ **(v19.247 R16 升級)**:9 caller files / 18 call sites 全 migrate 至 L2 `services.fund_service.get_latest_fx`(thin facade 呼 L1 實作,L1 業務 0 改動)<br>- `repositories.news_repository.fetch_market_news`(`ui/tab1_macro.py:1188` / `ui/tab3_t7_ledger.py:2710`)— 11 RSS feeds 並聯 + keyword filtering + systemic risk classify + dedup + sort | L3 UI 直接 import L1 facade(剩 2 fn);v19.247 R16 後 `get_latest_fx` 已上提 L2 wrapper,僅 `tdcc_search_fund` + `fetch_market_news` 仍適用本例外 | **v19.247 R16 部分升級記錄**:9 UI caller 全 migrate `from services.fund_service import get_latest_fx`,L2 thin facade 呼 L1 實作(允許 L2→L1 方向)。L1 業務 0 改動(`diagnose_fx_sources` internal use 保留)。剩 2 fn 維持例外:`tdcc_search_fund`(1 caller)+ `fetch_market_news`(2 caller),caller 數少升級 ROI 不對等,§-1 等指令再動。**升級觸發條件(維持)**:user 明確要求集中 cache、新增 source、後處理 bug。F-H6 v19.79 原決策 + R15/R16 對齊。 |
