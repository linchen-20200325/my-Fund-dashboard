...
7. **規則 5／6 對「總管自己的產出」同樣適用 —— 派工單是其中最危險的一種。** 規則 5 已明訂「**會下判斷／會給結論／會寫入檔案 → 一律派工，不論動作看起來多小**」，規則 6 已明訂總管的全稱句「**要嘛派一組獨立驗，要嘛明說沒驗過**」。**這兩條的文字從一開始就涵蓋總管自己，本項不新增任何規範、也不放寬任何一條。** 2026-08-31 客戶質問後盤點出來的不是「缺一條規矩」，而是另一件事：**總管把規則 5／6 完整套用在 subagent 的產出上，卻從未套用在自己的產出上** —— 十項自身錯誤，**零派工、零獨立驗**（逐項見 §-2.A）。

   **為什麼派工單是最危險的那一種產出**：它是**唯一不經任何檢查就直接驅動 production 工作的東西**。subagent 的 diff 上游有規格、下游有 CI、有獨立稽核、有總管複驗；**派工單的上游沒有任何人**。它的錯會被**放大到每一個收到它的組**，而且**只會在下游把成本付掉之後**才浮出來 —— 十項裡有四項（#1／#8／#9／#10）就長在派工單裡，**四項全部由執行組照做之後才抓到**：執行組在替總管做品管，而且是**事後**。

   ⚠️ **本項要擋的自我豁免句**：「**這是我要交給別人做的規格，不是我自己動手寫實作，所以不算產出。**」對照規則 5 自己的警語（「『這件事小到不用派工』這個判斷本身，就是最常出錯的地方」）—— **派工單正是規則 5 第二項「會下判斷或給結論」所指的東西**，規則 5 一個字都不必改就已經涵蓋它；缺的從來不是條文，是**真的照著做**。**六款執行條款與十項實證見 §-2.A。**
...
> - **本節的十次（2026-08-31，本 repo）**：總管**多半沒有自己動手寫實作** —— 他寫的是**派工單、規格與判斷**。病灶是**這些產出從未經過規則 5／6**，也就是**把規則 5／6 只朝外用、不朝內用**。
>
...
⚠️ **#1／#8／#9／#10 四項全部長在派工單裡，四項全部由下游執行組抓到** —— 而且都是在他們**照著錯的規格把成本付掉之後**。
...
本 session 這件事發生**四次**（#1／#8／#9／#10），而**四次全是順便發現的** ——
...
- **本組已實測**（指令與數字如上）：slow lane 的收集與執行、`pytest.ini` 的 `addopts`、
  ...
  只解析表格**第一格**裡的 `EX-` 例外 ID，本節新增表格的第一格是 `#1`~`#10`，**不受影響**。
- **本組未實測**：表中 **#1~#9 這九項的事發經過**（那是總管提供的事故紀錄，本組**沒有**回頭重建當時的現場）；
  「十項**全部**零派工」這句**全稱句**同樣出自總管，本組未獨立查證。
