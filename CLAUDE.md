...
7. **總管自己的產出同樣要驗 —— 派工單是其中最危險的一種。** **規則 6** 已明訂：總管自己產出的全稱句**要嘛派一組獨立驗，要嘛明說沒驗過**，且**不得**寫進任何會被後人讀的記錄當成事實。**派工單正是「總管依規則 2 產出的規格與判斷」，所以本項的法源是規則 6。** 2026-08-31 客戶質問後盤點出來的是：**總管把這條完整套用在 subagent 的回報上，卻從未套用在自己的派工單上** —— 十三項自身錯誤，**零獨立驗**（逐項見 §-2.A）。
   ...
   - ~~「規則 5 已明訂『會下判斷／會給結論／會寫入檔案 → 一律派工，不論動作看起來多小』……**這兩條的文字從一開始就涵蓋總管自己**」~~
     ...
     **三步都可自驗**：(1) 派工單 ＝ **規格 ＋ 判斷**，正中但書的列舉；(2) 「此列」＝規則 5 的三個 bullet，**含本項原本引用的第二項（會下判斷或給結論）**；(3) 但書的**唯一**回收條件是「**寫進檔案**」，而**派工單不是 repo 檔案** → 回收條件不成立。
     → **依現行規則 5 的文字，派工單本來就免派工，只受規則 6 拘束。**
   - ~~「十三項自身錯誤，**零派工**、零獨立驗」~~ → **「零獨立驗」**。**依但書，派工單零派工是合規的**，把它當成違規事實是錯的；**真正的違規只有規則 6 的「零獨立驗」**。⚠️ 下方表格「**有派工嗎**」那一欄，記的其實是「**有沒有第二雙眼睛**」，**不是**「有沒有違反規則 5」。
...
- **不是只有那個 Python 守衛會碰 `CLAUDE.md`**：`.pre-commit-config.yaml` 有
  **4 個 hook 沒有寫 `exclude`**（`trailing-whitespace`／`end-of-file-fixer`／
  `check-merge-conflict`／`check-yaml`），會**逐字處理本檔且會擋 CI**。
  2026-08-31 實測**目前全部安全**，但改本檔時**它們同樣是關卡**，不要只想著那一個 Python 守衛。
- **容量登記（量測日 2026-08-31，量到本次 commit 為止）**：本檔
  `252,861`（`4851465`）→ `271,340`（`23c5544`）→ **約 `279,783`（本 commit）**，
  §-2 三輪合計 **+26 KB**；上限 `check-added-large-files` 為 **512,000**，目前 **54.6%**。
