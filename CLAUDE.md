...
| **EX-PASSTHRU-1** | UI 直呼以下 L1 fetcher(無對應 L2 業務 wrapper):<br>- `repositories.fund.tdcc_search_fund`(`ui/tab2_single_fund.py:147`)<br>- `repositories.fund.get_latest_fx`(自帶 manual `_FX_CACHE` TTL_300 + v19.201 P2-6 註冊 `_CACHE_REGISTRY`;9 caller files / 18 call sites:`ui/tab2_single_fund.py` ×3, `ui/tab_fund_grp_health.py` ×4, `ui/helpers/d_mode.py` ×2, `ui/helpers/fund/checkup.py` ×2, `ui/helpers/fund_grp_health/investment.py` ×2, `ui/tab5_data_guard.py` ×2, `ui/helpers/v2_editor.py` ×1, `ui/tab3_portfolio.py` ×1, `ui/tab3_t7_ledger.py` ×1 — v19.244 R12 doc 對齊;原 doc 只寫 1 處 caller 已嚴重漂移)<br>- `repositories.news_repository.fetch_market_news`(`ui/tab1_macro.py:1188` / `ui/tab3_t7_ledger.py:2710`) | L3 UI 可直接 import L1 「pass-through 用 + 無 L2 業務值」的 fetcher | §8.2 規則「cache 才能集中」核心理由失效於本場景:fetcher 已**自帶 cache**(無外散風險)或**無 cache**(L2 wrapper 無從補上),且**無 L2 業務 wrapper**(多源 fallback / 結果後處理皆無)— L2 加一層 = pure pass-through = §8.1 step 6「用不到的抽象」反例。**升級觸發條件**:若未來新增 cache 需集中(跨 fetcher 統一 TTL)、多源 fallback chain、或結果後處理 → 應升級 L2 service。F-H6 v19.79 決策;v19.244 R12 復查 9 caller files / 18 call sites 升級觸發條件未達到(無多源 fallback / 結果後處理), 維持例外。 |
