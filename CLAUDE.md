...
- **TW 出口 YoY**:~~`fetch_tw_export_yoy` 仍掛 `TaiwanMacroEconomics`(不存在)→ 現況恆無資料,新源(MOF 線,同 Stock repo)待 user 點名再評估(§-1)~~
  ...
  **驗證指令(repo 根執行)**:`git grep -n "dataset.*TaiwanMacroEconomics" -- '*.py'` → **0 命中**;寬掃 `git grep -n "TaiwanMacroEconomics" -- '*.py'` 的命中**全部**是「這個 dataset 不存在」的歷史註解、守衛測試與 schema 範例字串,**沒有一處是取數**(人工逐一判讀)。
