...
- §8.2.A 例外清單:EX-CACHE-1 / EX-AI-1 / ~~EX-POLICY-1~~(v19.212 P0-3-#4 退役) / EX-CRUD-1 / EX-PASSTHRU-1 / EX-L1ORCH-1(v19.238 登錄)
...
| **EX-L1ORCH-1** | L1 fund orchestrator import L2 `calc_metrics`:<br>- `repositories/fund/fx_and_main.py:27` → 於 `fetch_fund_by_key`(line ~67)收尾呼叫<br>- `repositories/fund/fund_orchestration.py:30` → 於 `_finish_metrics`(line ~373)+ `fetch_fund_from_moneydj_url`(line ~971)收尾呼叫 | L1 fund orchestrator 抓 NAV 後,即時呼 `services.fund_service.calc_metrics(s, divs, risk_override)` 把序列 packaging 成 `result["metrics"]` dict | §8.2「L1 不得 import L2」核心理由是避免 L1 把業務語意硬編碼。但本場景的 L1 orchestrator(fund_orchestration 1139 LOC + fx_and_main 627 LOC)定位為「**抓 + 打包**」的 facade,單一 metric call 是 packaging 收尾。三條替代路徑都更差:<br>(a) 整檔搬 L2 → L2 反過來 import 大量 L1 `_src_*` fetcher,違憲方向反轉<br>(b) 拆 return raw,caller 自呼 → 公開 API 改動(`result["metrics"]` 約 10+ UI/L2 caller)= §8.1 step 6「過度設計」<br>(c) Lazy import 隱藏 → 純 cosmetic,靜態檢查仍違憲<br>**緩解措施**:本例外僅限 `calc_metrics` 單一 symbol;同檔嚴禁追加其他 L2 import(`services.macro_service` / `services.portfolio_service` 等)。**升級觸發條件**:若 orchestrator 需要呼 ≥2 個 L2 symbol、或 L2 開始把業務判斷 push 回 L1 → 改採方案(b)拆 return + L2 wrapper。v19.238 決策。 |
