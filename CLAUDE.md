...
7. **總管自己的產出同樣要驗 —— 派工單是其中最危險的一種。** **規則 6** 已明訂：總管自己產出的全稱句**要嘛派一組獨立驗，要嘛明說沒驗過**，且**不得**寫進任何會被後人讀的記錄當成事實。**派工單正是「總管依規則 2 產出的規格與判斷」，所以本項的法源是規則 6。** 2026-08-31 客戶質問後盤點出來的是：**總管把這條完整套用在 subagent 的回報上，卻從未套用在自己的派工單上** —— 十三項自身錯誤，**零獨立驗**（逐項見 §-2.A）。
   ...
   - ~~「規則 5 已明訂『會下判斷／會給結論／會寫入檔案 → 一律派工，不論動作看起來多小』……**這兩條的文字從一開始就涵蓋總管自己**」~~
     ...
     **總管的判斷：讀法 A 較強，但這是解讀，不是事實**（理由三條，皆出自規則 5 自己的文字）：
     ...
     2. 但書結尾寫「**豁免的是『總管可以下這個判斷』，不是『總管可以自己動手寫』**」——
        ...
        不是總管自己動手寫成的檔案。
     3. 讀法 B 會使規則 5 **不可滿足**：寫派工單要先派工，而派那一工又需要一張派工單 → **無限遞迴**。
   ...
   **為什麼派工單是最危險的那一種產出**：它是**唯一不經任何檢查就直接驅動 production 工作的東西**。subagent 的 diff 上游有規格、下游有 CI、有獨立稽核、有總管複驗；**派工單的上游沒有任何人**。它的錯會被**放大到每一個收到它的組**，而且**只會在下游把成本付掉之後**才浮出來 —— 十三項裡有**六項**（#1／#8／#9／#10／#11／#12）就長在派工單裡，**六項全部由執行組照做之後才抓到**：執行組在替總管做品管，而且是**事後**。
...
| 5 | 自訂的 `session_` 驗收字表**必然誤報** —— 全 repo 110 檔含它，其中 71 個是 `st.session_state` | ❌ | 稽核組實測 |
...
⚠️ **#1／#8／#9／#10／#11／#12 六項全部長在派工單裡，六項全部由下游執行組抓到** —— 而且都是在他們**照著錯的規格把成本付掉之後**。
...
且「十三項全部**零獨立驗**」是一句**承重數字**，混入非總管項就汙染了它（同本節開頭「**不要合併計數**」）。
...
- **(c)** PR body 與 commit message 掃識別碼：先掃 `session_[0-9A-Za-z]{16,}`，再逐一取 `claude.ai/code/session` 的上下文判讀。⚠️ **不要用 `grep session_`**（實證 #5：全 repo 110 檔含它、71 個是 `st.session_state`，**必然誤報**）。
- **(d)** 檔案邊界**逐檔**確認無夾帶。
...
- **本組未實測**：表中 **#1~#7、#9、#10 的事發經過**（那是總管提供的事故紀錄，本組**沒有**回頭重建當時的現場）；
  「十三項**全部零獨立驗**」這句**全稱句**同樣出自總管，本組未獨立查證。
...
- **容量登記（量測日 2026-08-31，量到本次 commit 為止）**：本檔
  ...
  → **約 `285,567`（本 commit）**，§-2 **五個 commit** 合計 **約 +32 KB**；
