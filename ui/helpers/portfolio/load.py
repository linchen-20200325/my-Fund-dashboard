    ...
    with ThreadPoolExecutor(max_workers=4) as _ex:
        ...
        for _fut in as_completed(_futs):
            ...
            try:
                ...
                #  (b) 同一批失敗**已經被彙總上報**了 —— 每檔的失敗訊息寫進
                #      `load_error`,由 `ui/tab3_portfolio.py`（:1573 等處）逐檔呈現。
                # ⚠️ 2026-08-28 稽核 A7 更正：本註解原本說「已被下方 `errors` 清單
                #    （`st.warning("部分基金載入失敗：…")`）彙總上報」—— **那個出口
                #    其實也活不下來**：它的下一行就是 `st.rerun()`,依已證實的機制會被
                #    沖掉。**真正活得下來的出口是 `load_error`**（上一句）。
                #    結論不變（這批仍不改色）,但引錯出處會誤導下一個人。
                # 另註：本函式結尾的 `st.rerun()` 也會把整段進度 log 一起沖掉,
                #      所以這一行本來就只在載入過程中短暫可見。
