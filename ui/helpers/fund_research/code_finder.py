...
2. **本檔是 `repositories.fund.tdcc_search_fund` 的第二個 UI 呼叫點。**
   該 fetcher 已登記在 `CLAUDE.md §8.2.A` 的 **EX-PASSTHRU-1**（UI 直呼 L1 facade），
   原登記處是 `ui/tab2_single_fund.py`。**登記表的更新不在本工作包的檔案邊界內**
   （`CLAUDE.md` 不得改），已在 PR 描述具名回報請總管裁決。
   ⚠️ 不要把「測試沒紅」讀成「這件事已經合規」—— 本 repo 目前**沒有**任何機器規則
   在守 UI→L1 的呼叫點數量。
