...
def nav_age_emoji(nav_date_str, today=None):
    ...
    """
    # v19.299 FIX: 台灣時區 fallback，避免 Streamlit Cloud UTC 00:00~08:00 TW 顯示昨天
    _TW_TZ = _dt.timezone(_dt.timedelta(hours=8))
    _today = today or _dt.datetime.now(_TW_TZ).date()
...
def render_mj_freshness_banner(items: list, title: str = "MoneyDJ 資料新鮮度") -> None:
    ...
    if not items:
        return
    # v19.299 FIX: 使用台灣時區（UTC+8），避免 Streamlit Cloud UTC 服務器在 00:00~08:00 TW 顯示昨天日期
    _TW_TZ = _dt.timezone(_dt.timedelta(hours=8))
    _today = _dt.datetime.now(_TW_TZ).date()
    _parts: list = []
    ...
    st.markdown(
        ...
        f"（hover chip 看完整時戳 ｜ 規則：🟢 ≤2d / 🟠 ≤7d / 🔴 >7d ｜ ⚠️ 基金 NAV T+1~T+3 公布屬正常，燈號顯示淨值日期非抓取時間）</span><br/>"
        f"{' ｜ '.join(_parts)}"
        f"</div>",
