...
def health_column_config() -> dict:
    ...
    return {
        ...
        # 值由 unified.sharpe_provenance_by_code 產出(讀 metrics.risk_metric_meta.sharpe),
        # 標籤仍帶 MoneyDJ 頁面代號 wb07 —— 改標籤會動到 test_switch_strategy,另案處理;
        # 這裡的 help 先用「官方」/「自算」把使用者看得到的字翻譯過去。
        "Sharpe 來源": cc.TextColumn("Sharpe 來源", width="small",
            help=("左邊那個 Sharpe **到底是誰算的、量了多長的期間**。"
                  "標「官方」= 直接用 MoneyDJ 官方風險表公布的現成數字"
                  "(1Y = 一年期、⚠️ 6M = 只有六個月,期間比別檔短、跨檔比大小要小心);"
                  "標「自算 N d」= 官方沒給,本站用最近 N 個交易日的淨值自己算;"
                  "⬜ — = 完全沒有值。本欄照實顯示,不重算、不猜。")),
...
def dividend_column_config() -> dict:
    ...
    return {
        ...
        # (wb01 / local_calc / ret_1y_total / ret_1y / NAV 年化);
        # 那些字串在 L2,不在本模組,故 help 以使用者看得到的關鍵字對照說明。
        "1Y 來源": cc.TextColumn("1Y 來源", width="small",
            help="左邊那個數字是怎麼來的,可靠度由高到低:"
                 "標「MoneyDJ 官方」= 直接抄官方績效表,最可信;"
                 "「本地還原淨值法」= 官方沒給,本站用淨值 + 配息記錄還原;"
                 "「本地含息」= 本站自算(若括號寫「N d 窗口」,代表只有 N 天資料換算成一年,"
                 "跨檔比大小要小心);「純 NAV」= 只有淨值漲跌、**不含配息**;"
                 "「序列年化」= 資料最少的推估,最不可靠;「—」= 完全沒資料。"),
