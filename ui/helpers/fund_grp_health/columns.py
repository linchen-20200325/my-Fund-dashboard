...
#    本檔兩處 help 文案原本寫死「組合配置」。
#    **歸因要寫清楚：這不是七→五打壞的，它本來就壞了** —— 七→五之前 ④ 叫
#    「📊 配置 & 帳本」，七→五之後叫「📊 我的配置」，**兩個時期都沒有**
#    一個叫「組合配置」的分頁。它是憑印象手寫出來的名字，從來沒有進過
#    `_TAB_LABELS`，所以任何「比對歷史分頁名」的守衛都掃不到它。
#    舊寫法的理由仍然成立（要區分健診等權 vs ④ 依實際本金加權，這個對比是對的）；
#    被權衡掉的只是那個**憑印象手寫的名字**。
    ...
    return {
        ...
        "相對品質分": cc.TextColumn("相對品質分", width="medium",
            help="**這檔在同類裡的相對品質**(0-100,越高越前段)。6 個面向各自排名再加權:"
                 "同類官方排名 25 + Sharpe 25 + 下檔波動 20 + 操盤 10 + 配息可持續 10 + 費用率 10。"
                 "其中**「同類官方排名」用 MoneyDJ 全同類大母體**(幾百檔)、其餘 5 項用**本組同伴**"
                 "排名 —— 兩種母體不同,合起來只當「相對強弱」參考。⚠️ 這是**相對分、不是絕對分**:"
                 "「本組 N 檔」是這一組同伴裡的位置(健檢/帳本=你的持倉＋選股池、批次=你上傳那批);"
                 "「・僅參考」= 同組少於 4 檔、樣本太少。某個面向沒資料就退出計算、不會拿 0 分充數。"
    ...
    return {
        ...
        "每月配息 (TWD)": cc.NumberColumn("每月配息 (TWD)", format="%.0f",
            help="每個月大概可以領到多少**台幣現金** = 最近一筆配息 × 你的持有單位數 × 匯率。"
                 ...
                 f"「{_tab_label('portfolio')}」分頁則用各檔實際投入的本金。"),
        ...
        "每月配息單位數": cc.NumberColumn("每月配息單位數", format="%.2f",
            help="如果把每月領到的配息**再買回同一檔**,大約可以多買幾單位"
                 ...
                 f"「{_tab_label('portfolio')}」分頁用各檔實際投入本金。"),
