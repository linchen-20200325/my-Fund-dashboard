...
⛔ **但下面三件事本檔沒有裁決，也不得被引用為已裁決** —— 已回報總管：

- **(A) ia Tab 04 的清單是不是窮舉？** 它**沒有**列
  「➕ 加入與管理基金」「配置總覽」「持股重疊度」「費用與扣款」「AI 摘要」「Raw data」，
  而它的「這裡不放什麼」**也沒有排除它們**（只排除了 02／03 的職責與巢狀 `st.tabs`）。
  ⚠️ **這一點有一個具體後果，見下方「空狀態的指路依賴什麼」**：ia 的六塊裡**沒有任何
  一塊能新增基金** —— 若把 ia 的清單讀成窮舉，④ 將**無法加入任何標的**，
  而 ④ 的第一句職責是「所有會改變我部位的動作都集中在這裡」。
  **這是本檔認為它不是窮舉的理由，但那是推論，不是裁決。**
- **(B) 保單那一塊的版面** —— ia 把「保單與扣款標的」畫成 **3 欄網格裡的一張卡**；
  `policy-split-wireframe.html` **決定 E** 逐字寫「保單明細表**維持全寬**，不塞進三欄」。
  ⚠️ **本批不觸及這個衝突，因為本批一張表都沒畫**（骨架階段沒有列可顯示）。
  ⛔ **下一批把保單明細接上時，這一條必須先有裁決** —— 那時才是「3 欄卡 vs 全寬表」
  真正對撞的時刻。
- **(C) 區塊名** —— ia 叫「保單與扣款標的」，policy-split 把 ④ 那一區改名「📋 保單資料」。
  本檔用 ia 的字面（依 1／2／3），**未裁決**。改的話只要動
  :data:`BLOCK_POLICY` 一個常數，畫面其餘一格都不用動。
...
- ⛔ **但它的有效性繫於上面的未決事項 (A)**：**本頁自己沒有畫「加入與管理基金」**
  （ia Tab 04 沒有列它）。若日後 ④ 真的被收斂成 ia 的六塊，這句指路會**當場變成死指路**，
  而且使用者將**沒有任何地方可以加基金**。
  → **這是 (A) 必須裁決的具體理由，不是抽象的版本學問題。**
...
- :data:`_DEFAULT_BUDGET_TWD` ＝ **0**，**刻意不照線框的 200,000**。
  理由：那是**使用者的錢**，不是模擬參數。預先填一個金額，使用者只要沒注意到就會
  拿到一份「用他沒有的 20 萬」算出來的再平衡計畫。§1 取嚴 → 預設 0，
  而且 **0 不算送出**（:func:`_normalise_plan`），退回等待輸入。
  ⚠️ **這是本檔唯一一處偏離線框字面的地方，已登記回報總管。**
...
#: 線框 Tab 04 逐字。⚠️ **未裁決的區塊名**，見模組 docstring 的 (C)。
