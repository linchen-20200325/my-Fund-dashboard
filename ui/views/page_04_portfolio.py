...
- **有持倉、但這一塊的內容還沒填** → :data:`_PENDING_NOTE` 的灰態。
  ...
  **四塊灰態現在各自有一個真的去處**，一塊都不再指回本頁的試算 ——
  ...
  到不了舊 ④）。換股顧問那一塊由測試檔的
  `test_the_switch_card_points_at_the_place_that_really_renders_it`
  用 AST 確認「指的那個地方真的會渲染它」；
...
ADD_FUND_SCOPE_NOTE: str = (
    ...
    "指到 `where_to_find('pf_load')`（🗂️ 保單分組視圖）。"
...
REASON_SWITCH: str = (
    ...
    # ⛔ **這裡刻意寫「它」，不寫區塊名** —— 那個名字有 SSOT
    #    （`story_nav.section_label("switch")`），寫進**活字串**就是手抄第二份，
    #    `test_the_switch_block_name_comes_from_the_ssot_not_a_hand_copy` 會當場轉紅
    #    （2026-09-08 CI 實測抓到本組，不是推論）。
    #    ⚠️ 本常數是 module 層，取不到 `switch_block_label()`
    #    （那一支必須在**渲染當下**求值，理由見它自己的 docstring）——
    #    所以正解是**不要提那個名字**，反正卡片標題與指路都已經帶著它了。
    "它本身在下面這個地方就用得到")
...
def _render_mix() -> None:
    ...
    if _core is None:
        ...
        #    **2026-09-08 第三輪就地更正（有意識的更正，不是漏刪 · 決策者：AI 總管，
        #    依第三輪獨立稽核 A1）—— 這兩行的兩個半句今天都不成立**：
        #      (1)「去哪補 ＝ ➕ 加入與管理基金」→ **已於第二輪改掉**（見下一段的劃線）；
        #      (2)「同空狀態那一則，由 AppTest 實跑釘住」→ **它引用的那一則已經被本 PR 自己改掉**
        #          （空狀態現在指本頁的加入區，不是舊 ④），所以這個佐證**指向一個不存在的前例**。
        #    ⚠️ **位置本身就是教訓**：第二輪在**下一行**寫了一段完整的劃線更正，
        #       **卻沒回頭看它上面這兩行** —— 同一支函式裡，改了新的、留下舊的。
        #       第二輪擋下的 `_render_policy` docstring 是**同一種形狀**（文件在它描述的
        #       程式碼上方說假話），修了那一支、漏了隔壁這一支。
        # ⛔ **2026-09-08 第二輪：指路改掉了（有意識的更正，不是漏刪 · 決策者：AI 總管
        #    依獨立稽核）** ~~`where=where_to_find("pf_add")`~~
        #
        #    **舊指路的用意仍然成立**：這一格缺的是「本金」，所以它要指到「去哪填本金」，
        #    而**不是**指回本頁的試算 —— 那個方向從頭到尾都是對的。
        #    **被推翻的是它的事實面：`pf_add` 那一區填不了本金。**
        #    實測（AST，全檔掃 `invest_twd` 的寫入點）：`ui/tab3_portfolio.py`
        #    **只有** `invest_twd: 0` 的字面值（新條目）與原樣搬運既有值，
        #    **沒有任何一處把使用者輸入的金額寫進去**。使用者照著走會撲空。
        #    真正打得到字的是 `ui/tab3_t7_ledger.py` 的 `🟨 淨投資金額 (NT)`
        #    （`key=f"t7_init_inv_{pk}"` → `_f_obj["invest_twd"] = int(_inv)`），
        #    它住在舊 ④ 的「💼 持倉戰情（T7 帳本）」底下 ＝ `where_to_find("pf_ledger")`。
        #
        #    ⚠️ **這一行與 :func:`_status_tiles` 的 💰 那一格必須永遠是同一個答案** ——
        #    在此之前一個指 `pf_add`、一個指 `pf_policy_admin`，
        #    **同一個缺失欄位、同一個畫面、兩句相反的話**。兩處已一起改。
...
def invested_hint(*, n_loaded: int, n_waiting: int, n_failed: int) -> tuple[str, str]:
    ...
    **因為前一版就是在這裡出的錯，而當時的守衛看不到。** 這一格的指路連續錯了三次，
    ...
    其餘（一筆都沒有）—— ⚠️ **防禦性死碼，不是發生過的變種，讀的時候不要混為一談**
        → 指**本頁自己下面**那個加入區，不是別的分頁。
        ⛔ **2026-09-08 第五輪就地更正（有意識的更正，不是漏刪 · 依第四輪獨立稽核）**：
        ~~上一版在這個狀態也是錯的，而且沒有任何測試看得到：它落進 ``else``
        說「先把標的抓回來」＋指 `pf_load`……本組是在修第三次那個錯的時候自己找到它的。~~

        **這一段把一個「到不了的分支」寫成了「發生過的第四個變種」，那是假的。**
        **實測（spy 包住本函式，逐 session 形狀量）**：:func:`_render_policy` 開頭是
        ``if not _all_portfolio_rows(): _render_add_fund(); return``，而
        ``_holdings() ⊎ waiting ⊎ failed`` ＝ 全部 dict 條目
        ⇒ **走到這裡時 ``n_loaded + n_waiting + n_failed ≥ 1`` 恆真**。
        本組實測 **9 種 session 形狀**（空 list／鍵不存在／各單一態／兩兩混合／
        非 dict 混入／空 dict），**沒有一種讓這一支被呼叫到**；
        空清單的兩種**根本不會呼叫** :func:`invested_hint`。

        **兩邊理由並陳** —— 舊表述**有一半仍然成立**：那一版的 ``else`` 分支確實
        **在邏輯上**會對「三個都 0」給出一句錯話（指一顆不存在的鈕），
        「本組自己找到」也是事實；**被推翻的是它的射程** ——
        它從來**沒有到過螢幕上**，所以不該與前三個變種並列成「第四個變種」。

        ⭐ **那為什麼還留著這一支？** 因為它是**餵錯資料時的落點**：第四輪稽核的
        M11（``pending_split(_loaded)``）就讓畫面**真的印出了這一句**
        （「⬜ 一檔標的都還沒有」，而同一畫面下一行說「另有 2 檔尚未載入或載入失敗」）。
        **它到不了不是因為它多餘，是因為上游現在餵對了** —— 而「上游有沒有餵對」
        由 `tests/test_wf04_add_holdings.py::`
        `test_the_call_site_feeds_invested_hint_the_right_three_numbers` 守著。

        ⛔ **2026-09-08 第五輪就地更正：上一版替「不要刪」寫的理由是反的**
        （**有意識的更正，不是漏刪** · 依第四輪稽核 R-C · 本組實測複驗）
        ~~不要因為它是死碼就刪掉：刪了之後同一個錯會變成 ``IndexError``
        或更糟的靜默，而不是一句看得出不對的話。~~

        **實測（刪掉這一支 ＋ 疊加 M11，真渲染 S2）**：函式落到結尾回 ``None``，
        呼叫端解包 → ``TypeError: cannot unpack non-iterable NoneType object``，
        被 `safe_section` 接住，畫面出現**紅框** ——
        「**「保單與扣款標的」區塊渲染失敗** — `TypeError`… 此區塊已隔離」＋技術細節收合區。
        **不是 ``IndexError``，更不是靜默。**

        ⚠️ **而這一刀砍在相反的方向**：在 §1（Fail Loud, Never Fake）之下，
        **紅框是「炸掉」，留著這一支才是「造假」** —— 對照組實測：留著 ＋ M11 →
        畫面印出「一檔標的都還沒有」，而**同一畫面下一行寫著「另有 2 檔尚未載入或載入失敗」**。
        **一句看起來很正常、但與隔壁互相矛盾的假話。**

        **所以真正的理由是這個（不是上面那句反話）**：
        1. **本函式是純函式，契約應該是全函數（total）** —— 收三個 ``int``、回 ``(str, str)``。
           對一個合法輸入三元組 ``(0, 0, 0)`` 回 ``None``，是**部分函數**，
           那是更差的介面；未來任何新呼叫端都可能合法地傳進 ``(0, 0, 0)``。
        2. **:func:`test_the_invested_hint_is_right_in_every_state` 逐一跑 27 種組合**
           （含 ``(0,0,0)``）。刪掉這一支，那條守衛會**在解包時炸掉**而不是斷言 ——
           **一條會炸的守衛，讀起來像壞掉，不像抓到東西。**
        3. **「餵錯就炸紅框」這個角色，現在由呼叫端那條守衛承擔，而且是在出貨前** ——
           靠 runtime 崩潰去接，代表**已經出貨、使用者已經看到紅框**了。

        ⚠️ **據實寫下這個取捨的代價，不粉飾**：留著它，代表**未來若出現一條
        呼叫端守衛沒有覆蓋到的餵錯路徑**，畫面會是**一句貌似合理的假話**，
        而不是紅框。**這個取捨成立的前提，就是那條呼叫端守衛真的守得住** ——
        前提若不再成立（例如有人繞過 :func:`_status_tiles` 直接呼叫本函式），
        **這個決定要重新評估**。
...
def _status_tiles() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"label": STATUS_INVESTED_LABEL,
         ...
         # ⛔ **2026-09-08 第二輪：這一格的指路改掉了（有意識的更正，不是漏刪）**
         #    ~~"where": where_to_find("pf_policy_admin")~~
         #    ~~"missing": "…金額來自保單表的 invest_twd 欄，在雲端 Sheet 上填好後讀回"~~
         #
         #    **舊指路本身走得通**（在雲端 Sheet 上填好、再從保單管理讀回，確實會生效），
         #    **被推翻的是它旁邊那句推理**：本組上一輪寫過「App 內沒有通用的輸入口」——
         #    **那是一句假的全稱句**。實測（AST，見下）：
         #      · `ui/tab3_portfolio.py`（＝ `pf_add` 那一區）**從來不寫使用者給的金額** ——
         #        它只會生出 `invest_twd: 0` 的新條目，或原樣搬運既有值。
         #      · `ui/tab3_t7_ledger.py` **有**一個 `🟨 淨投資金額 (NT)` 的 `number_input`
         #        （`key=f"t7_init_inv_{pk}"`），送出後 `_f_obj["invest_twd"] = int(_inv)`
         #        直接寫進 `portfolio_funds`。
         #        ⛔ ~~**那是全站唯一「使用者打一個金額進去、它就落在持倉清單上」的地方**，~~
         #        ⛔ ~~而且無 OAuth／schema 條件。~~
         #        → **2026-09-08 第三輪就地更正（有意識的更正，不是漏刪 · 依稽核 A4）——
         #          兩個半句都過強**：
         #          (1)「**全站唯一**」不成立：T7 的 A/B/C 落帳（`t7a_amt_new` /
         #             `t7a_amt__{pk}` 等 `number_input`）經
         #             `ui/helpers/io/data_registry.py::_sync_invest_twd_from_ledgers`
         #             同樣會寫 `portfolio_funds[i]["invest_twd"]`。
         #             ⚠️ **它們也住在 `pf_ledger` 底下 ⇒ 指路的結論不變**，變的是這句話的射程。
         #          (2)「**無 OAuth／schema 條件**」就 OAuth／schema 而言為真，
         #             但**漏掉真正會咬人的那個條件** —— usable-funds 閘門（見下方 ⭐ 那一段）。
         #             那正是第三輪擋下本輪的原因。
         #
         #    **改指到那裡的真正理由不是「換一個比較好的地方」，是本頁自己在說兩句相反的話**：
         #    同一個缺失欄位（`invest_twd`）在 :func:`_render_mix` 指去 `pf_add`、
         #    在這一格指去 `pf_policy_admin`。**兩句話不可能同時是對的**，而實測顯示
         #    **兩句都不是最好的那一句** —— `pf_add` 那裡根本填不了，
         #    ⛔ ~~`pf_policy_admin` 要離開 App 去改 Sheet 再回來讀。~~
         #    → **2026-09-08 第三輪就地更正（有意識的更正，不是漏刪 · 依稽核 A4）——
         #      這句話只在 OAuth 設定下成立，不是無條件**：SA（非 OAuth）設定下
         #      `ui/helpers/portfolio/policy_admin_section.py` 的 `form_policy_upsert`
         #      **有 in-app 的 `number_input("invest_twd")`** → `upsert_policy_row`，
         #      也就是**在 App 裡就填得到**。
         #      ⚠️ **這一點沒有推翻本格的選擇，但它縮小了排除 `pf_policy_admin` 的理由**：
         #      現行選擇建立在「**哪一個在本格出現的那個狀態下一定渲染得出來**」，
         #      不再建立在「哪一個不用離開 App」。
         #    → 兩處統一指向 `pf_ledger`（＝舊 ④ 的「💼 持倉戰情（T7 帳本）」）。
         #
         #    ⚠️ **兩個誠實的但書，刻意寫在文案裡不藏起來**：
         #    (1) 那張表要**三欄一起**才算數（`_inv <= 0 or _cu <= 0 or _fx <= 0` → `continue`），
         #        所以文案講「金額、平均買入淨值、匯率三欄一起填」，不騙他只填一格就好。
         #    (2) `add_entry.NEW_ENTRY_INVEST_TWD` 的就地註解仍然成立：**金額的真相源是保單表**。
         #        T7 那條是「把對帳單抄進來」的入口，不是第二個真相源 —— 它送出後同批
         #        `_funds_to_sheet` 會把有保單號的那幾檔回寫保單分頁。**兩者不衝突。**
         #    ⚠️ 這個 key 與 `pf_add` / `pf_load` 一樣指**舊 ④ 的區塊**，舊分頁拔除時要一起回頭處理。
         # ⛔ **2026-09-08 第三輪：這一格的指路與文案改成「看狀態」（有意識的更正，不是漏刪
         #    · 決策者：AI 總管，依第三輪獨立稽核以**真渲染**擋下）**
         #    ~~"missing": "已載入的標的都沒有填投入金額，加不出總投入…"（無條件）~~
         #    ~~"where": where_to_find("pf_ledger")（無條件）~~
         #
         #    **舊寫法在「有已載入的標的」那個狀態下是對的**，那一半沒有變。
         #    **被推翻的是它的無條件性** —— 它在**另一個狀態**下同時錯了兩件事：
         #    「加了但還沒抓到」（rows 有、一檔都沒 `loaded`）時 ——
         #      (1) **文案是假的**：它說「**已載入的**標的都沒有填投入金額」，
         #          而那個狀態下已載入的是 **0 檔**（同一畫面下一行的範圍說明就寫著「只加已載入的 0 檔」）。
         #      (2) **指路是死的**：T7 的「✏️ 編輯持倉」住在 `if not _pf_t7:` 的 **else** 分支，
         #          而 `_pf_t7 = usable_funds(...)`。一檔都還沒抓到時那整個 expander **不渲染**，
         #          `🟨 淨投資金額 (NT)` **不存在** —— 使用者走過去會看到
         #          `st.info("請先在上方「組合管理」載入至少一檔基金…")`。
         #    ⚠️ **而那個狀態是本 PR 自己造出來的**（`origin/main` 在該狀態整塊不畫），
         #       所以這是本批的債，不是既有債。
         #
         #    ⭐ **為什麼用 `_loaded` 當分界，而不是隨便挑一個條件** —— 這是**同一個謂詞**：
         #      · 本頁 :func:`_holdings` 濾的是 `loaded and not load_error`；
         #      · T7 的閘門 `ui/helpers/session.py::fund_is_usable` 也是 `loaded and not load_error`。
         #      **兩者逐字相同 ⇒ `_loaded` 非空 ⟺ T7 那個輸入格一定渲染得出來。**
         ...
         #    → **這是同一個病的第三個變種**：一、指去做不到那件事的地方；
         #      二、指去那個狀態下不存在的地方；三、指去我們自己剛說沒用的地方。
         #      **三次都是「換目的地」而不是「換狀態」。**
         #    ⛔ **2026-09-08 第五輪就地更正（有意識的更正，不是漏刪 · 依第四輪獨立稽核）**：
         #      ~~四、指去一顆不會渲染的鈕。**四次**都是…~~ —— **(iii) 不是「第四個變種」。**
         #      實測 9 種 session 形狀，`_render_policy` 的開頭 early-return 讓
         #      「三個都 0」**到不了**（詳見 :func:`invested_hint` docstring 最後一段）。
         #      **它是防禦性死碼，從來沒有到過螢幕上**，與前三個並列會讓後人以為它發生過。
...
def _render_policy() -> None:
    ...
    **一筆都沒有時只畫加入欄位，其餘一律不畫** —— 這不是偷懶，是鐵則 04：
    ...
    ## ✅ 狀態列四格的「去哪補」（2026-09-08 第二輪已修完）

    ⛔ **舊表述整段保留在下面加刪除線 —— 有意識的更正，不是漏刪**
    （決策者：**AI 總管**，依第二輪獨立稽核；理由兩邊並陳）。
    **為什麼非改不可**：這一段就長在它所描述的那段程式碼**正上方**，
    而下一個人是**先讀 docstring 才讀程式**的。一份在自己描述的程式碼上方
    說假話的文件，比 PR 描述說假話更糟 —— 那正是本 repo 整部憲法在防的東西。

    ~~⚠️ **本批沒有修、但本批讓它更常被看到的一件事（登記，不夾帶）**~~
    ...
    **舊表述哪一半仍然成立**：「那幾條指路的目的地都不對」**完全正確**，
    而且是它先指出來的 —— 沒有那一段登記，這件事不會被排進來。
    **哪一半被推翻了**：
    1. **「三格」是錯的，實際是四格** —— 漏算了 📋 保單（`_status_tiles` 回傳的
       **4 個** dict 當時**全部**帶 `where_to_find("pf_add")`，AST 實數）。
    2. **「本批仍然不動它」「請另派一批」自 2026-09-08 第二輪起為假** ——
       就在這一段下面的程式碼**已經改完了**。當初判斷不夾帶是對的
       （那時它確實是三個不同動作、各要先決定新五頁的落點）；
       **被權衡掉的是它的前提**：`CLAUDE.md §1` 之下，
       「明知是假、還讓它更常出現在使用者眼前」不能出貨，而本批正是讓它更常出現的那一批
       —— 於是它從「別批的事」變成**本批自己的收尾義務**（`§-1.5.1c 判定 3`）。

    **現行四格的去處（逐格不同，理由見 :func:`_status_tiles` 就地註解）**：

    ===================== ==========================================================
    格                     去哪補
    ===================== ==========================================================
    📒 目前帳本            `pf_policy_admin`（該區 `btn_pick_my_sheet` 寫 `policy_sheet_id`）
    📋 保單                **本頁自己**的加入表單那格「保單編號」（射程限制已寫進文案）
    🕐 上次讀回            `pf_policy_admin`（該區 `t3_io_panel_load_run` 寫 `t3_last_load_at`）
    💰 總投入              `pf_ledger` —— ~~（全站唯一「使用者打金額進去就落在持倉清單」的地方）~~
                          → **2026-09-08 第五輪更正（N2）**：見下方「兩句過強宣稱」註
    ===================== ==========================================================

    ⚠️ **💰 那一格與 :func:`_render_mix` 必須永遠是同一個答案**（同一個缺失欄位
    `invest_twd`）。在此之前一個指 `pf_add`、一個指 `pf_policy_admin` ——
    **同一畫面兩句相反的話**，由第二輪稽核擋下。兩處現已一起指 `pf_ledger`。

    ## ⛔ 兩句過強宣稱，2026-09-08 第五輪就地更正（有意識的更正，不是漏刪）

    **N2 —— 同一句話，第三輪只改了一處。**
    「全站唯一『使用者打一個金額進去、它就落在持倉清單上』」這句在
    :func:`_status_tiles` 的就地註解裡**已於第三輪劃線收窄**（T7 的 A/B/C 落帳經
    `_sync_invest_twd_from_ledgers` 同樣會寫），**但上面那張四格表裡的同一句原封未動**。
    ⚠️ **這正是第三輪 R1 自己記下來的形狀**：「修了那一支、漏了隔壁這一支」——
    **第三輪把它寫成教訓，第四輪又犯了一次。**
    → **現行讀法**：`pf_ledger` 是**指路的正確答案**（那些寫入點都住在它底下），
    但**不是**「全站唯一」。

    **N3 —— 「目前全站唯一按得到載入的地方（`pf_load`）」不成立。**
    **實測（AST，依外層容器歸屬，不是靠最近的抬頭）**：全站三顆載入鈕分屬**兩個區段** ——
    `btn_pf_load_all_top` 與 `btn_load_ungrouped` 在 `_ov_group`
    （`#### 🗂️ 保單分組視圖`）；**`btn_pf_load_all` 在 `_sec_add`
    （`### ➕ 加入與管理基金`）**。
    ⚠️ **不能用「最近的 `###` 抬頭」去歸屬** —— 舊 ④ 先宣告容器、之後才回頭填，
    **原始碼順序 ≠ 渲染順序**（本組第一版就是這樣算的，把 `_ov_group` 那兩顆
    歸到了「🔬 持股重疊度診斷」底下）。
    → **指路本身不變**（`pf_load` 在該狀態下確實按得到，見 :func:`invested_hint`
    第二支的依據），**變的是「唯一」這個字**。
    """
