...
_VERDICT_UNCHECKED: str = (
    "**還不知道 —— 這一頁一次都還沒去查。** "
    "底下每一塊各管一件事，都要你按一下才會去查；"
    "按之前這一格說不出「能」或「不能」——**不知道不等於沒問題。**")
...
_VERDICT_CHECKED: str = (
    "**資料來源已經查過了 —— 逐源結果在底下「{block}」那一塊。** "
    "這一格**不**替它壓成一句話：壓了就會變成第二份跟底下不同步的真相，"
    "而使用者會信排在前面的那一句。")

#: 結論層三張卡。**每一張回答「這個問題的答案住在哪一塊、現在有沒有答案」**，
#: **不是**「答案是什麼」。
#:
#: ⭐ **為什麼這是有內容的、不是三張空卡（本批新增的唯一一件使用者看得到的事）**：
...
_CARD_KEYS_TITLE: str = "🔑 連線與金鑰"
_CARD_KEYS_NOTE: str = (
    "回答：上面那兩件事**能不能做**。金鑰或授權缺一項，"
    "上面兩塊查出來的就會是「連不上」而不是「資料有問題」——"
    "**兩者長得很像，先看這一塊再看上面。**")
...
def _conclusion_cards() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"title": _CARD_KEYS_TITLE, "note": _CARD_KEYS_NOTE,
         "state": STATE_NOT_READY, "where": _below(BLOCK_KEYS)},
    ]
...
def _render_conclusion() -> None:
    ...
    if _diag_gate_is_on():
        not_ready(_VERDICT_CHECKED.format(block=BLOCK_HEALTH),
                  where=_below(BLOCK_HEALTH))
    else:
        not_ready(_VERDICT_UNCHECKED, where=_below(DIAG_GATE_LABEL))
