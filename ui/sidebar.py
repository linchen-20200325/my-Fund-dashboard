    ...
    with st.sidebar:
        ...
        if st.button(
            ...
            try:
                ...
                # st.rerun()」——全 ui/ + app.py 掃到 **3 處**（量測日 2026-08-28）：
                # 本行、tab1_macro_longterm.py 的快取清除、tab_manage.py 的雲端同步;
                # 後兩者**已改色但同樣看不見**,已就地登記待後批。
                # ⚠️ 這個「只有 3 處」是**單組 AST 掃描的窮舉句,沒有第二組驗過**
                #    (§-2 規則 6)。掃描條件刻意保守（只認「緊接的下一個 statement」）,
                #    所以它會**漏掉**中間夾了其他語句的情形 —— 是下界不是全部。
