...
#    ~~不可能漂移~~ 少一個出處** —— 目錄由本表 for-loop 產生，標題由 `_chapter()` 讀本表產生。
#    守衛 `tests/test_manual_anchor_toc.py` 另從**行為面**驗兩邊真的對得上。
#
#    ⚠️ **2026-08-31 就地更正：~~「不可能漂移」~~ 是說過頭了，已被突變證偽**
#    （**有意識的更正，不是漏刪** · 決策者：**AI 總管** · 依據：**實測突變**）。
#    **舊表述的理由仍然成立**（一張表確實把「兩個出處」收成一個，這個好處是真的）；
#    **被權衡掉的是它的絕對語氣**：本表的**第 2 欄（目錄短標）與第 3 欄（章節標題）
#    本來就是兩套不同的字**，同一列改一欄不改另一欄，結構上**完全做得到**。
#    **實測**：只改 `weather` 那列的第 3 欄、第 2 欄不動 → 目錄與章名當場對不起來，
#    而 `tests/test_manual_anchor_toc.py` + `tests/test_tab6_manual.py` **全綠**。
#    → **真正在結構上不可能漂移的是 `anchor` / `key`**（同一欄導出，
#    `_anchor()` 對未知 key 直接 `KeyError`）；**人看的標籤沒有這個保證。**
#    改章名時**兩欄都要看一遍** —— 這件事目前**沒有任何守衛在擋**（刻意，屬另一個 scope）。
#
# 欄位：(key, 目錄短標, 章節標題)
...
⚠️ 這張表**現在只是把偏離程度分成三級，不再對應任何處置動作**。
原本第二欄的處置欄與其下的行動指引已依母法（不產生直接買賣建議）移除；
分級線本身是客觀計算，故保留。**要不要調整部位、怎麼調整，由你自己判斷。**
""")
...
""")
    ...
    #     是錯的，已一併更正為 ~~8~~ **7**（該處說明是下一批的交接依據，寫錯會讓下一批
    #     建立在假前提上）。⚠️ 8→7 為**有意識的更正，不是漏刪**（2026-08-31 · AI 總管）：
    #     該 docstring 原列的 8 處誤判中，`app.py` 那一筆已隨本 PR 移出 live string、
    #     命中不再存在，理由見該 docstring 的更正註。
    #
...
#### B. {_section_label('fund')} 的「四分位」— Sharpe vs 同類

{_where_to_find('fund')} 對單一基金另有一組四分位燈號，比的是**風險調整後報酬（Sharpe）**，
不是報酬本身：
...
| 🗂️ **`nav_history`** | 固定名稱、**無底線** | **逐日累積的歷史淨值**（主鍵 = 代碼 + 日期）。長期報酬 / 3Y / 5Y / 低基期全靠它。**手動編輯只需填 `code｜date｜nav` 三欄**（日期 `2020/1/2` 或 `2020-01-02` 都吃）；後面 `fund_name｜source｜recorded_at` 留空即可，**系統會自動補**、也容忍空白 | 每日自動累積 + Tab5「NAV 歷史匯入」+ 📋 管理室 CSV 匯入 | 🚨 **絕對不要刪** |
