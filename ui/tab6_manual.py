...
def render_manual_tab() -> None:
    ...
    # v19.435:「曾經查過的基金標的清單」整段已搬到 📋 我的管理室 → 選股池(避免同 st.tabs run 兩處
    # 都渲染 _fh_* widget → DuplicateWidgetID)。此處只留指路。
    st.markdown("### 📋 曾經查過的基金標的清單")
    st.info("此清單已搬到「📋 我的管理室」分頁 → 📁 選股池 區:可新增 / CSV 還原 / 下載 / 清空 / "
            "升等預設,並一鍵「匯入 / 合併進選股池」。")
