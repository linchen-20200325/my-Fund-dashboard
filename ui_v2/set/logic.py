...
GAPS = {
    "SET-GAP-取數失敗圖示": (
        "客戶 2026-09-24 裁示取數失敗改用 ⛔、黃燈保留 ⚠；44 第五節第五小節系統錯誤模板現行字面仍是「⚠ 取數失敗」，"
        "這一裁示要不要改 44 與既有頁尚未定。本頁把取數失敗的圖示集中成 FETCH_FAIL_GLYPH 一個常數，"
        "目前照 44 現行字面（⚠），待 44 同步後只改那一個常數。"
    ),
    "SET-GAP-取數失敗": (
    ...
    "SET-GAP-燈圖示共用": (
        "在 FETCH_FAIL_GLYPH 還是 ⚠ 的這段期間，紅燈（取數失敗）與黃燈（有逾期）同用 ⚠；"
        "顏色與狀態字不同，圖示相同。常數改成 ⛔ 之後自然分開（見 SET-GAP-取數失敗圖示）。"
    ),
    "SET-GAP-某類無列不入燈": (
...
FETCH_FAIL_GLYPH = "⚠"
# 存檔寫入失敗框的圖示（客戶 2026-09-24 裁示「失敗框補圖示」，拍板原型用 ⛔）。與上一個常數是兩件事。
SAVE_FAIL_GLYPH = "⛔"
# 黃燈的圖示（客戶裁示「黃燈保留 ⚠」）。
WARN_GLYPH = "⚠"
EMPTY_GLYPH = "⬜"
