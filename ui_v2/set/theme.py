...
**這三個一個字都不動**；其餘的色與姊妹頁 `ui_v2/alo/theme.py` 同值（同一套 token），
本檔**不 import** 它（各頁自足，見 `__init__`），而是逐值抄一份、在本檔自己的 `CONTRAST_PAIRS` 裡重算。

客戶 2026-09-22【基金頁設計引導】：一般文字 ≥ 4.5；大文字 ≥ 3；非文字 UI 元件 ≥ 3。
`44` 第五節共通硬要求：狀態不靠顏色單獨辨識 —— 本頁每一個狀態都另有圖示與文字（見 `logic`）。

⚠️ 登記：`44` 第五節卡片表只宣告四狀態的**顏色語意**（中性／灰／黃／紅），**沒有宣告任何色碼**。
下面的色碼屬畫法不屬規格；判定住在 `logic`，回傳的是語意字串不是色碼。

本頁四種顏色語意都會用到：`SET-0` 規則欄逐字給了灰、黃、紅三色；`SET-2` 的 `failed` 列與
`44` 5.5「系統錯誤」模板用紅；存檔寫入失敗的框用灰（客戶 2026-09-24 裁「失敗框改灰」，拍板原型同畫法）。

⚠️ **前提（SET-GAP-深色主題前提）**：下面每一組對比、以及畫面層那條瀏覽器實測，
...
# ── 狀態顏色語意 → 色碼（語意見 `44` 5.1 卡片表） ──
STATE_GRAY = "#8b949e"
STATE_GRAY_BG = "#11161d"  # 灰態容器底（存檔寫入失敗的框；與 INPUT_BG 同值，不帶新色碼）
STATE_WARN = "#e3b341"
STATE_ERR = "#ff4b4b"      # 與姊妹頁 hld／exp／alo 同一個紅（44 5.5 系統錯誤）
...
def tone_hex(tone: str) -> str:
    """把 logic 回傳的顏色語意換成色碼。查不到就**炸**，不退回中性色（§1 Fail Loud）。"""
...
#    與 `ui_v2/alo/theme.py` 同一個判斷（線框那一組的分類，客戶未逐字裁，可推翻）。
