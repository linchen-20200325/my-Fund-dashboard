...
GAPS = {
    "ALO-GAP-線框導覽鈕": (
        ...
        "44 現行規則欄（2026-09-24 第二十一輪客戶裁示）是持倉表空或不空都出現，線框缺的是持倉表為空的那一種，"
        "44 已登記線框待重繪。本頁照 44。"
    ),
    "ALO-GAP-取數失敗": (
    ...
    "ALO-GAP-失敗時導覽鈕": (
        "holding 取數失敗時持倉表是空還是有資料不知道，44 沒寫這一種。本頁判讀：44 現行規則欄空與不空兩種都掛，"
        "不論是哪一種都掛，所以 ALO-4 照掛導覽鈕；但不把它當成空 —— 燈不說尚未建立任何持倉、ALO-4 摘要不寫缺 holding。"
    ),
    "ALO-GAP-燈狀態字": (
        "取數失敗時燈維持灰（44 燈沒有紅），但狀態字不寫「中性」——那會與失敗的文案打架。"
        "本頁改印「狀態：取數失敗」這句事實描述，圖示用 ⛔（與 44 系統錯誤模板同一個圖示，FETCH_FAIL_GLYPH）；這一句是本頁擬的，44 沒寫。"
    ),
    "ALO-GAP-設定取數失敗": (
    ...
    "ALO-GAP-燈圖示共用": (
        "已裁結案（2026-09-24，決策者：客戶）：取數失敗改用 ⛔（44 第五節第五小節系統錯誤模板已同步），黃燈仍用 ⚠，"
        "兩者不再共用。舊登記「⚠ 同時用於黃燈與取數失敗、待客戶裁」寫下時為真。"
    ),
    "ALO-GAP-存檔失敗圖示": (
        "44 對存檔寫入失敗只寫「卡上顯示失敗訊息原文」，沒寫失敗框帶什麼圖示。"
        "客戶 2026-09-25 裁示：存檔寫入失敗的框用 ⛔（與取數失敗同一個圖示，SAVE_FAIL_GLYPH ＝ FETCH_FAIL_GLYPH），⚠ 只留給黃燈。"
        "舊畫法（失敗框寫死 ⚠）寫下時是本頁自擬，已依裁示改掉。"
    ),
    "ALO-GAP-保單失敗從嚴": (
    ...
    "ALO-GAP-導覽改指派": (
        "已裁結案（2026-09-24，決策者：客戶）：44 規則欄把那一枚的用途改為去 Sheets 維護持倉與指派；"
        "同日第二十一輪再裁出現條件為持倉表空或不空都出現（有持倉＝改的場景因此有入口）。"
...
# ⛔ 本頁畫「取數失敗」用的圖示只住在這一個常數裡（客戶 2026-09-24 裁示：取數失敗 ⛔、黃燈 ⚠）。
# 44 逐字「`⛔ 取數失敗：<訊息原文>`」
FETCH_FAIL_GLYPH = "⛔"
# 「存檔寫入失敗」框的圖示（客戶 2026-09-25 裁示：存檔寫入失敗 ⚠ → ⛔；⚠ 只留給黃燈）。
# 刻意綁在 FETCH_FAIL_GLYPH 上、不另寫字面：兩種失敗框用同一個圖示，改一處兩處跟著走。
# ALO-GAP-存檔失敗圖示：44 只寫「卡上顯示失敗訊息原文」，沒寫圖示；本頁照客戶裁示用 ⛔。
SAVE_FAIL_GLYPH = FETCH_FAIL_GLYPH
# 黃燈的圖示（客戶裁示黃燈仍用 ⚠）。
WARN_GLYPH = "⚠"
# ALO-GAP-燈圖示共用：已裁結案，取數失敗 ⛔、黃燈 ⚠，不再共用。
_LAMP_LOOK = {
    "灰": ("⬜", "狀態：中性"),
    "黃": (WARN_GLYPH, "狀態：要多看一眼"),
}
# ALO-GAP-燈狀態字：取數失敗時燈仍灰，但不寫「中性」。
_LAMP_LOOK_FAILED = (FETCH_FAIL_GLYPH, "狀態：取數失敗")
...
def fetch_failed_text(message: str) -> str:
    ...
    return f"{FETCH_FAIL_GLYPH} 取數失敗：" + message
...
def _build_alo4(dataset) -> dict:
    ...
    # 44 逐字「持倉表為空或不為空，本塊都掛一枚「前往 Sheets 維護持倉」按鈕」
    # ALO-GAP-失敗時導覽鈕：holding 取數失敗時是空是滿不知道；兩種都掛，所以照掛，但不把它當成空。
    # ALO-GAP-線框導覽鈕：線框缺持倉表為空的那一種，照 44。ALO-GAP-導覽目的地：不接連結。
    # ALO-GAP-導覽改指派：已裁結案（用途改為去 Sheets 維護持倉與指派；出現條件第二十一輪改為空或不空都出現）。
