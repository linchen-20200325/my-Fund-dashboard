...
GAPS = {
    ...
    "ALO-GAP-導覽改指派": (
        "已裁結案（2026-09-24，決策者：客戶）：44 規則欄把那一枚的用途改為去 Sheets 維護持倉與指派，出現條件不變。"
        "舊登記「出現時沒有指派可改」寫下時為真；本頁畫面不因此改變（按鈕標籤與下方說明都不變）。"
    ),
...
def _build_alo4(dataset) -> dict:
    ...
    if holdings_empty:
        ...
        # ALO-GAP-導覽改指派：已裁結案（2026-09-24 用途改為去 Sheets 維護持倉與指派），畫面不變。
