...
| **P2-5** | `repositories/macro_repository.py` 1078 LOC 拆 5 子檔(`_helpers` / `fred` / `yf` / `alternate`(Defillama+AAII+ISM+compass+China)/ `math_utils`),主檔留 15 LOC shim(−98.6%) | `6a75636` | ✅ 完工 | import smoke verified(fetch_fred / fetch_yf_close / zscore 解析到子模組)|
