...
- **P1-1 leftover 2 處**:`services/portfolio_service.py:612 _safe_num_ps`(_ps 後綴變體)/ `ui/tab2_single_fund.py:836 _fmt_pct`(inline closure,non-ratio 行為不同)
- **C2 8 個 MISS fetcher**(同 §6 剩餘 backlog):scalar / list / fallback chain / tuple,complexity-justified
- **C3 第二輪 sidebar 抽取**(同 §6):over-engineering risk,§-1 工作準則「沒實際 bug 不動」
- **Pre-existing fail 3 群**:test_app_apptest 2 個(OAuth refresh / 240s timeout) + test_financial_repository(yfinance 環境)
