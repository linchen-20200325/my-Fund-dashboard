...
- **目前版本**：v18.260p4_StrategyGrid（危機回測室 + 4 策略 × 3 門檻 grid_search + heatmap，Phase 4）
  - **#113**：上接 #112。新增 `services/crisis_strategy_grid.py`（~220 行）+ 25 單元測試 + 擴充 `ui/tab_crisis_backtest.py` 加「🧪 策略網格搜尋」section。引擎核心：`StrategySpec` frozen dataclass + `DEFAULT_STRATEGIES`（buy_and_hold/signal_exit/signal_half/buy_dip）+ `run_strategy`（t-1 訊號決定 t 倉位避免前視偏誤、計算 final_value/total_return/max_drawdown/sharpe/crisis_return_pct）+ `grid_search`（4×3 cell）+ `results_to_dataframe` + `build_heatmap_data`（pivot 成策略×門檻矩陣）+ `rank_results`（top-N）。UI 3 inputs（訊號 selectbox、metric selectbox、門檻文字框）+「跑網格」按鈕 + plotly heatmap（RdYlGn 色階）+ Top-5 排行 DataFrame + 完整 12-cell expander。累計測試 64 passed（Phase 1: 20 + Phase 3: 19 + Phase 4: 25）。
...
- **前版**：v18.260p3_MacroSignalLookback（危機回測室 + 總經訊號預測力驗證，Phase 3）
- **前二版**：v18.259_InvestCalcTWD（投資試算加 TWD 即時匯率換算）
