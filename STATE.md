...
## ⚠️ 2026-09-02 斷層登記:本檔自 2026-08-12 起漏記 132 批,本次補五筆條目 + 結構性變更索引

**先講清楚本檔的現況,免得後人把它當成完整的**:下一節(2026-08-12・PR #625)以下的內容
**一路正確、未被本次改動**;但從那之後到 2026-09-02 之間的東西,本檔**幾乎全部沒記**。

- **斷層起點**:PR **#625** 是最後一筆有專屬存檔的批次;寫入那筆記錄的是 **#626**
  (`docs(state): 存檔 v19.433`)。**#627 以後全部未記。**
- **斷層規模(量測日 2026-09-02)**:GitHub API 列出 `state=closed` + `base=main` 的
  **#625~#759 連號 135 個,無缺號**;其中只有 **#754 沒有 `merged_at`**(它被 #757 取代)。
  → **未記錄的已合併 PR ＝ #627~#759,共 132 個。**
- ⚠️ **本檔曾在 2026-08-31 被動過,但那不是補記**:`8eb13b8` 確實修改了 STATE.md,
  改的是**檔案深處兩則舊條目的就地註記**(T3a 死碼結論、crisis_backtest 共用者由三條減到一條),
  **沒有新增任何頂層條目**。所以「最後一筆條目停在 2026-08-12」與「檔案三週沒被 touch」**是兩回事**。

**本次補記的範圍(刻意不補滿 132 批)**:只補 **2026-09-01/02 的五批完整條目**(見下五節)
＋ 本節這份**結構性變更索引**。理由有二:(a) 本檔自陳是「極簡熱資料檔」,132 筆逐條展開會讓它
不再是熱資料。**未展開的部分列在下方索引,不是已補記。**
⚠️ **本節初稿還寫過第二個理由「快撞到 `check-added-large-files` 上限」,該理由不成立,已撤**
(**有意識的更正,不是漏刪**;2026-09-02 稽核指出,本組讀 hook 源碼確認):
`check_added_large_files.py` 是 `if not enforce_all: filenames_filtered &= added_files()`,
而 `added_files()` ＝ `git diff --staged --name-only --diff-filter=A` ——
**`STATE.md` 是已追蹤檔的修改(M)不是新增(A),這個 hook 結構上永遠不會檢查它**;
CI 與 `.pre-commit-config.yaml` 也都沒有帶 `--enforce-all`(實測 0 命中)。
**四個數字本身沒錯**(補記前 435,132 / 補記後 452,255 / 上限 512,000 ＝ `--maxkb=500`×1024 / 約 88%,
量測日 2026-09-02),**錯的是它的約束力** —— 那不是一道會擋下本檔的閘門。
**「不逐條展開」這個決定不受影響**,它現在只靠「極簡熱資料檔」這一個理由撐著。

### 結構性變更索引(讀 repo 前必看的幾件事;**非窮舉**)

> 下列 SHA 皆取自 `git log --first-parent origin/main`。標 ⚠️ 者其 commit subject **不含 PR 編號**。
> ✅ **2026-09-02 更新:這幾筆的 PR 編號已由「以標題比對」升級為以 GitHub API 的 `merge_commit_sha` 逐一核對**
> (本組實測 `8eb13b8`→#743、`f98df07`→#740、`21c7615`→#739、`5f798ee`→#738,**4/4 全對**;
> 稽核另以同法核對本索引 32 組 PR↔SHA,**MISMATCHES = NONE**)。
> **升級的是查證方法,不是結論** —— 原本的標題比對碰巧全對,但它取決於「標題有沒有撞名」,`merge_commit_sha` 不會。
> ⚠️ **`5f798ee`(＝ #738 輪動配對兩元件:② 持倉互斥避險 + ③ 互補配對探索,客戶 2026-08-31 拍板 Q1~Q4)
> 本身沒有被寫進下方索引** —— 它落在「未展開的 132 批」裡,**這裡只登記它的存在,不是已補記。**

- **七分頁 → 五分頁資訊架構(IA)重整** —— 這是本斷層期最大的變化,**分頁名稱與職責全變了**:
  - 現行五分頁(SSOT `ui/helpers/story_nav.py`,2026-09-02 實測):
    `macro` 🌐 市場總覽 / `health` 💊 持倉體檢 / `research` 🔍 標的探索 /
    `portfolio` 📊 資產配置 / `settings` ⚙️ 設定與診斷。
  - #744 七→五接線(`cc37709`)、#735 個基深掘+批次分析併為「基金研究」(`041eecc`)、
    #736 「我的配置」照線框重排(`ce7819a`)、⚠️#739 「設定與診斷」合併頁(`21c7615`)、
    ⚠️#740 健診改單行連結(`f98df07`)、#758 分頁改名 + IA kit(`3e1eaad`)、
    #759 換股顧問搬到「資產配置」(`e38805c`)。
  - 客戶拍板線框已進版控:`docs/wireframes/`(#728 `592b1fa`、#748 `12d5856`;
    2026-09-02 實測 8 份線框 + README,其中 `ia-wireframe.html` 是五分頁的依據)。
- **死碼實體刪除三批**:#719 刪 10 個 0-caller 檔(`4ae3c43`)、#730 Phase 1.4 刪 11 個
  0-caller 模組(`149d1dd`)、⚠️#743 auto_search 封閉死簇整簇刪除(`8eb13b8`)。
  → **若照舊條目去找檔案而找不到,先查這三批,那多半不是壞掉而是被刪了。**
- **把「宣稱」變成「機器可查證」的守衛**:#729 UI 三鐵律(`b00eabf`)、
  #733 L2 純度(`1acd089`)、#726 6 個純函式 UI 元件 + 41 條突變驗證(`d6f5f87`)。
- **快取／退避誠實性(§1「只快取成功結果」)**:#741 失敗不入 `_ttl_cache`(`3aceccb`)、
  #757 熱錢 fetcher(見下)、#756 gspread 冷卻(見下)。
- **資料真實性**:#750 換源前先比幣別(`9312ca8`)、#755 幣別四格表(見下)、
  #751 拔除 2016-08 停更的 FRED NAPM／ISPMANPMI(`e634703`)、#717 年高/年低視窗不足據實標註(`c9abaab`)。
- **三態/五態顏色分離**:#720(`461f811`)、#721(`1a66ad7`)、#745(`9d5edda`)。
- **治理文件**:憲法 §-2(#713/#714)、§-1.5 虛擬軟體公司運行規範(#716 `cde969a`)、
  §-2.A 執行條款(#749 `1b077ef`)、憲法事實更正(#724 `68967d3`、#752 `b5b0464`);
  新增 `TODO.md` 三階段追蹤(#727 `a81e460`、#731 `2036896`、#734 `9e89427`)。
- **未展開的功能線(只留線索,本組未逐批查證)**:LINE 推播與配息月曆(約 #630~#636、#703~#711)、
  NAV 自動補齊與 ISIN→secId(約 #661~#666、#676~#677、#683、#685)、
  保單跨幣別扣款決策(約 #699~#702)。

### 這份索引的掃法與它**掃不到**的東西(依 §-2 規則 6,不宣稱窮舉)

- **掃法**:`git log --first-parent origin/main`(取 SHA 與 subject)
  ＋ GitHub `list_pull_requests(state=closed, base=main)` 兩頁併集(取編號與 `merged_at`)。
- ⛔ **掃不到 (a)**:本 worktree 是 **shallow clone** —— `git log` 只回溯到 **#695(2026-08-21)**。
  **#627~#694 無法用 git 核對 diff 或 SHA**,本組對那一段只有 API 給的**標題**。
- ⛔ **掃不到 (b)**:`base` 不是 `main` 的 PR、以及**未經 PR 直接推上 main 的 commit**。
  ⚠️ **2026-09-02 補:本組原本只「揭露」這個類別,沒有去查它空不空 —— 稽核指出後實測,它不是空的。**
  以 main 的 first-parent commit 比對全部 PR 的 `merge_commit_sha`,**窗內有 5 筆直推**
  (五顆皆實測不屬於任何 PR 的 `merge_commit_sha`):
  `96bd538` / `10b6a18` / `76d9fca`(三顆 cron bot「每週全球總經歷史增量 `[skip ci]`」)、
  **`a34cc51`**(補歷史淨值改用代號挑基金)、**`10466cd`**(Layer 1-4 稽核修正:假成功／捏造值／跨畫面矛盾)。
  ⚠️ **後兩顆是實質 code 變更**,而 `10466cd` 的主題正是本檔該記的東西 —— **它們不在下方索引裡。**
  → **「132」作為 PR 計數仍然正確,但它不是斷層總量**;總量至少要再加這 5 筆。
  ✅ 另一半是好消息:**`base` 不是 `main` 的 PR 在 #627~#759 區間內實測為空**(0 筆)。
  **教訓:揭露一個類別 ≠ 查證它 —— 只寫「這裡我掃不到」而不去看它空不空,等於把缺口留成裝飾。**
- ⛔ **掃不到 (c)**:**標題不等於事實**,而本組自己的查證深度也**不是一致的**,據實分三級:
  寫「見下」的五批 —— **讀完整 commit message + file-level diffstat,並抽查核對若干具體宣稱**
  (SHA↔PR 對應、合併日、分頁名 SSOT、巢狀 `st.tabs` 是否還在),**但未逐行讀完整 diff**;
  結構性變更索引各批 —— **只讀 commit subject 與 PR 標題**;
  「未展開的功能線」那三行 —— **只按編號區間歸類,連 subject 都沒有逐筆讀**。
  本 repo 有現成的反例可以對照:#747 的標題寫「說明書那一層巢狀 `st.tabs` 消失」,
  **那個範圍限定是真的、括號裡也誠實寫了「全站仍有一處會渲染」**,而 2026-09-02 實測
  `ui/tab3_t7_ledger.py::render_t7_section` 內的 A/B/C 巢狀 `st.tabs` **確實還活著** ——
  也就是說**該標題沒有說謊,但只讀標題前半句的人會以為巢狀分頁已經全站絕跡**。
- **本節由資料工程組單組產出,未經第二組獨立複驗。**

---

## ✅ 2026-09-02 存檔:淨值幣別守衛改「四格表驅動」—— 關掉一整類,不再逐條補(PR #755・e221dbc)

**採用點未接**;`nav_history` 加開幣別欄並確保寫入端帶上,只封堵污染源。
⚠️ **本節初稿寫「production 一個字都沒動」,那句在批次層級為假,已更正**
(**有意識的更正,不是漏刪**;2026-09-02 稽核指出,本組實測確認):
`git diff --numstat e221dbc^ e221dbc -- services/ shared/ ui/` → **5 檔 +198/−15**,
其中 **`ui/helpers/nav_history_hook.py` 在寫入路徑開始帶 `currency`**,是**真行為變更**。
**根因是射程被放大**:那句話在 #755 的描述裡是**只涵蓋「守衛深度」那兩輪**的 round-scoped 說法,
本組把它讀成了整批 batch-scoped。**#755 標題自己寫的「先不接採用點」才是正確的表述。**

- **不變式一句話**:幣別欄 **== 最後被採用那條序列自己宣告的幣別**;它不宣告就是空字串。
  **「不知道」永遠不可以被上一個來源的宣告頂替**(§1:空是誠實的未知)。
- **四格表(換源次數 × 最後採用者有無宣告)**:單次/連續換源 × 有宣告/不宣告。
  **右下那格(連續換源、最後採用者不宣告)原本零驅動**,而它是 production 的常態 ——
  全 repo 只有晨星 / Yahoo / FundClear 會宣告幣別,**MoneyDJ 主線不宣告**。
- **三條舊守衛被四格表取代,語意一條沒少**(斷言全部併進統一斷言);收集數 43 → 44
  (移除 3 條、新增 4 格,淨 +1),**沒有跳過、停用或隔離任何測試**。
- **突變驗證**:`_cand_ccy or cur_ccy` → 2 格轉紅;
  `_cand_ccy or (cur_ccy if src.endswith("(ISIN)") else "")` → 正是本輪新補那一格轉紅。
- 📌 **值得記的判斷**:總管已立**停止規則** —— 對任何有限測試集永遠存在存活突變,
  「找得到就發回」會無限遞迴。**發回門檻是 (a) 突變寫法慣用 且 (b) 命中一整類沒被驅動的子情境**;
  本批是**只命中 (b)** 但仍發回,理由是「關掉整類比補單條便宜」。

## ✅ 2026-09-02 存檔:換股顧問自 ② 持倉體檢搬到 ④ 資產配置(線框第二批・PR #759・e38805c)

依客戶拍板線框 `docs/wireframes/ia-wireframe.html`:Tab 02「這裡不放什麼」逐字寫著
「換股建議與再平衡試算 → 04(那是決策,不是診斷)」。**② 全篇只診斷、不建議。**

- **④ `ui/tab3_portfolio.py`** 新增 `_sec_switch` slot(位置在重疊診斷與帳本之間);
  **資料重用、不重抓** —— 吃 ④ 本來就在算的 `_funds_extra`(同一份已餵給輪動配對/組合績效/效率前緣)。
- **② `ui/tab_fund_grp_health.py`** 改成一行灰色指路,分區名走 `story_nav.where_to_find('switch')`
  SSOT,**不手抄分頁名**;`story_nav` 新增 `switch` / `pf_add` 兩個分區 key。
- **④ 內不開巢狀分頁**:分區靠 `st.subheader(..., anchor=...)` + 錨點。區塊名依線框由
  「換股池顧問」改為「換股顧問」。
- **刻意的行為變更(不是零行為變更)**:空持倉分支由 `st.info`(藍)改為空狀態三要素(灰)
  —— 藍色不在三態語意內;組合績效快照 `_maybe_snapshot` 的觸發頁面由 ② 改為 ④(repo 端仍依日期去重)。
- **檔案刻意不搬**:`switch_advisor_section.py` 留在 `ui/helpers/fund_grp_health/` ——
  ⑤ 的選股池 CRUD 另有 4 處直接 import 它,搬檔屬純 cosmetic 大重構。
- ⚠️ **本批自己更正了自己寫的一句假宣稱**:原登記「兩組績效 KPI 三個標籤字串相同但**演算法不同**」
  —— **實測推翻**。`n_days ≥ 60` 時年化報酬/年化波動 σ/最大回撤**三個數字完全相同**(含 round 與格式化);
  根因是兩條路徑都收口到 `portfolio_returns()` → `metrics_from_return_series()` 同一個 SSOT。
  「三個標籤字面相同」那半仍為真(以 `cat -A` 逐位元組核對)。

## ✅ 2026-09-02 存檔:五分頁 IA 第一批 —— 共用元件 + 分頁改名(PR #758・3e1eaad)

依 2026-09-01 客戶拍板線框落地五分頁資訊架構的第一批。**本批不搬任何功能。**

- **新增 `ui/helpers/ia/`**:`layout.py`(鐵則 01:`GRID_COLS=3` 具名常數、`card_row`/`card_grid`
  自動分列、`wide_table` 空資料強制走空狀態,不畫空表格外框)、`gated_form.py`(鐵則 02:
  `applied_form()` 把「一定要有 submit 鈕」「submit 結果要拿去 gate 重運算」收成單一入口)、
  `cards.py`(鐵則 03 的**組合層**:`state_card(state=...)` 把挑顏色從自由心證變成必填參數)、
  `empty_state.py`(鐵則 04:空狀態三要素 —— 標題/缺什麼/去哪補)。
- ⚠️ **鐵則 03 的顏色本身沒有重做**:SSOT 早已在 `ui/helpers/render_state.py` 與 `shared/colors.py`,
  本批**既未新增顏色常數、也未 re-export** 那三個函式(re-export 會讓同一函式有兩條 import path)。
  → PR 標題寫「四大鐵則」,實際是 **01/02/04 + 03 的組合層**,讀標題時請留意。
- **五分頁改名(只改顯示名)**:`macro` 市場定調→**市場總覽** / `health` 組合健診→**持倉體檢** /
  `research` 基金研究→**標的探索** / `portfolio` 我的配置→**資產配置** / `settings` 設定與診斷(未改)。
  **key、順序、站號、每個 slot 對應的 render 函式一律未動**,`app.py` 的 `st.tabs` 一行未改
  (它早就走 `story_nav.tab_label()` SSOT,改名只動 SSOT 一處)。舊名進 `RETIRED_TAB_LABELS`,
  **與七→五那批分開標註** —— 這批沒有任何分頁被裁撤,退役的只有名字。
- **守衛** `tests/test_ia_kit.py`(**39 條** ＝ 32 原有 + 7 條稽核補洞;16 條突變逐條驗證會轉紅,
  但 #758 已註明**那 16 條不是行為封閉集**)。⚠️ 本節初稿寫「32 條」,那是**被 #758 自己取代掉的數字**
  (#758 描述兩處寫 39;本組實測 `pytest --collect-only` = **39 collected**,28 個 `def test_` 經 parametrize 展開)。
  新增站點依既有 fail-closed 契約登記
  (`GRID_EXEMPT_CALL_TOTAL` 87→89、`FORM_SITE_TOTAL` 6→7,兩處都就地寫明登記會讓契約涵蓋面縮小)。
- ⚠️ **舊分頁名的殘量(本節初稿把這一項寫錯了,而且錯得最嚴重,已更正)**:
  初稿寫 ~~「全 repo 另有**約 127 處**註解/docstring 仍寫舊分頁名(**皆非活字串,守衛全綠**)」~~
  —— **那整句是 #758 自己宣告為假的話,本組照抄了它的舊版**
  (**有意識的更正,不是漏刪**;2026-09-02 稽核指出,本組已回讀 #758 描述確認)。
  #758 的描述有一節標題就叫「**⛔ 我上一版說錯的話**」,內文逐字寫:
  「我寫過『那 127 處全是註解與 docstring,不會讓守衛轉紅』。**這句話是假的**,
  而且我拿『**守衛全綠**』當它的證據 —— **那不是證據,那是守衛的盲點**。」
  **#758 的 AST 實測分佈**:**comment 63 ／ docstring 58 ／ LIVE 20**;
  20 處活字串中 7 處在 `tests/`、4 處是 `RETIRED_TAB_LABELS` 表本身,
  **其餘 9 處在 production `ui/`、使用者看得見**(該 9 處 #758 已修)。
  **本組獨立複驗殘量(量測日 2026-09-02,以 AST 分類 Constant 並扣掉 docstring)**:
  現行樹上 **LIVE = 11**(7 在 `tests/`、4 在 `ui/helpers/story_nav.py` 的 `RETIRED_TAB_LABELS`,
  屬設計如此)—— **20 − 9 = 11,與 #758 的數字吻合。**
  ⚠️ **這一筆是本節主題的正面反例,務必記住**:本節整節在講「不要照抄 PR 標題／來源說法當事實」,
  而它自己照抄的這一句,**正好是來源自己撤回的那一句**。
  **照抄的風險不只是「來源可能錯」,而是「來源可能已經自己更正了,而你抄的是更正前的版本」。**
- ⚠️ **另一處未結的登記**:① 的名字有**來源衝突**(派工單 IA 表寫「大盤總經」、客戶逐字確認的線框寫「市場總覽」),
  本批取線框,**係實作組判斷、未經第二組驗證**。

## ✅ 2026-09-01 存檔:gspread 兩站點補跨呼叫失敗冷卻(批次 2・PR #756・649d08d)

客戶 2026-09-01 指示。**根因**:`infra/source_backoff.py` 的冷卻只由 `infra/proxy.py::fetch_url`
在進場處查,而 **gspread 走自己的 `requests.Session`、完全不經過 `fetch_url`** —— 對它而言退避是
「沒接」不是「不能接」。

- **實測基線**(monkeypatch gspread 必失敗、連呼 3 次,數上游往返):
  `pool.resolve_secid` 1 趟但 3 次都靜靜回 `None`(失敗被吞成空池並入快取);
  `nav.coverage_status` 3 趟;只有 `worksheet()` 失敗時 **6 趟且完全沒有 raise**;
  Tab5 `_cached_nh_coverage` 失敗回 `{}` 入快取,**上游恢復後仍回 `{}`、鎖滿 `TTL_5MIN`**。
- **兩把鑰匙,不是一把**:429/配額 → `gspread:quota:<actor>`(Sheets 讀取配額是 per user per project,
  記在單一本會放任同憑證繼續打別本);403/404/5xx/連線 → `gspread:sheet:<actor>:<sheet_id>`
  (本 repo 有兩本試算表,「一本分享了、另一本沒有」是預期狀態,記在憑證層會誤殺健康的那本)。
- **gspread 的 404/407 刻意不沿用 `kind_for_status` 的 0 冷卻** —— 那條豁免服務的是
  「輪月份 slug、舊月份 404 是正常流程」的探測鏈;gspread 沒有探測鏈,照抄會讓設定錯誤被永久連續轟炸。
  改判最短冷卻(unreachable, 60s)。
- **`pool_repository._load_pool_map`** 由「吞成 `{}`」改為照實 raise(**空選股池是合法狀態**,
  兩者同義即 §1「空有兩義」);「不阻斷抓取鏈」那半由新的 `_pool_map_or_empty()` 在**快取之外**承接。
- **`services/nav_history_gs.load_points`** 進場查冷卻、收尾登記成敗;冷卻期內 raise `NavHistoryError`
  (**不回 `[]`** —— 那會與「這檔真的還沒累積」同義)。內層 `worksheet()` 的 except 改為只放行
  `WorksheetNotFound`,API 錯誤往上拋 —— 原寫法把 429 壓成「沒有這個分頁」,冷卻機制永遠學不到那次失敗。
- ⚠️ **稽核擋下的最嚴重一項(N1,相對修改前是迴歸)**:寫入成功**不解除讀取冷卻** →
  使用者按「📥 匯入到 nav_history」成功寫入後,`_clear_nh_caches()` 清得掉快取、**清不掉退避**,
  緊接著的 `coverage_status()` 仍 raise → **紅色「累積內容讀取失敗」出現在一次成功匯入的正下方**,
  最長 15 分鐘(429 則 30 分鐘)。修法:**寫入成功 → `record_gspread_success`**
  (寫入成功同時證明配額沒滿、這本開得起來、而且可寫,比任何讀取探測都強);
  pool 新增 `_after_pool_write()` 取代 4 個寫入路徑原本單獨的 `_clear_pool_cache()`。

## ✅ 2026-09-01 存檔:熱錢 fetcher 失敗路徑 —— 內拋外譯 + dataset 粒度退避(PR #757・c633a0b)

**根因**:`@st.cache_data` 對回傳值快取、對例外不快取;而 `repositories/hot_money_repository.py`
兩支 fetcher 的慣例是「失敗 → 回 `(空 DataFrame, err 字串)`」—— 一次上游瞬斷會把那個空值
**鎖滿整個 TTL**(外資 30 分 / 匯率 10 分),按「強制重抓」也只是把同一份失敗快取再讀一次。
違反憲法 §02「只快取成功結果」與 §2.4「超過 TTL 應重新抓取」。

- **內拋外譯(三層,公開呼叫介面形狀不變)**:`_fetch_*_uncached()` 失敗 raise(未被裝飾,例外不進快取)
  → `@st.cache_data _cached_*()` 只存成功結果 → 公開 `fetch_*()` 接住例外,翻回既有回傳形狀 `(df, err)`。
  ⚠️ **「公開介面一個字不變」嚴格說不成立**:公開 callable 由 `CachedFunc` 變成 function、
  少掉 6 個 streamlit 私有屬性;`_ST_CACHE_REGISTRY` 註冊名由 `fetch_*` 變 `_cached_*`
  (`global_refresh_all` 逐個 iterate 呼叫 `.clear()`、不按名字查,行為等價)。
- **節流不變式**:失敗**只在「確實有節流器」時才 raise**,否則 return 讓 TTL 承擔 ——
  否則會出現「一個以不要轟炸來源為題的改動,反而把請求量放大 3 倍」。
- **退避鍵由 host 縮成 dataset 粒度**:FinMind 的「額度用盡/缺類別欄/回非 JSON/`data:[]`」
  **全是 HTTP 200**(失敗寫在 body),`fetch_url` 一看到 200 就 `_note_success()` 解除冷卻,
  這一層一個節流器都不剩。**host 粒度會誤殺同來源、完全健康的 NDC dataset**
  (實測:host 鍵時 NDC `score=None`、打上游 0 次;改 dataset 鍵後 `score=31`、打上游 1 次)。
- **Tab1「強制重抓」修回真的會重抓(本批自己造成的迴歸)**:新增的 dataset 冷卻不在
  `_TAB1_TTL_CACHE_NAMES` 裡,於是清光快取後下一次呼叫仍被 `should_skip` 擋在進場處。
  實測(402 持續失敗,按鈕後的上游請求數):base 1 → 修復前 **0** → 現在 1。
  新增 `reset_key()` / `clear_source_backoff()`,**只解該 dataset 鍵、不碰 host 鍵**
  (429/5xx/403/逾時那些「上游真的在受苦」的分類一律不動)。
- **撤掉「非交易日」那個假歸因**:查詢視窗恆為 74~379 個日曆日(days 來自 slider(60,365) + 14),
  而台灣最長連假約 9 天 —— **一個 ≥74 天的視窗回傳 `data:[]` 不可能是非交易日**。
  訊息改為誠實歸因(帶出實際視窗天數與實際類別清單)。
- 📌 `c633a0b` 是把原分支 `057a1bd` 的 8 顆 commit **重打成一顆、重寫訊息**;
  **兩者的「變更集」逐位元組相同**(去掉 index 行後 patch sha 相同、8 個被改檔案的 blob hash 8/8 相同),
  逐輪推導與實測過程留在原分支的 commit history。
  ⚠️ **本節初稿寫 ~~「兩者 tree 完全相同、沒有任何檔案內容變更」~~,那是假的,已更正**
  (**有意識的更正,不是漏刪**;2026-09-02 稽核指出,本組實測確認):
  `git rev-parse c633a0b^{tree}` → `0d80daf…`、`057a1bd^{tree}` → `1a795c7…`,**兩者不同**;
  `git diff --stat` 兩者差 12 檔(全是 dividend_calendar)。**原因是兩者的 base 不同**,
  「變更集相同」與「tree 相同」是兩件事,初稿把前者寫成了後者。
  ⚠️ **這正是本 repo 記過的病:寫下一個可被照跑、卻沒真的跑過的指令輸出。**
  下一個人照著 `git rev-parse` 重跑會拿到兩個不同 hash,然後合理懷疑整條記載 ——
  **一個看起來有出處的假宣稱,比一個沒有出處的宣稱更難被推翻。**
...
- **#609 merged**:① 組合績效(portfolio_performance)+ 基期欄 + 4D 評分 SSOT 不一致修正。
- **#610 merged**(squash `e0de443`):換標決策引擎(v19.423)+ 效率前緣診斷(v19.424)+ 景氣位階適配(v19.425)。
- 三張大表(組合/持倉/批次)共用欄組今增:基期 / 策略燈號 / 換標策略分 / 景氣適配 / 適配傾向;
  Tab3 加組合績效 + 效率前緣;三張加 🔀 換標決策區 + 🧭 景氣適配區。
- 誠信護欄:前緣「教學非建議」、景氣「參考傾向非%配置」、換標「資料不足→灰燈」、評分守衛「資料不足以評等」。
- 3 design agent + 6 稽核 agent(換標2/前緣1/景氣1/評分2)findings 全修;全非-slow 2858 + AppTest 21。
...
- **B/C 判 WONTFIX(不同資料,不併)**:戰情室 3 表(核心/波段/3-3-3)為不同 row/sort/styling/
  aux 的**不同資料**(radio 已於 v18.163 收斂該困惑);總經頁 VIX/薩姆/composite 多為互補
  (sparkline vs gauge vs 門檻卡,部分作者已註記 complementary)→ 合併會毀掉區別,違反 user
  「不要合起來」+ §-1,不做(唯一 soft-dup:hero raw-composite vs realtime weighted-composite,
  留 Phase 4 評估)。
- **A 實作(唯一乾淨去重,含真 bug)**:景氣位階 score 為 `calc_macro_phase` 0-10 循環評分
  (恆 ≥ 0),`ui/tab_fund_grp_health.py:107` 誤用 `{:+.1f}` 顯示「+6.5」,與 hero 的 23 指標
  **淨分**(genuinely signed)撞臉。新 SSOT `ui/helpers/macro/helpers.format_phase_score`
  (單一格式 `{phase} {score:.1f}/10`,禁正負號,缺分不捏 0)+ 遷 `tab2:513`(byte-identical)
