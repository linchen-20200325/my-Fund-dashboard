...
- **目前版本**：v18.267_Ship_FxAndMacroValidation（合併 ship：PR #125 Frankfurter FX 第三來源 + PR #124 Tab1 Macro Score 預測力驗證 Phase 3.5）
...
- **前版**：v18.263_FormulaSheetInCalc（Tab2 投資試算加「📐 完整計算公式」expander — 含數字代入 / 4 分支全覆蓋）
...
- **前二版**：v18.262_DataRegistryOsImportFix（補 os import 救活 app）
- **前三版**：v18.261_CrisisTabButtonsFix（危機回測室按鈕 session_state cache）
- **前四版**：v18.260p6_InvestCalcTWDFirst（投資試算改 TWD 為主，內部換原幣算單位/月配息/月配股）
...
- **前五版**：v18.260p5_AIAdvice（危機回測室 + Gemini AI 策略建議，Phase 5）
- **前六版**：v18.260p4_StrategyGrid（危機回測室 + 4 策略 × 3 門檻 grid_search + heatmap，Phase 4）
- **前七版**：v18.260p3_MacroSignalLookback（危機回測室 + 總經訊號預測力驗證，Phase 3）
  - **#109**：v18.258 投資試算加 TWD 換算（非 TWD 基金 `get_latest_fx("{CCY}TWD=X")` → 顯示本金 / 年息 / 月息 TWD；stash 加 `fx_to_twd / amount_twd / *_twd` 進 AI snapshot）。
  - 更早 #108/#107/#106/#105/#104/#103-98 沿革見 git log；STATE.md 極簡保留近 7 版主線（v18.260p3 ~ v18.266）。
