...
- **進度標記**：✅ 代碼淨化與收尾完成（2026-06-02）
  - **#143**：fix — `infra.proxy.fetch_url` 補 429 rate-limit exponential backoff。配合 PR #164（mk_clock 抓不到 CPI/利率 時改顯示警示）追根因：`repositories/macro_repository.py:fetch_fred` 確實有走 NAS proxy（`from infra.proxy import fetch_url`），但 `infra/proxy.py:make_retry_session()` 的 `status_forcelist=[500,502,503,504]` **沒含 429**，外層 fetch_url 迴圈也只對 407/403/ProxyError/Timeout 特判，**429 落入「非 200」的 implicit 分支被靜默放棄**。v18.277 在 `scripts/update_macro_history.py:159` 已加 `_FRED_429_BACKOFF_SEC=(2.0,4.0,8.0)` 重試邏輯，但**未鏡像到核心 fetch_url** → Tab1 即時抓取（user 截圖場景）每次都單發無重試。本 PR 把同套 2/4/8 秒 backoff 搬進 `infra/proxy.py:fetch_url`：新增 `_RATE_LIMIT_BACKOFF_SEC` 模組常數 + `_rl_atmp` 指針；偵測 `r.status_code == 429` → `time.sleep(backoff_seq[_rl_atmp])` → `continue` 重試（最多 3 次，總計最多 14 秒）。耗盡後 print 並 return None；不影響既有 407 / 403 / 5xx / ProxyError / Timeout 路徑。**好處**：全 repo 所有走 NAS proxy 的 endpoint 都受益（FRED `_fred` × 12 series + Yahoo Chart + er-api + Frankfurter + MoneyDJ 等 30+ caller）。新 test_proxy_infra.py +5 case（429→200 一次重試成功 sleep [2.0]、4 連 429 耗盡 sleep [2.0,4.0,8.0] return None、429→429→200 sleep [2.0,4.0]、backoff 常數 == (2.0,4.0,8.0) 鎖死、407 不該觸發 429 sleep）。回歸 test_proxy_infra + test_app_smoke + test_tab1_macro + test_macro_core + test_mk_clock + test_get_latest_fx_fred_fallback **211 passed**。F541 (extraneous f-string) lint warning 為 pre-existing（已在 main baseline）。
