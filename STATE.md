...
- **目前版本**：v18.267_MacroScoreValidation（port PR #124 Phase 3.5 — Tab1 Macro Score 預測力驗證重算歷史 0-10 分對齊崩盤）
  - **#126**：port 自另一 session 的 PR #124（base 在 v18.260p6，會洗掉我 #117-#125 改動所以不直接 merge）。新增 `services/macro_validation.py` + 19 單元測試 + 擴充 `ui/tab_crisis_backtest.py` 加「📊 Tab1 Macro Score 預測力驗證」section（夾在 Phase 3 與 Phase 4 之間）。引擎核心：`SCORE_RULES` 9 指標（PMI/YIELD_10Y2Y/YIELD_10Y3M/HY_SPREAD/M2/FED_BS/VIX/CPI/UNEMPLOYMENT）+ `aggregate_score`（公式同 calc_macro_phase）+ `calc_macro_score_series` + `verify_score_vs_crises` + `compute_period_stats`（含 scipy Welch t-test）。UI 3 sliders + plotly 走勢圖 + crisis vlines + 命中表 + 3 metric 卡。port 過程手動 cherry-pick 2 個新檔 + insert Phase 3.5 section 到最新 main 的 tab_crisis_backtest.py（保留 PR #117 cache 邏輯），1 個 test literal 配合 `years → years_disp` 更新。**18 → 19 passed**（fast lane 122 passed 零回歸）。
  - **#125**：v18.266 Frankfurter (ECB) FX 第三來源 — 解 FRED DEXTWUS 已停發。chain 變 Yahoo → FRED → Frankfurter。免 auth，支援 30+ 對含 TWD。test_get_latest_fx_fred_fallback 9 → 14 passed。
  - **#123**：v18.265 把 st.secrets 讀取與 FX 抓取分開 try — Yahoo 永遠會被嘗試，避免 secrets 異常擋掉 Yahoo。
  - **#122**：v18.264 get_latest_fx 加 FRED 第二來源 fallback — Yahoo / NAS proxy 掛掉時的備援。支援 USD/JPY/EUR/CHF/CNH/CNY → TWD。via_usd / via_usd_inv 兩段式。簽名向後相容（fred_api_key 預設空）。test +9 case。
