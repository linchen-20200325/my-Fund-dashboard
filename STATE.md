...
- **目前版本**：v18.259_InvestCalcTWD（投資試算加 TWD 即時匯率換算）
...
- **前版**：v18.258_InvestCalc（單一基金加投入金額試算 → 單位數 / 配息）
...
- **前二版**：v18.257_NoStStop（移除 st.stop() 救活下方所有 sections）
...
- **前三版**：v18.256_NestedExpanderHotfix（校準卡解說改 checkbox toggle）
...
- **前四版**：v18.255_AISnapshotFullCoverage（校準卡白話三段式 + 9 章節進 AI 總體檢）
...
- **前五版**：v18.254_CalibHealthInAISnapshot（兩校準器去合成模式 + 校準健檢入 AI 白話總體檢）
...
- **前六版**：v18.253_RiskCalibrationReal（3-factor 風險評分校準補上真實 FRED+SPX 模式）
