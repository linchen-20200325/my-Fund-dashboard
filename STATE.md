...
## ⚠️ 2026-09-02 斷層登記:本檔自 2026-08-12 起漏記 132 批,本次只補「結構性變更」
...
不再是熱資料;(b) `check-added-large-files` 上限 512,000 bytes,本檔補記前已 435,132 bytes
(量測日 2026-09-02),逐條展開會直接撞上限。**未展開的部分列在下方索引,不是已補記。**

### 結構性變更索引(讀 repo 前必看的幾件事;**非窮舉**)

> 下列 SHA 皆取自 `git log --first-parent origin/main`。標 ⚠️ 者其 commit subject **不含 PR 編號**,
> 是本組**以標題比對** GitHub API 的 PR 標題推得,**不是 commit 自己宣告的**。
...
**production 一個字都沒動**;本批補的是**守衛深度**。`services/` 三檔與 `shared/data_quality.py`
的改動集中在寫入端契約,`nav_history` 加開幣別欄並確保寫入端帶上(**先不接採用點**,只封堵污染源)。
...
- **守衛** `tests/test_ia_kit.py`(32 條,16 條突變逐條驗證會轉紅);新增站點依既有 fail-closed 契約登記
  (`GRID_EXEMPT_CALL_TOTAL` 87→89、`FORM_SITE_TOTAL` 6→7,兩處都就地寫明登記會讓契約涵蓋面縮小)。
- ⚠️ **兩處未結的登記**:全 repo 另有**約 127 處註解/docstring 仍寫舊分頁名**(皆非活字串,守衛全綠),
  本批刻意不夾帶;① 的名字有**來源衝突**(派工單 IA 表寫「大盤總經」、客戶逐字確認的線框寫「市場總覽」),
...
- 📌 `c633a0b` 的 tree 與原分支 head `057a1bd` **完全相同**(僅把 8 顆 commit 重打成一顆、重寫訊息),
  **沒有任何檔案內容變更**;逐輪推導與實測過程留在原分支的 commit history。
