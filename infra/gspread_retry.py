...
def kind_for_gspread_error(exc: BaseException) -> str:
    ...
    **這條理由在 gspread 這裡同樣不成立，但不成立的原因不一樣**：gspread 走自己的
    `requests.Session`、**不經過 NAS Squid**，所以「罰錯人」的前提（中間有一層我方
    proxy）根本不存在；而 Google 若真的回 407，那就是**對方**在回應這把憑證，
    退避的正是該退避的那個對象。至於「把設定錯誤偽裝成已跳過」那半：**60 秒**短到
    使用者改完 secrets 重整就恢復，且本模組進入退避時一律 stdout log
    （`[source_backoff] … 進入退避`），不會靜默。
