...
def kind_for_gspread_error(exc: BaseException) -> str:
    ...
    `proxy_auth`(407) 是 **0 冷卻**，理由是「404 是 URL 的問題不是來源的問題」——
    那條理由服務的是 `tw_pmi_repository` 那種「輪好幾個月份 slug、舊月份 404 是
    正常流程」的探測鏈。**gspread 沒有任何探測鏈**：這裡的 404 只有一個意思 ——
    「這個 sheet_id 不存在，或這把憑證沒被分享」，**每次 rerun 都會重演**。
    照抄 0 冷卻等於讓設定錯誤永遠被連續轟炸，所以本函式把那兩類改判為最短冷卻
    （`unreachable`，60 秒）—— 短到使用者「去 Sheets 按分享 → 回來重整」就會恢復。
...
def record_gspread_success(actor: str, sheet_id: str) -> None:
    ...
    成功同時證明了兩件事：這把憑證沒有被限流（配額鍵可解）、這本試算表讀得到
    （試算表鍵可解）。
    """
    from infra import source_backoff as _sb
