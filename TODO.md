...
| 1.3 核心計算純化 | ~~⏳ **仍待查證**~~ → ⚠️ **查證已結案，修復未做** | ~~本組的 grep 字表不完整，須獨立一組重掃~~ → **PR #733 → main `1acd089`** 以 AST 全量掃描取代 grep，**查出 45 項現況違規並登記**；該 PR 明文「**刻意不修任何一項**」 |
...
- [x] **Phase 1 總項**~~（所有子項打勾後才可勾此項）~~　→ **2026-08-28 v3：已放行**（含義與保留事項見上表）
  ...
  **1.3 嚴格說並非「子項完成」，而是「子項被重新定義後完成」**，故不適用原括號的字面條件。
  ...
  - [x] **1.3 核心計算純化**　⚠️ **打勾的是「查證」，不是「純化」——先讀完這一格再引用**
  ...
  - [ ] **2.2 UI 元件化（6 個新元件）**
        - **現況**：已有 **6 個新 UI 元件**的工作在進行中，總管交辦時指為分支
          ...
          **本組仍未查證其程式碼內容**（本任務只新增／維護 `TODO.md`，不讀他組工作區）——
          上表所引 PR 標題與合併時機均取自 PR 描述與 API 回傳，**不是**對其實作正確性的背書。
