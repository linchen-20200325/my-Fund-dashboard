...
def test_the_same_fund_across_two_policies_is_counted_once_in_the_conclusion():
    ...
    **稽核實測**：把結論層的 `_uniq_by_code(...)` 拿掉 → **全綠**，
    ...
    本條四半都釘（缺任一半，突變就會活）：
      (a) **計數**：兩筆同 code 只能算一檔；
      (b) **金額**：合計不得因為重複而變成兩倍；
      (c) **端到端**：畫面上的分母講的是去重後的檔數；
      (d) **AST 靜態**：`_render_conclusion()` 必須真的餵去重過的東西。

    ⛔⛔ **「四半」不是「四道獨立防線」—— 這一段是獨立稽核逐半拆開驗出來的，別讀反。**

    - **(d) 單獨是可以被騙的。** 它的判準是 `"_uniq_by_code(" in _src` ——
      **只看符號名在不在**。稽核造了一顆突變（**保留符號名、但實際不去重**），
      **(d) 抓不到**。
    - 抓到那一顆的是 **(c)**：畫面印出「這 2 檔裡…🏆 2 檔」，也就是**同一筆錢加了兩次**。
    - ⇒ **(d) 擋「整條拿掉」，而且沒有 streamlit 也跑得動（本機關卡唯一看得到的一半）；
      (c) 擋「假去重」，但只在 CI 跑得動。兩半互補，缺一不可。**

    ⛔ **因此不要因為「有四條」就過度信任，也不要以為可以刪掉其中任何一條。**
    ⚠️ (a)(b) 是**單元層**的：它們自己就先呼叫了 `_uniq_by_code`，
    所以對「頁面有沒有去重」這件事**沒有防護力** —— 它們守的是 `_peer_verdicts` /
    `_income_tally` 拿到去重後的輸入時算不算得對。**四半守的不是同一件事。**
    """
