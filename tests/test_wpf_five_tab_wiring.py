...
def test_no_live_string_hardcodes_a_tab_name():
    ...
    **同一個突變由形態向那一條抓到**（「參考 / 診斷」＋`分頁` 命中指路形狀）。
    ...
    實測（2026-08-31 收尾批，**必修 1 修完後的現行樹**）：加進去會多出 **11 處**命中，
    拆解為 **3 處真死指路 + 8 處真誤判**。
    誤判那 8 處是：「組合健診大表」是**表名**（`tab_batch_analysis` ×5）、
    「### 💊 基金組合健診」是本頁自己的**區塊標題**、
    `merge_context.py`「未知的『設定與診斷』區塊名稱」是**錯誤訊息**、`app.py` 的 **CSS 字串**。
    把 8 個假豁免塞進豁免表，換來的涵蓋率**形態向已經免費提供**，
    ...
    **有意識的更正，不是漏改**（日期 2026-08-31 · 決策者：AI 總管 · 依據：第三組複驗實測）。
    ...
    漏網的唯一原因就是本節這個去 emoji 盲點。**兩處已於同批修掉**，故現行命中降為 11。
