...
def test_no_live_string_hardcodes_a_tab_name():
    ...
    **同一個突變由形態向那一條抓到**（「參考 / 診斷」＋`分頁` 命中指路形狀）。
    ...
    實測（2026-08-31，現行樹）：加進去會多出 **13 處**命中，其中
    **10 處是真誤判** ——「組合健診大表」是表名、「### 💊 基金組合健診」是本頁自己的
    區塊標題、`merge_context.py` 的「未知的『設定與診斷』區塊名稱」是錯誤訊息、
    CSS 字串等。把 10 個假豁免塞進豁免表，換來的涵蓋率**形態向已經免費提供**，
    而且會讓豁免表本身變得沒人讀得完（豁免表一長，它就退化成白名單）。

    ⚠️ **但那 13 處裡有 3 處是真的、而且兩條守衛目前都抓不到**（誠實登記，交下一批）：
    ...
    **這就是兩條守衛合起來之後仍然存在的盲點，寫在這裡而不是假裝沒有。**
    ...
    | 形態向 | 任何手打的名字，只要句子帶「「X」分頁 / 分頁「X」/ 請至「X」」的形狀 | **不帶引號**、或**不帶「分頁 / Tab」字樣**的指路句（如「先到組合健診載入基金」） |

    **兩條的盲點不重疊，所以並行有意義；但兩條的盲點聯集不是空集，所以不得宣稱全覆蓋。**
    """
