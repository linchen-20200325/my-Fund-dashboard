...
#:
#: **歷史紀錄（2026-09-07，同一天）：本表的標籤與敘述已被抓到 6 處與實際不符。**
#: **⚠️ 這是「已知 6 處」，不是「已全部更正」，更不是窮舉** ——
#: 已更正的是這 6 處，**沒有人查過有沒有第 7 處**，本檔也不宣稱沒有。
#:
#: ==== ================================================== ==================================
#: #    被抓到的不符                                         誰、用什麼切法抓到
#: ==== ================================================== ==================================
...
# **完全在射程外** ⇒ 它底下兩句「唯一 caller 是 X」的**假全稱句**，
# **CI 一次都沒紅過**，最後是靠人回頭讀 `CLAUDE.md` 才撿到。
#
...
def test_a_sole_caller_claim_must_actually_be_sole():
    ...
    **這一條是本輪那兩處不符的直接解藥。** 被抓到的是：

    * `_render_pairs_ui` 寫「唯一 caller 是 `render_rotation_section`」——
      **實測兩支**（另一支是 `render_rotation_section_from_df`）。
    * `_render_pairs_body` 寫「下游：同上」—— **實測兩支**，而且直接 caller
      **根本不是** `render_rotation_section`，是 `_render_pairs_ui`。
