...
#: 已更正的是這 7 處，**沒有人查過有沒有第 8 處**，本檔也不宣稱沒有。
#:
#: ==== ================================================== ==================================
#: #    被抓到的不符                                         誰、用什麼切法抓到
#: ==== ================================================== ==================================
...
#:      舊 ② 根本沒掛在 `app.py` 上，那六塊目前不可見          「不守什麼」那一段早就寫著**
#: ==== ================================================== ==================================
#:
...
_MOVED: str = (
    ...
    "⛔ **2026-09-07 更正：『現在看得見』是假的 —— 本表第七處被抓到的不符**"
    "（**有意識的更正，不是漏刪**；決策者：AI 總管，依獨立稽核實測）。"
    "**實測（`origin/main` 與本分支皆同）**：`app.py` 的五個分頁槽裡沒有舊 ②，"
    "`render_fund_grp_health_tab` **連 import 都沒有**（唯一命中是一句註解）。"
    "⇒ **截至本次修改，這六塊對使用者是看不見的。**"
    "⚠️ **推翻它的證據就在同一個檔案裡** —— 本檔開頭「不守什麼」那一段逐字寫著"
    "「舊 ② 的接回是另一組的 PR；**在那之前，那六塊對使用者仍然是看不見的**」。"
    "**docstring 誠實、表格誇大，而沒有人交叉讀 —— 這正是本表一直在犯的那個病。**"
    "**現行（兩種狀態都為真的寫法）**：客戶 2026-09-07 原則是"
    "**新增 Tab 並行掛載、舊 Tab 一律保留不動**，直到客戶親自驗收才刪；"
    "本支在**呼叫圖上**經保留的舊 ② 仍到得了（守衛："
    "`test_a_pending_label_must_still_have_a_live_path`），"
    "**待舊 ② 接回 `app.py` 後即可經它看見** —— **在那之前，畫面上看不到。**"
    "⛔ 那條守衛保證的是「**路徑存在且完好**」，**不是**「**它現在被掛出來了**」。"
    "⛔ **不得**把它讀成「所以可以不用做 ③」——"
...
def test_a_sole_caller_claim_must_actually_be_sole():
    ...
    本輪兩處不符的**原始措辭**（逐字取自 `c321c0a`）落點如下 ——
    **這張表是實跑出來的，不是推論**（把刪除線拿掉讓原話重新生效，逐句各跑一次）：
    ...
    ⇒ **四句原話裡，本條只攔得住兩句。** #3／#6 那兩種寫法**結構上看不見**。
