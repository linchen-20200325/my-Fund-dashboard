...
⚠️ ~~「2026-09-07 批次接上真取數之前，這一頁沒有任何一條路徑會靠近寫入。」~~
**那句話是假的，寫下的同一天就被本組自己查掉**（**有意識的更正，不是漏刪**）：
深度區的**真**取數鏈會寫本地磁碟快取 ——
`repositories/fund/nav_metrics.py::_nav_history_cache_save` 做
`mkdir(parents=True)` ＋ `write_text(...)` 到 `cache/nav_history/{code}.json`。
**那是既有行為，不是本批引入的**，而且它寫的是**本地快取**、不是客戶的 Google Sheet
（客戶授權那句話點名的是 Sheet）。**但它確實是一次渲染期的磁碟寫入。**
⛔ **本檔看不到它**，理由見下方缺口 6 —— 請不要把本檔的全綠讀成
「這一頁渲染時什麼都不寫」。
...
`str.replace()` / `dict.update()` / `set.clear()` / `os.makedirs()` 這一類**偽陽性**。
⚠️ **本組是抽樣判讀，沒有逐一分類這 147 個**，所以**不宣稱「全部是偽陽性」** ——
能確定的是這個量級不可能被逐條讀完，而一份上百個待判讀項的守衛不會被讀，只會被關掉。
...
6. ⛔ **真的取數鏈整條看不到，這是本檔最大的一個洞，逐字寫明**：
   渲染 harness 一律把 `auto_fetch_moneydj` 換成假件
   （`tests/test_wf03_research_skeleton.py::_render` 的長註寫了為什麼：不換的話
   每一條帶 `applied=` 的測試都會**真的連外網**，而一份會連外網的守衛在 CI 上
   **不可重現** —— 它紅不紅取決於當天上游活著沒有）。
   ⇒ 那條鏈裡的寫入本檔**一個都看不到**，已知的一個是
   `repositories/fund/nav_metrics.py::_nav_history_cache_save`
   （`mkdir` ＋ `write_text` 到 `cache/nav_history/{code}.json`，**既有行為**）。
   **同理，批次真的跑起來之後那條鏈也看不到** —— 本檔一律 `submitted=False`（缺口 3）。
   ⛔ **不得**把本檔的全綠讀成「這一頁渲染時什麼都不寫」；
   它證明的是「**本頁自己寫的那些程式碼**沒有在渲染期寫入」。
   ⚠️ 這個洞要補的話是**另一個題目**（要嘛錄下取數鏈的寫入並判斷哪些是合法快取、
   要嘛把快取寫入本身移到使用者意圖之後），**本批沒有動**，已在 PR 具名登記。
