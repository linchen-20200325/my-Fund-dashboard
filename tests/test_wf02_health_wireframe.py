...
def test_those_strings_are_really_on_the_screen_not_just_in_a_constant():
    ...
    **「守衛守的是替身」** —— 底本（常數）釘死了，
    ...
    _all = _text(_parts)
    _missing = [_w for _w, _v in _verbatim_pairs() if _v not in _all]
    assert not _missing, (
        "下列線框逐字的句子**沒有出現在畫面上**（常數在、畫面沒有 ＝ 守衛守到替身）：\n  "
        + "\n  ".join(_missing) + "\n" + _all)
...
def test_the_eating_card_names_the_funds_it_counted():
    ...
    **線框 §2 逐字**：「**三張卡從「指標」改成「一件事」。** 現況三張卡是三個名詞；
    ...
    assert "配息覆蓋低於 1.0" in _body, (
        "卡片沒有用白話講「這是什麼意思」——\n"
        "  線框那張卡：「配息覆蓋低於 1.0，也就是在把你自己的本金配回來給你。」\n"
        + _body)
