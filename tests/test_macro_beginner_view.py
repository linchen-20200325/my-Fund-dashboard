        ...
        r = compute_four_horizon_summary(
            {
                ...
                # ⚠️ 2026-09-04 第五輪稽核 F8:再補兩顆 —— **同一個病第二次**。
                # 2026-08-05 那一輪已經發現「這條全綠是靠 3 顆缺席指標拿到的」
                # 並補了三顆;但**拐點桶還缺兩顆**(`YIELD_10Y3M` / `SLOOS`),
                # 而當時的判定是「取到一顆就算全清」,所以缺兩顆照樣綠、沒人發現。
                # 現在「都沒越線」要求點名輸入全在,這條測試才真的在測「全都健康」。
                "YIELD_10Y3M": {"value": 0.4},
                "SLOOS": {"value": 10.0},
            },
