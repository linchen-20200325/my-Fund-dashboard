...
def test_a_list_that_is_added_but_not_fetched_says_neither_of_the_two_wrong_things():
    ...
    **逐條 × 逐突變實測**（``F`` ＝ 該條會紅 ＝ 抓到了；``P`` ＝ 沒抓到）：

    ========================== ====== ====== ======
    斷言                        M10    M11    M20
    ========================== ====== ====== ======
    A1 ``EMPTY_TITLE_PENDING``  P      P      **F**
    A2 ``EMPTY_TITLE_NONE``     P      P      **F**
    B1 ``_want_missing``        **F**  **F**  P
    B2 ``_want_where``          P      P      P
    B3 ``pf_ledger not in``     **F**  P      P
    ========================== ====== ====== ======

    ⇒ **M20 只有 A 組抓得到；M11 只有 B1 抓得到；M10 由 B1／B3 抓到。**
    ⚠️ **B2 一顆都沒抓到** —— 它在這裡的理由是補齊鐵則 04 的第三要素，**不是殺傷力**。
    **據實列出，不灌水。**
    ...
    **餵錯任何一個數字，渲染出來的就會是別支的句子 ⇒ 當場紅。**
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ⛔⛔ 【第 2 組｜守 `_status_tiles` 的 💰 那一格】**共三條，B1／B3 不得刪除。**
    #     B1 是**唯一**擋得住 M11 的東西（`_status_tiles` 被餵過濾後的清單）。
    #     **上面第 1 組對 M11 全部 PASS，看不到它。**
    #     ⚠️ **B2（`_want_where`）在本檔量到的三顆突變裡一顆都沒抓到** ——
    #        它在這裡是為了補齊線框鐵則 04 的第三要素（「去哪補」要真的印出來），
    #        **不是**因為它擋得住哪一顆。**據實標明，不要把它算進「殺得掉」那一欄。**
    # ══════════════════════════════════════════════════════════════════════
