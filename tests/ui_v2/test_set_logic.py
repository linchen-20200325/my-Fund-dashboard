...
def test_六個檔都沒有import舊repo或網路或姊妹頁():
    """母體＝本頁全部六個原始碼檔（含 page.py 與 app_set.py）。AST 換算成絕對模組名再判。"""
...
def test_fixtures不import_logic_假資料不依賴判定層():
    ...
    assert not [t for t, _r in targets if t.startswith("ui_v2")], targets
...
def test_原始碼六個檔零禁詞_母體含page與app_set():
    """⛔ 交接檔 §8.1：姊妹頁的禁詞原始碼掃描漏過 page.py。本條的母體是六個檔，數目寫死。"""
    assert len(_ALL_SOURCE_FILES) == 6
