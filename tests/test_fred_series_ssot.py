...
def _macro_service_tree() -> ast.AST:
    return ast.parse(open("services/macro_service.py", encoding="utf-8").read())
...
def test_fred_constants_imported_from_ssot():
    """確認 macro_service 確實從 shared.fred_series import FRED_* 常數（SSOT 來源正確）。"""
    ...
    for const in ("FRED_CCSA", "FRED_ICSA", "FRED_CPI", "FRED_ISM_PMI"):
        assert const in src, f"{const} 應自 shared.fred_series 引入並使用"
