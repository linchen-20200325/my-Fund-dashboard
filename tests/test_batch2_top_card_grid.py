...
# ══════════════════════════════════════════════════════════════════
# 📌 `P-WHERECONTENT-1` —— **已搬進 `EXCEPTIONS.md §8.3.P`，那裡是正式住所**
#
# ⚠️ **2026-09-05 搬遷（有意識的搬遷，不是漏刪 · 決策者：DOC-P1 文件與治理組）**：
#    本段原本寫著完整的「待答問題／由誰查／觸發點」，並自陳
#    **「本批動不了 `CLAUDE.md`，請總管把本列搬進 §8.3.P」** —— 那次判斷是對的，
#    本輪照做了。**全文（含由誰查與六個觸發點）已移入 `EXCEPTIONS.md §8.3.P` 的
#    `P-WHERECONTENT-1` 那一列**，此處只留摘要與指路：
#    **待判定項的正式住所是憲法，不是測試檔** —— 下一組不會來這裡找待辦。
#
# **這條規則守不到什麼（留在這裡，是因為讀測試的人也需要知道）**：
#   `where=` 的**內容對不對**，`test_every_where_names_something_that_exists_on_screen`
#   **以 `「」` opt-in**、且只讀運算式裡的**字面值**（`_str_consts()`）。
#   `ui/views/page_01_macro.py` 的指路幾乎全走 `_where_to_load()` /
#   `where_to_find("diag")` 這種**執行期組出來的值**，AST 取不到字面值 → 看不見。
#   ⚠️ **本批擴大射程後那些站點全綠，但那不是「守住了」** ——
#      只是「那裡沒有手抄字串」。
#
# ⚠️ **搬遷時逐項重跑，發現本段原有兩個數字不準，已在憲法那一列就地更正**
#    （**有意識的更正，不是漏刪** · 日期 2026-09-05 · 決策者：DOC-P1）：
#      - 原寫 ~~「14 個 dict-carried 與 **4 個**直接呼叫」（＝18）~~
#        → 實測**執行期值共 19 個**（`card-dict` 13 ＋ `card-dict-item` 1
#          ＋ `not_ready` 3 ＋ `empty_state` 2）；直接呼叫是 **5** 個，不是 4 個。
#      - 更要緊的是：該檔唯一那個**字面值** `where=`（FRED 金鑰那句）
#        **沒有 `「」`** → 內容規則同樣看不到它。
#        **故該檔的內容覆蓋率是 `0/20`，不是 18/20，也不是 14/20。**
#    **原判斷的方向完全正確，被更正的只有數字與射程。**
#    重跑指令（repo 根目錄）：
#      python3 -c "import tests.test_batch2_top_card_grid as M; \
#        print([(fn, type(w).__name__) for f,l,fn,w in M._where_sites() \
#               if f=='ui/views/page_01_macro.py'])"
#    （`Call` ＝ 執行期值、`Constant` ＝ 字面值，須人工判讀。）
# ══════════════════════════════════════════════════════════════════
...
def test_not_ready_cards_carry_a_remedy_too():
    ...
    **「沒有它，占位只是把『消失』換成『灰色的消失』。」**
    ...
    - `where` 的**內容對不對** —— 那是
      ...
      **這一半有登記出口**：`P-WHERECONTENT-1`
      （**`EXCEPTIONS.md §8.3.P`**；2026-09-05 自本檔搬入憲法，本檔只留摘要）。
