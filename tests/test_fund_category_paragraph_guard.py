...
⚠️ **毒**語料一律從 `snap.json` 讀,不手抄第二份(§2.1 SSOT:手抄的會與實際資料漂移)。
⚠️ 但**合法標籤**語料(`_LEGIT_LONG_LABELS`)**是手抄的,而且只能手抄** ——
   `snap.json` 裡**真正是類別標籤**的只有「平衡型」「股票型」(各 3 字);
   唯一較長的短字串 ACDD19「中華民國境內之有價證券。」(12 字)是**說明書片段,不是標籤**。
   ⇒ 語料裡沒有任何「比 15 長但仍是標籤」的樣本,那個對照只能手抄。
   **代價要寫明:它不會自己跟著真實資料長大**,所以新增更長的合法類別名時
   **必須手動加進那個清單**,否則低側邊界測不到(見該常數上方的警語)。
"""
