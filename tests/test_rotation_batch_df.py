...
(1) 欄名 → dict key 對映(尤其 `吃本金燈號 (1Y · )` → `吃本金燈號`)、
(2) 跨類別配對可從 df 正常產出、
...
def test_rows_from_batch_df_maps_columns():
    ...
    assert row["吃本金燈號"] == "🟢 未吃本金"      # 大表欄名帶 (1Y · ) → 收斂為 key 吃本金燈號
...
def test_batch_df_cross_category_pairing():
    ...
    df = _df([
        {"code": "A", "基金名": "A", "基金類別": "股票型", "4D Grade": "A",
         "σ rank": "-0.20σ", "距 HWM %": "-3%", "操盤評分": 80, "吃本金燈號 (1Y · )": "🟢"},
        {"code": "B", "基金名": "B", "基金類別": "債券型", "4D Grade": "A",
         "σ rank": "-2.00σ", "距 HWM %": "-18%", "操盤評分": 75, "吃本金燈號 (1Y · )": "🟢"},
    ])
...
def test_batch_df_nan_row_not_polluting():
    ...
    df = _df([
        ...
        {"code": "B", "基金名": "B", "基金類別": None, "4D Grade": None,
         "σ rank": None, "距 HWM %": None, "操盤評分": None, "吃本金燈號 (1Y · )": None},
    ])
