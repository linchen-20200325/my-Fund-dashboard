...
--------------------------------------------------------------------
起因：**同一句話寫在兩個檔，上一輪只改了被抓到的那一處**（`EXCEPTIONS.md §8.2.A.1`
...
def test_every_grey_unit_says_where_to_look(unit: str):
    ...
    **真的失去了「（請先到：…）」**。
    ...
    #    真正驗「指路指到的東西存不存在」的規則見 `EXCEPTIONS.md §8.3.P` 的
...
def test_every_upstream_failure_marker_has_been_triaged():
    ...
    （`EXCEPTIONS.md §8.2.A.1` 驗證段 ④ 記載的正是這個失效模式）。
...
def test_the_page_asks_the_search_service_not_the_repository():
    ...
    `EXCEPTIONS.md §8.2.A.1` 的 **`EX-PASSTHRU-1`** 那一列自己寫著升級觸發條件：
