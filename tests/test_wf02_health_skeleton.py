...
def test_the_table_never_shows_a_bucket_grade_because_there_is_no_such_thing():
    ...
    **這是本批唯一一個「明明有東西可以填、但刻意不填」的欄位**，所以要釘住。
    ...
    # ⭐ **2026-09-09：判準由「有沒有出現 `4D` 這兩個字」換成「有沒有回答那個問題」。**
    #    **這是換一個更難騙過的判準，不是放寬** —— 逐條看：
    #      · **舊判準有一個真的洞**：`4D` 是**內部術語**，
    #        客戶 2026-09-08 拍板的線框 §2「這一頁改了什麼」第 4 條逐字要求
    #        「**使用者需要知道的是「這個數字為什麼是空的」，不是「我們送到哪裡了」**」，
    #        並把這句話的文案定為「**那一欄的評等定義還沒定案，不拿別的評等填進來充數**」。
    #        照舊判準，**唯一能通過的寫法就是把內部術語印在客戶畫面上**。
    #      · **新判準管的是同一件事的兩半**：欄名要指名（`五桶`），
    #        而且要說清楚「**為什麼是空的**」＋「**沒有拿別的東西填**」。
    #      · **而且多守了一條舊判準完全看不到的**：⛔ 這句話裡**不准有進度語言**。
    #        舊版寫「已送客戶確認」照樣全綠（它含 `4D`）——**那正是本批要拿掉的字**。
    assert "五桶" in _table_body, (
        "「五桶評等」整欄留白，但**這張表底下**沒有指名是哪一欄 —— "
        "沒指名的說明會被讀成在講別的東西。\n"
        "⚠️ 別把別的區塊的說明算進來：使用者看表的時候不會滑上去讀總分那一塊。\n"
        + _table_body)
    assert "還沒定案" in _table_body and "不拿別的評等填進來充數" in _table_body, (
        "「五桶評等」整欄留白，但**這張表底下**沒有回答「為什麼是空的」——\n"
        "  線框 §2 逐字：「那一欄的評等定義還沒定案，**不拿別的評等填進來充數**」。\n"
        "  少了後半句，使用者不知道我們是「沒有」還是「懶得填」；\n"
        "  少了前半句，空欄會被讀成「這幾檔沒有評等」。\n" + _table_body)
    _progress = [_w for _w in ("已送客戶確認", "4D", "分批上線", "下一批", "尚未答覆")
                 if _w in _table_body]
    assert not _progress, (
        "這張表底下那句說明混進了**我們的進度／內部術語**："
        + "、".join(_progress)
        + "\n⛔ 客戶 2026-09-08 拍板線框 §2 第 4 條：使用者要知道的是"
          "「這個數字為什麼是空的」，不是「我們送到哪裡了」。\n"
          "⚠️ 完整的工程理由不是刪掉，是**搬回程式碼註解**"
          "（`_table_rows` / `_render_health_score` 的 docstring 一個字都沒少）。\n"
        + _table_body)
