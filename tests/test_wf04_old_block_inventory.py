...
#: ⚠️ **`客戶裁決由新頁自建` 是 2026-09-08 新增的第六種**（**有意識的新增，不是自創**：
#: 本檔 `test_every_entry_is_filled_in_properly` 的失敗訊息逐字寫著
#: 「真的需要，先改 `DISPOSITIONS` 並在那裡寫清楚為什麼」—— 這裡就是「那裡」）。
#: **為什麼前五種都不對**：客戶 2026-09-08 拍板「**加入基金入口整合至 ⑨
#: 「保單與扣款標的」**」，也就是那個能力**由新 ④ 自己做一份**——
#: * `已委派` / `本批委派` 都是「新 ④ 去呼叫舊區塊」→ **沒有呼叫**；
#: * `刻意灰態` 是「明知道它存在、刻意不做」→ **做了**；
#: * `已由總管裁決搬走` 的定義是「搬到**別頁**，④ 不再擁有它」→ 方向相反，
#:   它是**搬進**新 ④，而且裁決者是**客戶**不是總管；
#: * `待客戶裁決` 在客戶答覆之後就是假的。
DISPOSITIONS: frozenset[str] = frozenset({
    ...
    "客戶裁決由新頁自建",  # 客戶點名要這個能力，但由新 ④ 自己做一份，不呼叫舊區塊
})
...
OLD_TAB4_INVENTORY: tuple[tuple[str, str, str, str], ...] = (
    # ── 8 個頂層 slot ────────────────────────────────────────────────
    # ⭐ **2026-09-08：客戶答覆了 —— 由 `待客戶裁決` 改為 `客戶裁決由新頁自建`。**
    #    這正是 `test_the_client_gated_entries_are_not_quietly_marked_done` 的
    #    docstring 逐字寫的那個情形：「客戶說『照 A 做』之後，把該筆改成……
    #    **同時**把本條的 `_GATED` 清單一起改小。」兩件事在同一個 commit 裡做完。
    #    ⚠️ **只移出這一格**：`_sec_ai` / `_sec_raw` 問的是**別的**區塊的去留，
    #       客戶 2026-09-08 一個字都沒有提到它們，⛔ 不得順手一起放掉。
    ("_sec_add", "➕ 加入與管理基金", "客戶裁決由新頁自建",
     "客戶 2026-09-08 拍板逐字：「加入基金入口：拍板整合至 ⑨「保單與扣款標的」，"
     "無持倉時直接就地展開輸入表單。」並指定它是退場順序（⑦→⑥→⑧→⑨）的**第一前置**。"
     "→ 新 ④ 自建：`page_04_portfolio._render_add_fund`（純函式部分在 "
     "`ui/helpers/portfolio/add_entry.py`），**不呼叫**舊 `_sec_add` 一行程式碼。"
     "⚠️ 兩者**不等價，落差已就地登記**在 `page_04_portfolio.ADD_FUND_SCOPE_NOTE`："
     "新版不打網路、不寫 Google Sheet、不收投入金額。"
     "⚠️ **舊區塊本身沒有被拔**（客戶拍板的是退場**順序**，不是「現在就刪」）——"
     "本格記的是「那個能力去哪了」，不是「舊的還在不在」。"),
...
def test_the_client_gated_entries_are_not_quietly_marked_done():
    ...
    **同時**把本條的 `_GATED` 清單一起改小。
    ...
    _GATED = {
        ...
        #    → **2026-09-08 移出：客戶答覆了**（逐字：「加入基金入口：拍板整合至 ⑨
        #    「保單與扣款標的」，無持倉時直接就地展開輸入表單」）。
        #    ⚠️ **客戶答的是「那個能力放哪」，不是「舊區塊什麼時候拔」** ——
        #    後者由退場順序（⑦→⑥→⑧→⑨）決定，本表也沒有在記它。
        #    ⚠️ **只移出這一格**：題三另外兩格（`_sec_ai` / `_sec_raw`）問的是別的區塊，
        #    客戶 2026-09-08 一個字都沒有提到它們（同 2026-09-07 移出 `_ov_group` 時
        #    就地寫下的那句警告）。
        "_sec_ai": "PR #791 題三（舊 ④ 頂層區塊去留）",
