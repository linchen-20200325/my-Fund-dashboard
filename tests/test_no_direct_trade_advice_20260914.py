...
⚠️ **2026-09-14 第四輪回修就地更正（有意識的更正，不是漏刪 · 決策者：AI 總管）**：
**上一段把源頭指錯一顆 commit，也把「漏兩輪」講成「漏一輪」。實測逐 rev**：
  · **`da16c32`** —— 無 C 組；頭段「兩個落地點」**為真**；bullet 只有 A／B。
  · **`d86118f`** —— **C 組 banner ＋ 8 條測試已落地**（＝第三個落地點）；
    頭段仍寫「兩個落地點」**（自此為假）**；bullet 仍只有 A／B。
  · **`53b92c4`** —— 新增 D 組；bullet 補成 **A／B／C／D**；頭段**仍然沒改**。
⇒ **失準發生在 `d86118f` 那一刻**，不是 `53b92c4`。`53b92c4` 是**第二次錯過** ——
它把同一段從「對外失準」惡化成「**內部也自相矛盾**」（頭段兩個、bullet 四組），但仍沒改頭段。
**⇒ 這一段漏了兩輪，不是一輪。**
**舊表述的用意仍然成立**（要指出「編輯這一段的人沒有同步計數」），
**被權衡掉的是它的歸因**：把源頭記在 `53b92c4`，會讓後人以為「編輯時順手看一眼就夠了」——
但 **`d86118f` 根本沒有編輯這一段**，它只是在**別處**讓計數變了。
**那才是真正的失效模式：讓計數失準的改動，與計數所在的那一段，不一定在同一個地方。**
**驗證指令（repo 根，單行；`git show` 唯讀，不動工作樹）**：
`for r in da16c32 d86118f 53b92c4; do echo -n "$r "; git show $r:tests/test_no_direct_trade_advice_20260914.py | grep -c '^# C 組'; done`
...
**守不到（據實寫明，不要把本檔讀成「合規已完成」）**：
  ...
  - ⛔ **B2 的禁字清單掃不到同一個檔裡「現在就在畫面上」的那幾段行動文字**
    ...
    **全部 `on_screen=True`**，而 `_forbidden` 八個條目**沒有一個是它們的子字串**
    ...
    ⚠️ **這則裁示的出處尚未查證（2026-09-14 第四輪回修就地補；`CLAUDE.md §-2` 規則 6）**：
    **客戶原話由總管轉達，repo 內查無獨立記錄。** 它只能當「**總管轉達的裁示**」讀，
    **不得引用為「已查證的客戶原文」**。完整登記見 `EXCEPTIONS.md::P-FROZENUNFREEZE-1` 的 (b)。
    ⚠️ **為什麼這裡也要寫一次**：這則裁示在 repo 內共出現**三處** —— 本處、
    `EXCEPTIONS.md` 的 `P-FROZENTABREF-1` 與 `P-FROZENUNFREEZE-1`
    （量測日 2026-09-14，指令 `git grep -c "只加免責" ebfd9d5f -- .`，輸出
    `EXCEPTIONS.md:2` ＋ 本檔 `:1`）—— 而但書原本**只掛在 `P-FROZENUNFREEZE-1` 一處**，
    另兩處是裸事實。一個「只掛在某些地方」的但書，等於留下可以繞過它的引用點，正是
    `CLAUDE.md §-2` 規則 6 末句「**留但書等於留引用點**」講的那個形狀。
    **本輪把另外兩處補齊**（PR 描述末段本來就有）。
    ⛔ **補但書不降低該裁示的效力**：「v1 只加免責、不改內容」照舊執行。
