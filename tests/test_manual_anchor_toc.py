...
def test_no_bold_hardcoded_tab_name_in_manual():
    ...
    **射程證據 A｜同字碰撞三族（2026-08-31 實測，目前皆未命中）**
    ...
    - **Google Sheet 的「工作表」在本 repo 文案裡也叫「分頁」** ——
      `ui/helpers/v2_editor.py:299`（隔 7 字）；
      `ui/tab6_manual.py::render_manual_tab` 第 `sheet-schema` 章的
      `**刪分頁前先看這裡**：系統會用到的分頁`（隔 7 字），
      以及同章同一段內最近的一處 `不可再生的資料**：其他分頁`（**只隔 3 字**）。
    ...
    ⚠️ **上面第一族的兩處 `ui/tab6_manual.py` 刻意寫成「符號 + needle」而不寫行號
    （2026-09-01 就地更正；有意識的變更，不是漏刪 · 決策者：**AI 總管**）。**
    ~~舊寫法：`ui/tab6_manual.py:617`（隔 7 字）、同檔 `:622`（只隔 3 字）~~
    —— **舊寫法在寫下的當天（`cda7909`）完全正確**，是本 PR 後續的 `a2ebb53`
    在同檔上游插入 3 行，把它們推成 620 / 625；**它不是寫錯，是註定會過期**。
    依 `CLAUDE.md` 反覆引用的「**禁止寫行號**，一律以『檔案路徑 + 符號名 + 模式描述』
    登記 —— 行號在任何一次重構後就失效，而重構不會觸發本清單更新」
    （`EX-PASSTHRU-1` 那一列自陳「**本次被更正的舊登記正是行號寫法的實例**」），
    **本輪的修法是讓這個位置不再有會漂移的東西，不是換一個新數字** ——
    換新數字只會讓下一次插行再製造一次同樣的假宣稱。
    ⛔ **請不要把行號加回來。** 上面兩個 needle 都能被一條 `grep` 重新定位；
    `隔 7 字` / `隔 3 字` 是**間距**，屬本段論證的實質內容、不隨行號漂移，故保留。
    ⚠️ **同段其餘 5 個行號（`v2_editor` / `tab_batch_analysis` ×3 / `story_nav`）本輪未動**：
    那三個檔**不在本 PR 的 diff 內**，不是本 PR 打壞的，改它們屬另一批的範圍
    （`CLAUDE.md §8.4 步驟 4`）；**本輪也未以 `_live_strings` 的判準逐一複驗它們**，
    讀者請據此打折信任那 5 個數字。
