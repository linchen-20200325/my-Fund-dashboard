...
_MIN_LIVE_TIER1_PER_FILE: dict[str, int] = {
    "CLAUDE.md": 150,
    # ⚠️ **2026-09-09 由 200 上調為 260**（**有意識的收緊，不是放寬**；
    #    決策者：**AI 總管**，⑥ 那一批第二輪）。
    #
    # **為什麼調**：`test_guard_still_sees_the_whole_constitution` 的餘裕 ratchet
    # 在本輪轉紅 —— **實測 324 > 200 × 1.6 ＝ 320**。那條守衛自己寫著
    # 「檔案長大到超出這個帶寬時，CI 會**要求你把下限往上調**（而不是安靜地
    # 讓保護力衰減）」，並指定**新下限取 `max(現行下限, 現值×0.8)`**。
    #     max(200, 324 × 0.8 ＝ 259.2) → **260**
    #
    # **量測（本組實測，指令見本區塊上方那段註解裡的一行 `python3 -c ...`）**：
    #     base `b345613` = **311** ／ `3deba1b` = **317** ／ 本輪 = **324**
    # ⚠️ **base 就已經是 311**，也就是**這個下限早就偏低了**，只是還沒撞到帶寬上界；
    #    本輪新增一列 `P-P02BLOCKNAME-1` 把它推過 320，**紅燈才出現**。
    #    ⇒ **不是本輪把保護力弄壞的，是本輪讓一個早就在衰減的東西現形。**
    #
    # ⛔ **這是把下限往上調（收緊），不是放寬** —— 調高之後「可以被靜默搬走而不觸發
    #    紅燈」的量從 124 降到 64。⛔ **不得**改大 `_PER_FILE_FLOOR_MAX_SLACK` 來閉嘴，
    #    那正好是那條守衛要防的動作。
    # ⚠️ 留 64 的餘裕是刻意的（不是貼著 324）：本 repo 鼓勵把過期引用加刪除線退役，
    #    而退役會讓「活的引用」變少 —— 貼著現值會把**正確的退役**變成紅燈。
    "EXCEPTIONS.md": 260,
}
...
#
# ⛔⛔ **本常數是一個【已登記的待判定項】：`EXCEPTIONS.md §8.3.P` 的 `P-FLOORSLACK-1`。**
#    **它自己可以被一行改掉，而它守的正是「不要一行改掉」** ——
#    實測（#835 第二輪，三道正對照齊全）：`1.6` → `99.0` ⇒ 本檔 **18 passed、突變存活**。
#    `test_guard_still_sees_the_whole_constitution` 那條 assert 的**失敗訊息**寫著
#    「⛔ 不要改大它來閉嘴」，**但那是一句訊息，不是一條規則** —— 沒有東西在守這個數字。
#    ⚠️ 而它寫在**失敗訊息**裡，比寫在註解裡**更沒有牙**：只有紅燈時才看得到，
#    而來改這個常數的人，正是**為了讓紅燈消失**才來的。
# ⚠️ **要改它之前請先讀 `P-FLOORSLACK-1`**（該列有「待答問題／由誰查／觸發點」三欄，
#    觸發點 (2) 逐字寫著「**有人被那條紅燈擋住**」—— 那就是你現在的處境）。
# ⛔ **#835 未動工**（`CLAUDE.md §-1` 無觸發、`§8.4 步驟 4` 不擴大 scope）；
#    本註只是把那份登記**接回程式碼** —— 一份在程式碼裡零回指的登記，讀不到的人等於沒有。
#    （`EXCEPTIONS.md §8.2.A.1` 要求新增例外必須「對應檔案加註解指回此表」；
#     #835 第三輪實測：全 repo **36 列 `P-` 登記，35 列有程式碼回指，只有這一列是 0**。）
...
_MIN_BYTES_PER_FILE: dict[str, int] = {
    "CLAUDE.md": 227721,
    # ⬆️ **2026-09-08 上調（#835）**：`131167` → `169138`。
    #    **這是收緊，不是放寬** —— 本表是**下限**（防掏空），只升不降。
    #    上調的**理由是這條守衛自己開的處方**：它在
    #    `現值 > 下限 × 1.6` 時轉紅並要求「取 `max(現行下限, 現值×0.8)`」——
    #    因為下限離現值太遠時，**它擋得住「整檔清空」，擋不住「刪掉一半」**。
    #    ⚠️ **舊值 `131167` 在 `origin/main` 上就已經過期**（該處現值 204,716 bytes，
    #    ×0.8 應為 163,772）—— 本輪的追加只是**把它推過 1.6 倍的門檻而已**，
    #    不是本輪造成的衰減。**量測日 2026-09-08。**
    #    ⚠️⚠️ **2026-09-09 就地講清楚（第三輪稽核登記；有意識的補充，值一格未改）**：
    #    **上面那個算式【不是】下面這個值的來源 —— 兩個數字都對，但它們不同源。**
    #        `204,716 × 0.8 = 163,772`  ← `origin/main` 的現值（上面那句在講的）
    #        `211,423 × 0.8 = 169,138`  ← **本 PR `444f7a0` 當下**的現值（實際設的值）
    #    照字面讀會以為前者導出後者，**它沒有**。取的是處方裡
    #    「`max(現行下限, 現值×0.8)`」**較大的那一個**，也就是後者。
    #    ⇒ **記這一筆的理由**：一個看起來像推導、實際不是推導的算式，
    #      會讓下一個人拿 `163772` 來「訂正」這裡 —— **那會是一次放寬。**
    "EXCEPTIONS.md": 169138,
}
...
_DECLARED_REGISTRY_IDS: frozenset[str] = frozenset({
    ...
    # 2026-09-09 ⑥ 那一批新增的兩列。
    # ⚠️ **本批初版漏了這兩行，是獨立稽核抓到的（必修 E）** —— 而 CI **全綠**，
    #    因為這份清單是**單向**的比對名單（`_DECLARED_REGISTRY_IDS - _present`），
    #    「登記簿多了一列而這裡沒寫」它看不到。上面那句
    #    「**新增登記簿列的人要同時更新這裡**」就是為了這件事寫的，而我還是漏了。
    # ⚠️ **兩列刻意分開登記、不合成一列**：它們在 ⑥ 的畫面上，但**分屬不同的檔案邊界**
    #    —— `P-P02JARGON-1` 改得動（只是不在那一批的清單裡），
    #    `P-P02DELEGJARGON-1` 住在被委派的舊模組裡、**本批動不了**。
    #    合成一列會讓「誰能修」這個差別消失。
