...
_MIN_LIVE_TIER1_PER_FILE: dict[str, int] = {
    "CLAUDE.md": 150,
    # ⬆️ **2026-09-14 上調（Q8 止血第三批 / #844）**：`200` → `264`。
    #    **不是為了讓紅燈閉嘴，是這條 ratchet 自己要求的動作**：本 PR 往 `8.3.P`
    #    新增一列登記（`P-FROZENTIERWRITE-1`）後，實測活 Tier-1 引用 **330**
    #    （**量測日 2026-09-14**），超過 `200 × _PER_FILE_FLOOR_MAX_SLACK(1.6) = 320`。
    #    取法逐字照本檔失敗訊息：**`max(現行下限, 現值×0.8)`（只升不降）**
    #    ⇒ `max(200, 330×0.8=264) = 264`。
    #    ⚠️ **本組初稿誤用了上面第 ~402 行那句「向下留約 20~25% 餘裕」算成 `247`** ——
    #    那正是失敗訊息裡點名「**2026-09-08 被實測推翻、自己是個放寬指令**」的那句話。
    #    `247 < 264`，照做會比規定更鬆。**留這一筆，是因為那個陷阱真的會咬到人。**
    #    ⚠️ **本 PR 沒有碰 `_PER_FILE_FLOOR_MAX_SLACK`** —— 那是本條明禁的動作
    #    （見該常數上方的 `P-FLOORSLACK-1` 註）。**調的是下限，不是帶寬。**
    #    ⚠️ **這是收緊**：可被靜默搬走的量由 `330-200=130` 收到 `330-264=66`。
    #    ⛔ **合併衝突（同欄位三方）**：#836 記載調到 **260**、#839 量到 **324**、
    #    本 PR 量到 **330**。**後合併者必須現場重量並取三者最大值**，
    #    不要直接沿用任何一個分支寫下的數字。
    "EXCEPTIONS.md": 264,
}
