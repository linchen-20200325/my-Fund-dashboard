...
### A 組｜~~可達性缺口（真的守不到，靜態分析的物理上限）~~ → **真的守不到（兩種成因）**

⚠️ **2026-09-03 標題更正（有意識的更正，不是漏刪 · 決策者：AI 總管 · 依據：本組原型實測）**：
舊標題只寫「**可達性缺口**」，**涵蓋不了本組後來新增的第 6 條（字面上不存在）** ——
那一條與「會不會被執行到」無關。**舊標題的用意仍然成立**（1~5 條確實全是可達性問題），
**被權衡掉的是它的射程**。現行 A 組 ＝ **(a) 可達性**（死分支騙過 AST）
＋ **(b) 字面上不存在**（原始碼裡既沒有那串字、也沒有 SSOT 名字）。

#### (a) 可達性：AST 看得到「這一行寫在 `render_t7_section` 裡」，**看不到它會不會被執行到**
...
**要擋住 (a) 得整頁真渲染**，而 T7 主體在沙箱拿不到 OAuth / NAV / FX。

#### (b) 字面上不存在：**標題被整個改名**

6. `st.markdown("#### B 段（改名）")`（**E1b**）—— 原始碼裡**既沒有段標籤那串字、
   也沒有 `_T7_SECTIONS` 這個名字**，畫面上多一個與目錄矛盾的標題。
   **任何靜態規則都看不到它**：沒有東西可以比對。

⚠️ **2026-09-03 第三次分類更正（有意識的更正，不是漏刪 · 決策者：AI 總管
· 依據：本組在 repo 外跑的原型規則實測）—— 這是同一個錯誤在同一份 diff 裡第三次發生。**
本組上一版把下列三顆放進 A 組（＝**明寫免修**），**實測證明它們都是可修的**，已全部移入 B-2：

    ~~6. 值在執行期才產生：st.markdown("#### " + dict(_T7_SECTIONS)["b"] + "…")（E7/E3）、f-string（E5）~~
    ~~7. 字被改掉：st.markdown("#### B投入再平衡（暫停使用）")（E1，少一個空白）~~

**實測（原型規則寫在 repo 外，本輪刻意不落地成守衛）**：

| 案例 | 「呼叫子樹不得出現 `_T7_SECTIONS`/`_t7_anchor`」 | 「空白正規化後比對字面值」 |
|---|---|---|
| E7/E3 | **抓得到** | — |
| E5 f-string | **抓得到** | — |
| E1 少一個空白 | 抓不到 | **抓得到** |
| **E1b 整個改名** | **抓不到** | **抓不到** ← 真上限，留在 A |

**兩條原型規則對現行 production 的誤傷數皆為 0。**
⛔ **本輪只改分類文字，不新增守衛**（總管射程限制）；規則本身登記在 B-2 待辦。

⚠️ **2026-09-03 第二次更正（有意識的更正，不是漏刪 · 依據：delta 稽核 E7/E8 + 本組複跑）**：
本段原寫 ~~「**對照組（證明只有刻意的死分支會漏）**……」~~ ——
**那個結論是假的，而且錯在對本檔有利的方向。**
稽核用**兩顆完全沒有死分支**的突變推翻它（本組已各自重跑確認）：

    E7  _t7_section_heading("b") 留著 + st.markdown("#### " + dict(_T7_SECTIONS)["b"] + "（暫停使用）")
        -> 13 passed（GREEN）
    E8  _t7_render_toc() 留著 + st.markdown("**📑**　[A加碼]" + "(#" + "t7" + "-a)")
        -> 13 passed（GREEN）

**原推論漏了一步**：舊對照組 `M14d`（刪掉呼叫再手寫）轉紅，是
`test_render_path_uses_the_ssot_headings_in_order` 發現**呼叫不見了**，
**指紋守衛在那顆突變裡根本沒出手** —— 用它去推「只有死分支會漏」是無效推論。
⚠️ **後果**：本檔把一個 **B 組（可修）** 缺口寫進了 **A 組（明寫免修）**，
**正是本節自己在批判的「替可修的漏洞發免修證明」。同一份 diff 裡又犯一次。**

### B-2 組｜**尚未修**的語法層缺口（可修，登記待辦，**不得算進 A 組**）

| # | 寫法 | 現況 | 補得起來的方式（**本輪未實作**） |
|---|---|---|---|
| E7/E3 | `st.markdown("#### " + dict(_T7_SECTIONS)["b"] + "（暫停使用）")` | GREEN | 呼叫子樹不得出現 `_T7_SECTIONS`/`_t7_anchor`（**實測抓得到、零誤傷**） |
| E5 | `f"#### {dict(_T7_SECTIONS)[...]}（暫停使用）"` | GREEN | 同上（**實測抓得到**） |
| E1 | `st.markdown("#### B投入再平衡（暫停使用）")`（少一個空白） | GREEN | 空白正規化後比對（**實測抓得到、零誤傷**） |
| E6 | `_lbl = "B 投入" + "再平衡"` 後 `st.markdown("#### " + _lbl)` | GREEN | 常數折疊 / `ast.literal_eval` |
| E8 | `st.markdown("**📑**　[A加碼]" + "(#" + "t7" + "-a)")` | GREEN | 同上 |
| E7v | `_lbl = dict(_T7_SECTIONS)["b"]` 後再畫 | GREEN | 函式內資料流追蹤（最貴的一顆） |

⚠️ **一處連稽核也講得比實況寬的地方，本組實測更正**：稽核寫「**串接**……一律繞得過」——
**不成立**。純字面值串接**現行守衛就抓得到**，因為本條走的是
`ast.walk(call)` 找**任一** `ast.Constant`，串接的每個運算元各自都是 Constant：

    E2  st.markdown("#### " + "B 投入再平衡" + "（暫停使用）")  -> 1 failed（RED，已擋住）

**真正漏掉的是「指紋跨運算元被切斷」（E6/E8），不是「有串接」。**
⛔ **本批不修 B-2**（總管指定本輪只改字、不改程式碼）；登記於此供下一批處理。

### B-1 組｜語法層覆蓋缺口（**當時漏掉，2026-09-03 已補，四條各自實測轉紅**）

⚠️ 這四條**不屬於** A 組，**不得**引用 A 組那句「靜態守衛的上限」替它們免修 ——
它們從頭到尾都是 AST 擋得住的，只是當時沒有人去擋（**B-2 組同理，只是還沒補**）：
...
def test_no_handwritten_toc_or_heading_text_bypasses_the_ssot() -> None:
    ...
    **本條擋的是「手寫的字面值」** —— 只要一個會畫字的呼叫**裡有一個字串字面值**
    命中下列任一指紋，就代表有人手抄了本該由 `_T7_SECTIONS` 產生的東西：
    ...
    ⛔ **射程只到「字面值」這四個字，不要多讀一個字。**
    ⚠️ **2026-09-03 就地更正（有意識的更正，不是漏刪 · 依據：delta 稽核 E7/E8 + 本組複跑）**：
    本段原寫 ~~「本條擋的是**字從哪裡來**，不是用哪個 API」~~ —— **那句是假的**，
    而且被一個**逐字反例**打掉：

        E3/E7  st.markdown("#### " + dict(_T7_SECTIONS)["b"] + "（暫停使用）")

    它**字面上就是「字從 SSOT 來」**，渲染出來含完整段標籤，**零字面值指紋 → 全綠**。
    **本條看的是「有沒有人把那串字打進原始碼」，不是「那串字的來源是不是 SSOT」。**
    完整的守得到／守不到清單見模組 docstring「本檔守不到的」，**含本組實測的
    E1/E2/E5/E6/E7/E8 六顆**（其中 **E2 是唯一被擋下來的**）。
    """
