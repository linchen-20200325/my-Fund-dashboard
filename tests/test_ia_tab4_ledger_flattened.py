...
6. **值在執行期才產生的手寫標題** —— 字面上沒有那串字，指紋原理上看不到：
   `st.markdown("#### " + dict(_T7_SECTIONS)["b"] + "（暫停使用）")`（E7/E3）、
   f-string 插值（E5）。⚠️ **這兩顆沒有死分支、SSOT 呼叫也還活著。**
7. **字被改掉** —— `st.markdown("#### B投入再平衡（暫停使用）")`（E1，**少一個空白**）。
   畫面幾乎看不出差別。**任何字面值指紋本質上都擋不住它。**

**要擋住 A 組的第 1~5 條得整頁真渲染**，而 T7 主體在沙箱拿不到 OAuth / NAV / FX；
第 6/7 條則是**字面值指紋的原理上限**（字根本沒被打進原始碼）。
...
值是由**純字面值**組出來的、只是被拆散，**常數折疊 / `ast.literal_eval` 抓得到**：

| # | 寫法 | 現況 |
|---|---|---|
| E6 | `_lbl = "B 投入" + "再平衡"` 後 `st.markdown("#### " + _lbl)` | GREEN（**可修**） |
| E8 | `st.markdown("**📑**　[A加碼]" + "(#" + "t7" + "-a)")` | GREEN（**可修**） |
