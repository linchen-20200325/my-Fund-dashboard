...
## §1-B 核心需求二：單軌完整版 UI/UX（白話與量化並列，不分模式）

> ⚠️ **2026-08-05 稽核 🟡 必修 6 更正**：本節原本規範「新手/老手雙軌 + 模式切換」，
> 但該功能**已被 user 兩次明確否決並移除**，SPEC 與實作長期矛盾、且矛盾的一方是 SPEC。
> 依 `CLAUDE.md §2.2`（可追溯）記錄兩次移除決策，並改寫為實況。
> **不得**依舊版 SPEC 把模式切換加回去 —— 那是 user 已否決的東西（`CLAUDE.md §-1`）。
>
> | 版本 | 日期 | user 決策 | 移除內容 | 現場證據 |
> |---|---|---|---|---|
> | v17.0 | — | 改單軌完整版 | 新手/老手 toggle | `ui/tab1_macro.py:800-803`「所有資訊一律展開，不藏」 |
> | v19.128 | 2026-06-25 | 「只保留專家」 | v19.125 三層 toggle（新手模式 / 進階模式 / 原理教室）全刪 | `ui/tab1_macro.py:818-821`；改為四時域（長期/中期/短線/拐點）分組 |

### 現行設計原則（單軌）

1. **不做模式切換**：`_expert_mode` / `_show_l2_plus` / `_show_l3` 恆為 `True`
   （`ui/tab1_macro.py:804-807`），保留為變數只是為了下游相容，不再有 UI 開關。
2. **白話與量化同頁並列**（非二選一）：每個指標卡同時給
   - 服務層 `desc` 口徑說明（`ui/tab1_macro_midcycle._card_note` 渲染）
   - `how_to_read` 教學表（`ui/components/macro_card_edu.py`，25 個指標）
   - 24 個月趨勢圖 + 警戒線（`_make_radar_sparkline` / `_apply_tp_thresholds`）
3. **代理值必須可見**：`is_proxy=True` 一律加 `⚠️` 前綴 + `proxy_note`
   （卡片：`ui/tab1_macro_midcycle._card_title/_card_note`；AI prompt：
   `ui/tab1_macro_ai._build_macro_ai_snapshot` 的 `[PROXY 代理值]` 標記）。§1 反造假。
4. **漸進式揭露改由「資訊層級」承擔，不由「使用者模式」承擔**：
   頁面順序 = 綜合健康度 hero → 五桶 summary bar → 📋 決策矩陣（結論）
   → 四時域細節 → AI 總結。總覽在前、推導在後，但**全部展開可見**。
...
> 白話優先；出現 Z-Score / σ / OAS 等術語時必須就地用一句話解釋。
...
### 兩套評分尺度的消歧義規範（2026-08-05 必修 3）

Tab① 同畫面存在兩個分數，**方法學不同、禁止合併或互相換算**：

| 位置 | 數值 | 語意 | 值域 | SSOT |
|---|---|---|---|---|
| 綜合健康度 hero 卡 | 如 `+15.5` | 多空**強度**（23 指標 Σ score×weight） | 有正負 | `services/macro_composite_score.calculate_composite_score` |
| 五桶 bar「🌳 長期」 | 如 `擴張 6.8/10` | 景氣**位階**（循環階段評分） | 恆 0–10 | `calc_macro_phase` + 文字格式 `ui/helpers/macro/helpers.format_phase_score` |

- 位階字卡文字**一律**走 `format_phase_score()`（禁止各處自行 f-string，禁用 `:+.1f`
  —— 位階恆非負，帶正號會與 hero 的有號淨分撞臉）。
- hero 卡下方必須有一行對照 caption，明說「這是強度、那是位階」。
- 不合併的理由：6+ consumer 吃 `phase.score`，合併會破壞既有語意。
