...
使用者輸入 MoneyDJ URL / 代碼
  ...
  → UI 渲染:
      ...
      └─ ④ AI 深度解盤：唯一 render_ai_summary_widget，吃全章節快照
            (v18.207 整併 — 原 analyze_fund_json 按鈕 / 個股新聞 AI / 末端 widget 三合一)
```
...
> 🆕 **v18.230 / 2026-05-28** — A/B/C 試算支援「目標單位數」模式（新標的可配股數、舊持倉可混搭）。痛點：A/B/C 只能 TWD/% → 新標的（例：安達）無持倉時 % 算不出。設計：UI 加「分配模式」selectbox 群（`📊 % / 💵 TWD` vs `🎯 目標單位數`），`_t7_has_position(pk)` 決定預設模式，mode 放 form 外讓 widget 即時 reactive；submit 時用 helper `_t7_units_to_twd(units, nav, fx)` 換算 TWD 餵舊 `Ledger.subscribe()` / `Switch.switch_*()`，**引擎介面零變動**。混搭規則：單位模式檔先吃固定金額，剩餘按 % 模式檔群內權重攤（B 沿用缺口比例分配）；C 賣方也加 `💱 賣出 %` vs `🎯 賣出單位數`。`ui/tab3_t7_ledger.py` +472 行；fast lane 640 passed / apptest 110 passed 零回歸。SPEC §3-AX 同步。
...
> 🆕 **v18.207 / 2026-05-24** — Tab2 三 AI 整併為唯一 `render_ai_summary_widget`。user 回饋「單一基金有很多 AI，只留一個且要吃全章節資料」。刪 v18.135 `analyze_fund_json` 紅綠燈按鈕、v18.205 個股新聞面 AI、v18.159 末端 `_render_tab2_ai_summary` → 「### ④ AI 深度解盤」單一 widget。全章節快照 `_snap`：基本/績效(1m-1y)/風險1Y(σ/Sharpe/Alpha/Beta from `mj_raw.risk_metrics.risk_table.一年`)/配息/買賣點(buy1-3/bb/ma60)/前10大持股(`_zh_holding`)/產業/三率穿透(`shield_{fk}`)/總經位階(`phase_info_s`)。新聞優先逐股 `_stknews_{fund}`（v18.206，≤15）否則資產類別過濾（≤8）。清理 tab2 unused import `analyze_fund_json` + `event_impact_analysis`。`pytest -m "not slow"` 606 passed、Tab2 AppTest 渲染。沙箱無 GEMINI_KEY → grep 確認快照變數皆 in scope。
