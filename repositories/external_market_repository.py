...
`fetch_multpl_pe` 兩個 fn 已整段刪除。兩者自 `services/valuation.py`
v19.251 退役後即 **production 0 caller**(全 repo 唯一提及處是 CLAUDE.md
§2.2 那筆 WONTFIX 紀錄本身),Forward P/E 現走
`shared/macro_buckets.py` 的 DESIGN literal。依 CLAUDE.md §-1.5.1c 判定 3
「未被任何模組引用(Orphaned)的過期代碼應實體刪除」處置;歷史留在 git。
"""
