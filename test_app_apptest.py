...
def test_tab4_backtest_button_with_two_funds_runs_without_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """注入 2 檔同 policy_id 的 loaded mock fund → 點「▶ 執行回測」→ 不應 exception。

    回歸目的：保護 Tab4 整條回測流程 — 包含
    (1) bt_mode radio 預設「按保單組合」→ bt_policy_pid selectbox 自動選 P001
    (2) bt_codes_from_pf / _bt_policy_weights 推導
    (3) run_backtest_btn 點擊後 cache-only path（_bt_force_refetch 預設關閉）
    (4) 月底 resample / equity / drawdown / Sharpe 計算鏈不可炸 exception
    """
    monkeypatch.setenv("FRED_API_KEY", "test-fred-key")
    monkeypatch.setenv("GEMINI_API_KEY", "test-gemini-key")
    ...
    app.secrets["FRED_API_KEY"] = "test-fred-key"
    app.secrets["GEMINI_API_KEY"] = "test-gemini-key"
    # 2 檔同 P001、各 600 個交易日 NAV，invest_twd 用來導權重
    f1 = _mock_loaded_fund(code="0050", name="元大台灣50", base=100.0, n_days=600)
    f1["invest_twd"] = 60000
    f2 = _mock_loaded_fund(code="0056", name="元大高股息", base=50.0, n_days=600)
    f2["invest_twd"] = 40000
    app.session_state["portfolio_funds"] = [f1, f2]
    app.run()

    assert not app.exception, \
        f"app.py startup exception: {[str(e) for e in app.exception]}"

    btn_key = "run_backtest_btn"
    found = [b for b in app.button if b.key == btn_key]
    assert found, (
        f"找不到 Tab4 「▶ 執行回測」按鈕 key={btn_key}；"
        f"現有 buttons keys: {[b.key for b in app.button]!r}"
    )
    # 按鈕應 enabled（disabled=False，因為 bt_all_codes 非空）
    assert not found[0].disabled, \
        "「▶ 執行回測」按鈕在 2 檔 loaded fund 同 policy 下應 enabled"

    found[0].click().run()
    assert not app.exception, (
        f"點擊「▶ 執行回測」後 runtime exception: "
        f"{[str(e) for e in app.exception]}"
    )

    # 驗證點按後渲染：應該至少有 caption / info / metric 提到 backtest 相關文字
    info_blobs = " ".join(
        str(getattr(e, "value", e) or "") for e in app.info
    )
    caption_blobs = " ".join(
        m.value for m in app.caption if isinstance(getattr(m, "value", None), str)
    ) if hasattr(app, "caption") else ""
    markdown_blobs = " ".join(m.value for m in app.markdown if isinstance(m.value, str))
    combined = info_blobs + " " + caption_blobs + " " + markdown_blobs
    # 寬鬆驗證：cache-only path 通常會顯「淨值擷取完成」/「年化」/「Sharpe」/「Max DD」
    # 任何一個關鍵字命中即可（不同分支會渲染不同字串，但必有結果區）
    hit = any(kw in combined for kw in [
        "淨值擷取", "年化", "Sharpe", "Max DD", "回測結果",
        "資料不足", "有效月底淨值",  # 即使資料不足分支也算 ok（mock NAV 是合成）
    ])
    assert hit, (
        f"點擊回測後結果區無任何 backtest 關鍵字渲染；"
        f"combined 前 500 字: {combined[:500]!r}"
    )
