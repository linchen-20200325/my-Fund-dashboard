...
- [x] **PR (next)** `v18.252` Tab2 投資試算改 TWD 優先 — 用戶反饋「請改成由台幣換算成原幣再買單位數，再算當下每 100 萬可以配多少息；配息要由原幣換回台幣」。`services/fund_service.py` 新 `calc_twd_investment(twd, nav, adr%, fx)` 純函式：TWD ÷ FX → 原幣 ÷ NAV → 單位數 → × NAV × ADR% → 配息原幣 → × FX → 配息 TWD/月。`ui/tab2_single_fund.py:1198+` 加「🪙 投資試算」expander（投入 TWD `number_input` + NAV/ADR/FX 資訊卡 + 4 metrics：可申購單位數 / 年化配息原幣 / 月配息 TWD / 年化配息率 TWD 實際）；FX 不可用退回原幣模式。中文「美元/歐元」↔ ISO 雙向 normalize。test_calc_twd_investment +9 case 含**截圖數字驗算**（NAV=12.24 / ADR=9.1% / TWD=1M / FX=32 → 91,000 TWD/年 / 7,583 TWD/月 ✓）+ FX/NaN/zero edge cases + round-trip invariant（FX 對沖後實際殖利率 = ADR）。**0 → 9 passed**。
