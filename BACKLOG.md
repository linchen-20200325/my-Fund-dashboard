...
## ⏸ 待審查 — 總表 Section 02 三張快覽卡（2026-09-03 批次二）

> **狀態：待審查，不是待辦。** 客戶已核准的線框
> （`docs/wireframes/ia-wireframe.html`，2026-09-01 拍板）把 Section 02 定為 5 卡網格，
> （⚠️ 2026-09-04 稽核 F5 就地更正：本行原寫「客戶已核准的線框」而未指名檔案，
> 其他兩處更引用了「**客戶拍板線框批次二**」這個**不在版控裡的名字**——
> `docs/wireframes/README.md` 正是為了禁止這種引用而存在。**有意識的更正，不是漏刪。**）
> 其中 5 張已於本批（`_render_top_card_grid`，`ui/tab1_macro.py`）落地：
> （⚠️ 2026-09-04 稽核就地更正:原寫「2 張」但下一行列了 5 個名字,數字與清單自相
> 矛盾。**有意識的更正,不是漏刪** —— 落地的一直是 5 張,錯的是那個數字。）
> 📊 景氣位階、🌊 波動與信用、🌡️ 通膨與利率、💰 熱錢動向、⚠️ 極端風險警語。
> 以下 3 張**明確不做**（線框有列，本批刻意不建）——不得補假資料佔位（§1），
> 也不得悄悄消失不提（§-2 揭露義務）。

> ⚠️ **2026-09-04 稽核 F5 就地更正（有意識的更正，不是漏刪）**：下列三項原本
> 一律寫「**待客戶確認 ×××**」，但那三個問題**客戶已核准的線框
> `docs/wireframes/ia-wireframe.html`（2026-09-01 拍板）Tab 01 全部答了** ——
> 照舊條文走，下一輪會為了已經有答案的問題再跑一次線框往返。
> **舊表述的理由仍然成立**（本批動工時的確有「這三張怎麼呈現」的未決點），
> 只是那些未決點在線框拍板時就已消滅，是本 BACKLOG 沒跟上，不是客戶沒答。
> **⚠️ 三項的狀態仍然是「本輪不做」**：更正的是**理由**，不是**要不要做** ——
> 本條不構成動工授權，要做仍須客戶點名（§-1）。

- [ ] **資產水位建議卡** — 既有邏輯已在 `services/allocation_ladder.py`
      （`allocation_from_composite`）跑，② 依據表下方已有一行文字版。
      ~~待客戶確認要不要收成 Section 02 的第 6 張卡（會讓 3 欄網格變 2 排
      3+3，屬版面異動，需先出線框）。~~
      **線框已答**：`ia-wireframe.html` Tab 01 把它列為「資產水位建議 ／
      核心 70 ／ 衛星 30」，chips 標 **`全寬`＋`三態`**（`class="card span3"`）——
      **全寬 = 它不進 3 欄網格**，所以「會讓網格變 3+3」這個舊理由**與線框
      相反、不成立**。剩下的未決點只有「文字版要不要收掉」，不是版面問題。
- [ ] **新聞情緒卡** — 現況：③ 例外層的「今日關鍵橫幅」＋ `detect_systemic_risk`
      已有系統性風險判讀，但沒有獨立卡片化。
      ~~待客戶確認呈現方式（燈號？情緒分數？）。~~
      **線框已答**：`ia-wireframe.html` Tab 01 該格逐字為「新聞情緒 ／
      **偏空 −0.3** ／ 五個 RSS 來源關鍵字命中，**不做加權平均**」——
      即**情緒分數**（非燈號），且明訂算法邊界。
- [ ] **總經燈號全表卡** — 現況：18 個指標的完整燈號散在下方四時域詳細區
      （長期/中期/短線/拐點），沒有一張總覽全表。
      ~~待客戶確認全表要放 Section 02（會很擠）還是留在原位置加一個
      「跳轉」連結。~~
      **線框已答**：`ia-wireframe.html` Tab 01 把它列為「總經燈號全表 ／
      17 項指標 × 值 / 位階 / 資料日期 / 來源」，並明寫「**欄位多，維持全寬
      橫向捲動，不塌成 3 欄**」（`class="card span3"`）—— 放 Section 02、
      全寬橫向捲動，「會很擠」那個顧慮線框已用「不塌成 3 欄」處理掉。
      ~~⚠️ 線框寫 **17 項**、本 repo 現況 18 項，**數字不一致本輪未查證**，
      真要施作時須先釐清是哪一項的差異。~~
      → **2026-09-04 第三輪稽核 B3 更正（有意識的更正，不是漏刪）**：舊句
      **只列了兩個數字，而實際上有三個，且它引的「本 repo 現況 18」不是 SSOT**。
      **三個數字（全部本輪實測）**：
      - **17** — 客戶線框 `docs/wireframes/ia-wireframe.html` Tab 01「總經燈號全表」逐字；
      - **18** — `ui/tab1_macro.py` 的一句**散文註解**（「不把 18 個指標合成 5 個」）
        與本 BACKLOG 上一行；**兩者都不是 SSOT，是同一個數字被抄了兩份**；
      - **28** — **真正的 SSOT**：`services/macro/us_indicators.py::EXPECTED_INDICATOR_KEYS`
        （該常數自己的 docstring 寫明它是「`fetch_all_indicators` 承諾**嘗試**產出的
        key」，並附 AST 漂移鎖 `tests/test_indicator_inventory.py`）。
      **也就是說：畫面上散文寫的 18 與程式的契約 28 差了 10 個**，而線框的 17
      是第三個獨立來源。**要施作這張卡之前，先釐清客戶要的是哪一組 17／18／28**，
      並一律以 `EXPECTED_INDICATOR_KEYS` 為程式端 SSOT，不得再抄散文裡的數字。
      ⚠️ **本更正只改數字與 SSOT 出處，不改本項「本輪不做」的狀態**（§-1）。
...
> 但**生產端仍然把打平講成「向下」**，據實登記於此。

- **現況**：`services/macro/us_indicators.py::calc_growth_inflation_axis` 的
  `growth_up = growth_score > 0` / `inflation_up = inflation_score > 0` 是**二分**
  運算，把「正負訊號筆數相抵（score 恰為 0.00）」併進「向下」，於是
  `quadrant` / `quad_desc` / `quad_icon` / `quad_color` / `quad_alloc` 這一組
  **封閉四值列舉**在打平時會給出「成長↑ 通膨↓」這種**不成立**的方向敘述。
- **本輪做了什麼**：新增 **schema-additive** 的 `growth_dir` / `inflation_dir`
  （`'none'` / `'tie'` / `'up'` / `'down'`），既有 key **一個字都沒動**；
  卡 3 改吃 `*_dir`，打平走灰態「⬜ 方向不明」。
- **為什麼不在生產端一次修完**：要讓 `quadrant` 表達「方向不明」得長出**第五
  象限**，連帶 `quad_color` / `quad_icon` / `quad_alloc` 都要有第五組值 ——
  那是**設計變更**（會改變畫面上可能出現的燈號集合），不是 bug 修復，
  依 §-1.5 v3 §03-2 ① 屬須先出線框的範圍。
- **風險現況（據實）**：~~`quad_*` 全系列目前 **production 0 caller**
  （2026-09-04 實測 `grep -rn 'quad_\|quadrant' --include=*.py`，除
  `us_indicators.py` 自身與卡 3 已改掉的那段外無命中），故**目前沒有人被誤導**。~~
  → **2026-09-04 第三輪稽核 B5 更正（有意識的更正，不是漏刪；措辭不精確，
  不是事實錯誤）**：稽核**三法交叉驗證後確認了實質宣稱**（無 `growth_up` /
  `inflation_up` 的 production 讀取、無整包 dict 迭代，含 runtime 全快徑埋樁），
  **但「0 caller」與「卡 3 **已改掉的**那段」兩處措辭都不準**：
  卡 3 的**正常分支現在仍然逐一讀 `quad_*`**（`quad_icon` / `quadrant` /
  `quad_color` / `quad_desc` 四個欄位，就在 `_render_top_card_grid` 的
  `else:` 分支裡），它**沒有被改掉，只是被前面的閘門擋在後面**。
  **現行正確表述**：**`quad_*` 恰有一個 production caller（卡 3 的正常分支），
  而它之所以安全，靠的是 `*_dir` 閘門先把「零觀測 / 打平 / 方向不明」分流走** ——
  **不是靠沒有人用它。**
  ⚠️ **這個差別正是 A3 為什麼要修**：那道閘門的向後相容 fallback 原本 **fail-open**，
  一旦漏過去，這個唯一的 caller 就會把打平併進來的 `quad_*` 原封送上畫面
  （實測與 F2 修復前的輸出逐位元組相同）。**「0 caller 所以沒人被誤導」會讓人
  以為這條路是死的；它不是死的，它是被一道閘門擋著的。**
  ⚠️ **「0 caller」取決於「有沒有漏看」，係單組 grep、未經第二組獨立驗證**
  （§-2 規則 5／6）—— 只能當**待驗事項**，**不得**作為「可以放著不修」的前提。
  **解凍條件**：任何新消費者要用 `quadrant` / `quad_*` 之前，先處理本項。

---

## ✅ 已結案 — `calc_macro_phase` 的零／低權重正規化（2026-09-04 第三輪稽核 A1 上游）

> ~~**狀態：已登記，本輪明確**不修**。** 本輪只在**卡片層**擋住（卡 1／卡 5 不夠就走灰態），
> **`calc_macro_phase` 的正規化與 `fetch_all_indicators` 的 `_fred_sources` 行為一個字都沒動**。~~
> ⚠️ **這是上游既有缺陷，它現在就在影響本批以外的顯示** —— 見下方「射程」。
>
> ### ✅ **2026-09-04 第四輪結案（狀態變更，不是漏刪；決策者：客戶明示授權擴大範圍）**
>
> **客戶原話：「fix this defect class once at the producer layer instead of card by card」**
> —— 上面那句「本輪明確不修」是**第三輪**的狀態，**第四輪已被客戶的範圍授權推翻**。
> **舊狀態的理由仍然成立**（改正規化＝改所有消費者看到的數字，屬 §8.4 步驟 4 的範圍問題，
> 總管不得自行拍板）；**被權衡掉的是它的前提** —— 那個範圍**現在有客戶授權了**。
>
> **落地方式與「推薦方案」不同，據實寫明**：推薦方案第一段是「讓 `calc_macro_phase`
> 回 `score=None` + `insufficient_reason`」。**實際沒有這樣做**，理由是它會改掉
> 「`score` 不可能是 None」這條所有消費者都依賴的既有契約（哨兵
> `test_the_producer_can_never_emit_a_none_score` 至今仍綠 —— **本輪未動生產端的算法**）。
> **改採側車（side-car）**：生產端在既有回傳值旁**多掛一個 `support` 物件**
> （`shared/evidence_support.py::EvidenceSupport`），**既有 key 一個都沒動、值一個都沒變**，
> 消費端只讀 `support.sufficient`。這與 §2.2 `calculate_composite_score(provenance_out=...)`
> 當年選側車而非改 signature 的理由**是同一個**。
>
> **生產端（4 個）**：`calc_macro_phase` / `calc_growth_inflation_axis`（`us_indicators.py`）、
> `macro_action_light`（`action_light.py`）、`calculate_composite_score`（`composite_score.py`）。
> **消費端（7 個，全部改讀 support）**：五張快覽卡 ＋ **①結論** ＋ **②依據**。
> ⚠️ **①結論與②依據的行為變更由客戶明示授權**：證據撐不住時改印誠實灰態
> （線框 Rule 04 三要素：標題／缺什麼／去哪補），**不是版面變更、不需新線框**。
>
> **上游三段連鎖的處置，逐段據實交代（不要只讀「已結案」三個字）**：
> - **第 1 段（`_fred_sources` 無條件寫入）**：**未動**。它是 `data_registry` 差集診斷的
>   存在理由，動它屬跨模組資料流變更。
> - **第 2 段（`elif not ind` 守衛恆不觸發）**：**未動**，但**它不再是唯一防線** ——
...
1. **`fetch_all_indicators` 無條件寫入 meta。**
   `services/macro/us_indicators.py` 的 `R["_fred_sources"] = _fred_srcs` 在**所有 `if` 之外**，
   於是**每一支 fetcher 都失敗時，回傳 dict 仍非空**。
2. **呼叫端那道「0 個指標」守衛因此永遠不觸發。**
   `ui/tab1_macro.py` 的 `elif not ind: st.error("❌ 沒有抓到任何總經指標（0 個…）")`
   —— `ind` 因為第 1 點恆為 truthy，**這句錯誤訊息在完全斷線時反而不會出現**。
3. **`calc_macro_phase` 的正規化只用「抓到的」指標當分母。**
   `norm = (earned_w + total_w) / (2 * total_w) * 10`；`total_w == 0` 時走 `else: norm = 5`。

### 實測證據（2026-09-04，真實 `calc_macro_phase`，非推論）

| 情境 | `ind` 內容 | 產出 |
|---|---|---|
| 完全斷線 | 只有 `_fred_sources` meta，0 個指標 | `score=5.0` `phase='擴張'` `phase_color='#00c853'` `advice='股優於債：核心高股息ETF + 衛星AI/半導體，設嚴格停利點'` |
| 18 取 1（VIX 平靜） | `{VIX: 12.0, w=1, s=+1}` | `score=10.0` `phase='高峰'` `advice='高峰期：適度獲利了結，轉向防禦型資產'` |
| 18 取 1（PMI 深收縮） | `{PMI: 41.0, w=2, s=-2}` | `score=0.0` `phase='衰退'` `advice='衰退期：保守為主…'` |

**也就是：分數的極值不是由經濟決定的，是由「哪一支 fetcher 剛好活著」決定的。**
⚠️ `us_indicators.py` 檔內**自己早就寫過這個病的名字**：「**失敗越多、N 越小、畫面越綠……
缺資料偽裝成正常的最上游形態**」——那段講的是診斷頁的差集，但同一個機制在計分路徑上更嚴重。

### 射程：**不只本批**（這是本項必須送客戶的理由）

同一顆 `phase` 也餵給 **①結論**（`macro_action_light(ind, phase["score"])`）與 **②依據**
（健康度表／五桶 summary），**那兩處現在就是這樣出貨的**，本輪**未動**。
**本輪修好的只有 Section 02 的五卡網格。**

### 為什麼本輪不修（§8.4 步驟 4）

- 改 `calc_macro_phase` 的正規化 = **改所有消費者看到的數字**，屬**範圍擴大**，
  依 §8.4 步驟 4 須客戶定範圍，**不得由總管自行拍板**。
- 改 `_fred_sources` 的寫入時機 = 動到 `data_registry` 的差集診斷（那正是它存在的理由），
  屬跨模組資料流變更（§8 觸發條件）。

### 總管推薦方案（§-1.5 v3 `03`-2 請示規範：附推薦、附具體原因、可一鍵決策）

**建議：修，但分兩段、且第二段才動數字。**
- **第一段（低風險，建議先做）**：讓 `calc_macro_phase` 在 `total_w` 不足時
  **回一個顯式的不足旗標**（例如 `score=None` + `insufficient_reason`），
  **不改任何既有的分數計算**——現行 `calc_macro_phase_zpct` 已有
  `status="insufficient_data"` 的先例可比照。所有消費者屆時各自決定怎麼呈現。
  ⚠️ 這會讓「`score` 不可能是 None」這條**現有事實**改變，
  `tests/test_batch2_top_card_grid.py::test_the_producer_can_never_emit_a_none_score`
  是刻意釘住它的哨兵，**做這一段時它應該轉紅，那不是回歸，是預期**。
- **第二段（要客戶點頭）**：①結論／②依據要不要跟著改成灰態。**那會改變客戶每天在看的畫面**，
  屬 §-1.5 v3 `03`-2 ① 的射程，**動工前須先出線框草稿**。

~~⚠️ **本項不構成動工授權**（§-1）。**在客戶裁示前，卡片層的閘門就是唯一的防線。**~~
→ **2026-09-04 已由客戶的範圍授權取代**（狀態變更，不是漏刪）。
**現行：卡片層不再是唯一的防線，生產端才是** —— 卡片層現在只讀 `support.sufficient`，
不再自己重推分母（守衛 `test_no_consumer_rewrites_the_sufficiency_gate_by_hand` 鎖住這一點）。

⚠️ **本項仍有兩件事**沒有**做，據實列出（§-2 規則 6，不得被讀成「全部處理完了」）**：
1. **上游第 1／第 2 段仍未動**（見上方逐段交代）—— 它們不在本輪授權的範圍內。
2. **卡 2（🌊 波動與信用）沒有 `EvidenceSupport` 物件**，它與其餘四張卡不同型：
   它的規則 3（不對稱）是由**既有的** `_radar_light_rank` 排序實現的
   （`⬜ 無資料` 排 0、比 🟢 的 1 還低 → 缺一盞燈永遠不會被當成「最溫和」）。
   **那一段第三輪已驗為 CLEAN 且不可逆轉，本輪刻意未動它** ——
   它的生產端 `services/risk_radar.py` 不在客戶授權的四個生產端清單內。
   **這是已知的不對稱，不是疏漏**；把它一併下沉屬另一次範圍決定（§8.4 步驟 4）。

---

## ⏸ 待客戶裁示 — 「景氣位階」卡的組成與線框不符（2026-09-04 第三輪稽核 B4）

> **狀態：已登記，本輪明確**不改它算什麼**。** 本輪只做了一件事：**讓卡片說出自己是什麼**。

- **線框寫的**：`docs/wireframes/ia-wireframe.html` Tab 01「景氣位階」逐字為
  「**NDC 燈號 ＋ PMI ＋ 殖利率差合成**」。
- **實際跑的**：`services/macro/us_indicators.py::calc_macro_phase` 的**美國**加權評分
  （殖利率曲線 ×2 / PMI / HY / M2 / 廣度 / Fed BS / DXY / VIX / CPI / Fed Rate / 失業率…）。
- **NDC 根本不在裡面**（實測）：`EXPECTED_INDICATOR_KEYS` 28 個 key **無任何 NDC 項**；
  台灣景氣對策信號由 `ui/helpers/macro/ndc.py` **另外抓**，只有 **②依據** 在用。
- **本輪做了什麼**：卡片 label 改為 `…｜美國 N 權重加權合成（未含台灣 NDC 燈號）`。
  守衛：`test_phase_card_discloses_that_it_is_the_us_composite_not_ndc`
  ＋ `test_expected_indicator_keys_really_does_not_contain_ndc`
  （後者釘住事實前提——哪天 NDC 真被加進去，那句免責聲明會變成新的假話）。
- **為什麼不改算法**：把 NDC 併進 `calc_macro_phase` = **改變客戶要的那個數字是什麼**，
  屬 §7 第 4 點的「定義」面、§-1.5 v3 `03`-2 ② 的業務規則，**必須客戶裁示**。

**總管推薦方案**：**建議維持現況（美國合成）+ 保留本輪的揭露，不併入 NDC。**
理由：(a) 這張卡的下游（①結論的 `macro_action_light`、②依據的健康度）全部建立在
**美國**這一套刻度上，硬併 NDC 會讓同一個分數同時代表兩個經濟體；
(b) 台灣景氣對策信號**已經**在②依據獨立呈現，客戶要的資訊沒有少。
**若客戶要的是「台股視角的位階」，那是一張新卡，不是改這一張** —— 屆時走 UI 線框草稿。

⚠️ **本項不構成動工授權**（§-1）。

---

## ⏸ 已登記未做 — 熱錢快覽卡的 session stash 沒有 TTL（2026-09-04 第三輪稽核 A5 衍生）

> **狀態：已登記，本輪不做。** 本輪修的是「**閘門被哪些入口清掉**」（三個入口全部補齊），
> **沒有**碰「stash 要不要自己過期」。

- **現況**：L1 `fetch_foreign_flow_series` / `fetch_usdtwd_series` 是 `TTL_30MIN`，
  但卡片的 `_hm_card_frames` stash **只在 session 內寫一次、之後永不重讀** ——
  一個長 session 裡，**這張卡與它下方的 ARCHIVED expander 可以顯示不同 vintage 的資料**。
- **為什麼本輪不做（不是漏掉，是有張力）**：F6 那道閘門的**存在理由**就是「不要每次 rerun
  重打網路」（`ui/tab1_macro_longterm.py` 記過 v19.340 的 AppTest 教訓：refused 連線的
  retry 會逐 rerun 累積）。**加 TTL ＝ 把那個設計反過來一半**，要同時滿足兩邊
  （失敗冷卻要短於成功 TTL、過期後必須真的重試、且不得回到連線風暴），
  屬**設計變更**而非 bug fix，需要自己的一輪規格與測試。
- **緩解現況**：本輪把三個刷新入口全部接上了，使用者**有**明確且真的有效的手動出口，
  且失敗態的 label 現在就指向那三個入口。
- **觸發點**：(1) 出現「卡片與下方 expander 數字不一致」的實際回報；
  (2) 有任務碰到 `_hm_card_*` 的生命週期；(3) user 直接點名。

⚠️ **本項不構成動工授權**（§-1）。

  新消費者一律**必須讀 `*_dir`**，不得用 `*_up` 判方向。

---

## ⏸ 凍結 — 境內基金「含息 3 年年化」(2026-08-11 user 拍板)

> **狀態：凍結，不是待辦。** 沒有解凍條件成立之前**不要重新開挖** ——
> 這一項在 2026-08-11 那一輪被重複調查了三次，每次都回到同一個結論。

- **現象**：user 5 檔境內持倉（`ACCP138` / `ACDD01` / `ACDD19` / `ACTI71` / `ACTI94`）
  的  3-3-3 恆顯示 ⬜。
- **根因（不是抓取失敗，是來源不存在）**：3-3-3 的 C2 定義是**含息**3 年年化，
  全站唯一來源是 MoneyDJ `wb01`，而**境內基金頁沒有 wb01/wb05/wb07**。
  舊的 `_fetch_domestic_perf` 拿基金代碼去打需要**公司代碼**（BFxxxx）的
  `yp020000`，每檔每次浪費 2 次 MoneyDJ 請求、結果注定為空。
- **`nav_history` 每日累積救不了這一項**：它累的是**純 NAV**，不含息。
  累到 2029 年 C2 依然無解（其他指標如 Sharpe / σ / MaxDD / Sortino **會**因累積而解鎖）。
- **已做**：`_fetch_domestic_perf` 改為不發請求、恆回 `{}`，docstring 寫明理由與解凍條件；
  `services/fund_screening.py` C2 那段的錯誤註解（宣稱 `ret_3y_ann` 是
  「MoneyDJ 算好的含息總報酬」——實際是 `fund_service` 從 NAV 自算、且不含息）已更正；
  `tests/test_domestic_perf_frozen.py` 守「不得偷偷解凍」。
- **解凍條件（任一成立即可重啟，需連同該測試一起改）**：
  1. 找到境內基金的**含息**報酬來源（投信官網 / TDCC / SITCA / 投信投顧公會）；
  2. 或改用「NAV + 配息還原」自算含息總報酬，並在 3-3-3 標示為自算口徑（§2.2 須可分辨）；
  3. 或由 user 用對帳單 CSV 匯入含配息的還原淨值。
- **未決（等 user 拍板）**：3-3-3 對這 5 檔目前顯示的是通用的 ⬜「資料不足」，
  讀起來像「暫時抓不到」。要不要改成明確的「境內無此來源（已凍結）」——
  會動到健診表「②來源」欄與 Tab② 的顯示，故另案。
...
- [x] **PR #166** `v18.118` AI-1 —  advisor prompt 接 Phase 4 driver + Phase 3-B 燈號回測
...
- [x] **PR #193** `v18.135` — 單一基金 +  AI 加「持股 × 新聞」交叉分析（fund_json 4→5 節）
...
- [x] **PR #160 — v18.91：老師名字 → 策略 1/2/3（陳=1 / 孫=2 / =3）— UI + AI prompt**（`8506487`, merged）— 使用者要求「把所有老師的名字拿掉，改用策略 1、2、3 表示」。固定對應：陳重銘→策略1、孫慶龍→策略2、老師→策略3。**範圍**（per user choice 「只改 UI 文字 + AI prompt」）：app.py 美林時鐘 4 段老師語音 + 三合一圖 caption + 戰情室 + Tab6 講義 +  AI 按鈕標題 + error message；ai_engine.py 3 個 prompt 自我介紹段 + docstring；mk_dashboard.py 「郭老師白話文」expander × 3 + 80/20 metric + help text；mk_clock.py expander 標題 + 教學示意 caption；macro_engine.py 10Y-2Y 翻正 signal text。不動 function/file/variable name 與 `#` 註解。grep 確認殘留「老師//老師/陳重銘/孫慶龍」全在 # 註解或 module docstring（不顯示給使用者）。27 smoke tests passed。
...
- [x] **PR #147 — v18.82-83：C 分區 + 搬位 + 回測按保單模式 + Tab5 資料診斷去重**（`8ccddd9`, merged）— 連續 4 個使用者反饋一次到位：(1)「C 賣方/買方放一起會看錯」→ expander 內三段式：📉 紅區（漸層深紅 + 4px 左紅邊）→ 漸層分隔線 → 📈 綠區（漸層深綠）+ 賣方標題加序號「🔁 #N」+ 買方權重總和綠/紅燈即時顯示；(2)「組合基金下方缺乏 AI 組合分析」→ root cause：v18.81  expander 寫在 `_panel_ph.container()` 裡而 placeholder 是在 A/B/C tabs **上方**創建的，渲染進去 = 顯示在 tabs 上方（使用者在 C tab 操作完往下看當然看不到），搬出 placeholder dedent 到 col 12 後 expander 渲染在 A/B/C 下方 + 加紫色 banner 標題卡片 + 「🗑️ 清除上次結果」按鈕；(3)「回測請以保單為一個組合個別回測」→ Tab4 加 radio：🎯 按保單組合（推薦）/ 🔧 自訂選擇，按保單模式選保單 → 自動納入該保單所有已載入基金 + 按 invest_twd 比例配權重 + 即時 dataframe 預覽（代碼/名稱/投資金額/權重）+ 權重 readonly st.metric 顯示；(4)「基金資料診斷很多重複，移除重複只保留單一」→ Tab5 按 code dedup（19 筆 → 7 unique）+ caption「移除 N 筆同 code 跨保單重複」+ expander 標題加標籤「· 共 N 保單共用」讓使用者知道該 code 跨幾張保單。249 fast tests passed。

- [x] **PR #145 — v18.81：T7 三件套大改（P&L bug + A 新投入多選 + 老師深度建議）**（`2c801dc`, merged）— 使用者貼 T7 截圖反饋三件事一次來：(1)「整體未實現損益 (TWD) 計算有錯」全表 +0.00% 報酬率；(2)「新投入也讓我可以多選標的，由保單先選，再來選標的」；(3)「ETf 組合下方也需要放一個 老師的建議」涵蓋缺點 / 換股 / 配置 / 高賣低買 vs 跌就買 + 系統性風險。**Fix**：(a) v18.81-A — auto-estimate 原本用 `_d_t7.today()` 與 latest NAV 當 cost basis → cost = current → P&L 永遠 0。新增 `_nav_at_date_t7(fund, date_str)` helper (pd.Series.asof) 取**投資日期當天 NAV** 做 cost、subscribe 用該日期，cost_unit ≠ current NAV → 真實 P&L 浮現；既有 ledger 守護不覆寫（要重估按重置帳本）；(b) v18.81-B — A 新投入單一 selectbox 改 policy selectbox + fund multiselect（必須放 form 外才能 reactive），form 內動態 render 每檔 number_input + 「📊 合計投入 NT$X,XXX」即時加總，submit handler 改 loop subscribe + sheet rows batch sync，暫存/直接套用兩種落帳都支援多檔；(c) v18.81-C — 新函式 `ai_engine.analyze_portfolio_mk_advisor(api_key, pf, phase, ledgers, ind)` 強制 4 節結構化 prompt：🚨 3 大缺點 / 🔄 對應景氣位階的換股 / ⚖️ 核心衛星現金 % / 🎯 高賣低買 vs 跌就買 + ⚠️ 系統性風險警示根據 VIX/T10Y2Y 給結論；位置：T7 帳本下方獨立 expander 預設收合，紅燈提醒先載總經。249 fast tests passed。
...
- [x] **PR #81 — v18.38：真實收益 vs 配息率矩陣按 code 去重**（`a117a89`, merged）— 截圖顯示長條圖出現多條同 code 跨保單的重複條目（19 條中有大量重複），右側產生空紅框且 KPI 數字被放大污染。`_loaded_pf` 計算後新增按 code 去重，與 v18.34 戰情室 / v18.36 T5 矩陣策略一致。
- [x] **PR #79 — v18.37：投資組合主清單按保單號碼分組成 expander**（`a0b96d8`, merged）— v18.35 per-fund expander 取代為「外層保單 expander」（一張保單 = 一個摺疊區塊；點開後 KPI + advice + 訊號全部攤平顯示）。Streamlit 禁止 expander 巢狀，因此移除內層 expander。無 policy_id 的基金歸「(未綁保單)」群組。🗑️ 刪除按鈕仍以原 portfolio index 運作。
...
- [x] **PR #76 — v18.35：投資組合主清單每檔詳細區包進 expander（預設收合）**（`d7cbd32`, merged）— Tab3 主清單迴圈內，把「💡 advice 建議帶 + 📍 訊號 6 格方塊」兩塊包進 `st.expander("📋 詳細建議 + 訊號", expanded=False)`，上方 KPI 列（名稱/NAV/配息/Sharpe/σ/🗑️）永遠可見。多檔基金時版面長度減約 60%。
...
- [x] **PR #73 — v18.34：戰情室分析視圖按 code 去重**（`986ac2f`, merged）— `render_mk_war_room()` 內 `loaded` 計算後新增按 code 去重（保留第一筆）；同基金綁多保單時，KPI 卡 / 核心戰情室表 / 波段觀測站表 / 衛星 vs SPY 折線圖只算一筆。Tab3 portfolio_funds 與 T7 帳本仍以 `(code, policy_id)` 複合鍵分流，不影響獨立記帳。223 passed / 0 failed。
...
- [x] PR #41 — 戰情室 expander 巢狀錯誤修復（**stock repo**, `1e9a0d4`, merged）
