...
def _display_max_below(value: float, ndigits: int) -> float:
    ...
    **下緣**(`value ≤ d - step/2`),那麼所有嚴格小於 `value` 的真值都只顯示得到
    `d` 的前一格;否則答案就是 `d` 本身。
    ⚠️ **這裡的容差方向是 fail-OPEN,不是 fail-closed —— 據實寫明,不要寫反。**
    退一格會讓上界變小 ⇒ 區間變窄 ⇒ **比較容易**判為「充足」。所以容差開太大
    就會放行不該放行的。取 `step/2 * (1 - 1e-9)`(相對 1e-9)的理由:
    `value` 由除法算出,真的落在分界點上時 `d - value` 只會差 `±1e-16` 等級,
    而 `1e-9 * step/2 ≈ 5e-11` 已經比那個誤差大四個數量級、又比任何**真實**的
    非分界距離小得多(相位帶最小刻度是 0.1)。
    ⚠️ **這是工程判斷,不是證明。** 佐證是量化:缺 0~3 項、密集格點(每格 5 個
    格內偏移)共 137k+ 個狀態,「契約說充足但生產端會翻帶」的數目在每一個缺項數上
    **都是 0**(見本輪 commit 說明的表)。**不是證明,是量到的。**
    """
